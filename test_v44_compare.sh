...
#
# Usage: bash test_v44_compare.sh
...
cp "$DECODERS_FILE" "${DECODERS_FILE}.bak_v44"
cp "$NNX_DECODERS_FILE" "${NNX_DECODERS_FILE}.bak_v44"

restore() {
    cp "${DECODERS_FILE}.bak_v44" "$DECODERS_FILE"
    cp "${NNX_DECODERS_FILE}.bak_v44" "$NNX_DECODERS_FILE"
    rm -f "${DECODERS_FILE}.bak_v44" "${NNX_DECODERS_FILE}.bak_v44"
}
trap restore EXIT
...
v44_total="N/A"; v44_temp="N/A"; v44_tps="N/A"; v44_loss="N/A"
...
run_variant() {
    ...
    local HLO_DIR="${OUTPUT_DIR}/hlo_v44_${name}"
    rm -rf "${HLO_DIR}"
    mkdir -p "${HLO_DIR}"
    unset XLA_FLAGS
    ...
    python -m maxtext.trainers.pre_train.train \
        "${COMMON_ARGS[@]}" \
        run_name="${name}" \
        base_output_directory=/dev/shm/v44/${name} \
        dump_hlo=true \
        dump_hlo_local_dir="${HLO_DIR}" \
        dump_hlo_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/hlo_v44_${name}" \
        dump_hlo_delete_local_after=false \
        dump_jaxpr=true \
        dump_jaxpr_local_dir="${OUTPUT_DIR}/jaxpr_v44_${name}" \
        dump_jaxpr_delete_local_after=false \
        dump_jaxpr_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/jaxpr_v44_${name}" \
        2>&1 | tee "$log_file"
...
printf "%-12s %-12s %-12s %-12s %s\n" "v44" "$v44_total" "$v44_temp" "$v44_tps" "$v44_loss"
echo ""

# Post-run: check HLO while loop count
echo "=== HLO While Loop Count ==="
for name in baseline v44; do
    hlo=$(find "${OUTPUT_DIR}/hlo_v44_${name}" -name "*jit_train_step*before_optimizations*" 2>/dev/null | head -1)
    if [ -n "$hlo" ]; then
