...
case_04() {
  run_case 04 nnx "RL GRPO smoke (Qwen3-0.6B)" \
    "${RL_MOD} model_name=qwen3-0.6b tokenizer_path=Qwen/Qwen3-0.6B chips_per_vm=4 num_batches=2 \
     rollout_data_parallelism=2 trainer_devices_fraction=1.0 sampler_devices_fraction=1.0 \
...
case_06() {
  run_case 06 nnx "RL GRPO functional (Llama3.1-8B)" \
    "${RL_MOD} model_name=llama3.1-8b tokenizer_path=meta-llama/Llama-3.1-8B-Instruct chips_per_vm=4 num_batches=2 \
     rollout_data_parallelism=2 trainer_devices_fraction=1.0 sampler_devices_fraction=1.0 \
     async_scheduling=False log_config=False enable_goodput_recording=False profiler=xplane debug.rl=False \
