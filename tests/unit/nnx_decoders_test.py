...
class TestNNXDecoderDeepseek4(unittest.TestCase):
  ...
  def test_get_decoder_layers_registers_deepseek4(self):
    ...
    A missing entry makes get_decoder_layers raise ValueError at __init__ (before any DEEPSEEK4
    dispatch runs), which silently regressed once during a branch reset. Non-scan -> DecoderLayer,
    scan -> ScannableBlock (mirrors Linen decoders.py).
    """
