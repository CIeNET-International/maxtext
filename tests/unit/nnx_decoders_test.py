...
class TestNNXDecoderDeepseek4(unittest.TestCase):
  ...
  def _assert_decoder_grad_parity(self, scan_layers):
    """Gradient COMPARISON through the FULL DeepSeek-V4 decoder: two numerically-transparent remat
    policies -- 'full' (save nothing) vs 'minimal' -- run the same math (both go through the same
    jax.checkpoint in _apply_layers_sequentially; only the save-policy differs), so they must yield
    the same loss and gradients (wrt every decoder + embedding Param), plus a real (nonzero) backward.

    Exact grad equality is asserted on CPU only: on accelerators the recomputed float32 forward rounds
    differently and can flip an indexer-top8 / MoE-top2 routing near-tie, legitimately perturbing a few
    gradient elements (see _assert_grad_parity_platform_aware). Loss parity holds on all platforms."""
...
class TestNNXPipelineStages(unittest.TestCase):
  ...
  def _assert_stage_grad_parity(self, stage_cls, num_layers=2):
    ...
    _assert_grad_parity_platform_aware(
        self, [xgrad_ref], [xgrad_remat], what="stage remat input-grad"
    )
