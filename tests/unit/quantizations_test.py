...
class QuantTest(unittest.TestCase):
  ...
  def quantization_config(self, quant, logits_tolerance=2e-1, grad_tolerance=5e-1, scan_layers=False):
    ...
    if quant not in ("fp8_gpu", "fp8_nanoo"):
      # FP8 GPU uses qwix.quantize_model which might change state
      import qwix
      qt_provider = quantizations.get_qt_provider(cfg)
      qt_model = qwix.quantize_model(qt_model, qt_provider, ids, decoder_positions, decoder_segment_ids, enable_dropout=False)
