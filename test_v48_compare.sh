#!/bin/bash
# V48: Manual loop splitting (Solution 5).
#
# Splits the inner microbatch scan and outer repeat scan each into TWO
# jax.lax.scan calls. Each jax.lax.scan compiles to one HLO while loop, so
# 2 logical scans -> 4 logical scans -> 8 HLO while loops (fwd+bwd each),
# matching Linen's 8 (NNX baseline emits 4).
#
# More while loops = more XLA scheduling boundaries = better buffer packing.
# If V48 -> ~23.3 GB: loop count drove the 6.6 GB gap. Loss MUST match 0.049.
#
# Usage: bash test_v48_compare.sh
...
cp "$DECODERS_FILE" "${DECODERS_FILE}.bak_v48"
cp "$NNX_DECODERS_FILE" "${NNX_DECODERS_FILE}.bak_v48"

restore() {
    cp "${DECODERS_FILE}.bak_v48" "$DECODERS_FILE"
    cp "${NNX_DECODERS_FILE}.bak_v48" "$NNX_DECODERS_FILE"
    rm -f "${DECODERS_FILE}.bak_v48" "${NNX_DECODERS_FILE}.bak_v48"
}
trap restore EXIT
...
v48_total="N/A"; v48_temp="N/A"; v48_tps="N/A"; v48_loss="N/A"
...
run_variant() {
    ...
    local HLO_DIR="${OUTPUT_DIR}/hlo_v48_${name}"
    rm -rf "${HLO_DIR}"
    mkdir -p "${HLO_DIR}"
    unset XLA_FLAGS
    ...
    python -m maxtext.trainers.pre_train.train \
        "${COMMON_ARGS[@]}" \
        run_name="${name}" \
        base_output_directory=/dev/shm/v48/${name} \
        dump_hlo=true \
        dump_hlo_local_dir="${HLO_DIR}" \
        dump_hlo_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/hlo_v48_${name}" \
        dump_hlo_delete_local_after=false \
        dump_jaxpr=true \
        dump_jaxpr_local_dir="${OUTPUT_DIR}/jaxpr_v48_${name}" \
        dump_jaxpr_delete_local_after=false \
        dump_jaxpr_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/jaxpr_v48_${name}" \
        2>&1 | tee "$log_file"
...
echo "================================================================"
echo "V48: Manual loop splitting — 2 scans -> 4 scans -> 8 HLO while loops"
echo "Date: $(date)"
echo "================================================================"
echo ""
echo "Mechanism: split inner microbatch scan + outer repeat scan, each into"
echo "           two jax.lax.scan calls, threading the carry across the split."
echo ""
echo "If V48 → ~23.3 GB: more while loops drove the 6.6 GB gap"
echo "If V48 → ~29.9 GB: while loop count is not the cause"
echo "Loss at step 2 MUST match baseline (0.049)."
echo ""

run_variant "baseline" "pipeline"
run_variant "v48" "pipeline_v48"
...
printf "%-12s %-12s %-12s %-12s %s\n" "v48" "$v48_total" "$v48_temp" "$v48_tps" "$v48_loss"
echo ""

# Post-run: check HLO while loop count
echo "=== HLO While Loop Count ==="
for name in baseline v48; do
    hlo=$(find "${OUTPUT_DIR}/hlo_v48_${name}" -name "*jit_train_step*before_optimizations*" 2>/dev/null | head -1)
    if [ -n "$hlo" ]; then
