#!/bin/bash
# Test pipeline_n3.py — the golden-pattern NNX implementation
# Runs correctness test + memory measurement.
#
# Redirects the pipeline imports (decoders.py, nnx_decoders.py, test file) to
# `pipeline_n3` via sed, runs, then ALWAYS restores via trap. Pass a different
# variant as $1 (e.g. `bash test_pipeline_n3.sh pipeline_n3_sharding`).
set -uo pipefail   # NOT -e: a failing pytest must not skip the memory runs.

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
cd "$SCRIPT_DIR" || exit 1
OUTPUT_DIR="${SCRIPT_DIR}/variant_test_results_n3"
mkdir -p "${OUTPUT_DIR}/logs"

VARIANT="${1:-pipeline_n3}"
...
echo "=========================================="
echo "PIPELINE N3 TEST SUITE — variant: ${VARIANT}"
echo "=========================================="
...
# Test 2: Memory measurement (Linen decoder + NNX pipeline)
echo ""
echo "--- Test 2: Memory + Throughput (Linen decoder) ---"
python -m maxtext.trainers.pre_train.train maxtext/configs/base.yml \
    run_name="n3_linen_decoder" \
...
# Test 3: Memory measurement (NNX decoder + NNX pipeline)
echo ""
echo "--- Test 3: Memory + Throughput (NNX decoder) ---"
python -m maxtext.trainers.pre_train.train maxtext/configs/base.yml \
    run_name="n3_nnx_decoder" \
    base_output_directory="${OUTPUT_DIR}" \
    model_name=llama2-7b \
    dataset_type=synthetic \
    steps=15 \
...
# Test 4: HLO analysis (compile-only)
echo ""
echo "--- Test 4: HLO Dump (compile-only) ---"
