#!/bin/bash
# V46: Solution 2 -- full pipeline_utils.py port into the NNX circular pipeline.
#
# NNXCircularPipeline.__call__ is rewired to run the *exact* Linen path:
#   create_gradient_accumulation_scan -> Level 1+2 custom_vjp
#   create_pipeline_stage            -> Level 3 custom_vjp (linear_transpose)
#   create_flax_pipeline_scan        -> nn.scan(nn.remat(...)), triggers _partial_pack
# A real nn.Module adapter (_PipelineAdapter) bridges the Linen-signature
# run_one_iteration / weight_prefetching / get_pipeline_remat_policy that
# pipeline_utils expects onto NNXCircularPipeline's NNX-signature methods.
#
# Validates the claim: 6.6 GB gap is from _partial_pack scope isolation.
# If V46 → ~23.3 GB: claim proven. If → 29.9 GB: something else matters.
#
# Usage: bash test_v46_compare.sh
...
cp "$DECODERS_FILE" "${DECODERS_FILE}.bak_v46"
cp "$NNX_DECODERS_FILE" "${NNX_DECODERS_FILE}.bak_v46"

restore() {
    cp "${DECODERS_FILE}.bak_v46" "$DECODERS_FILE"
    cp "${NNX_DECODERS_FILE}.bak_v46" "$NNX_DECODERS_FILE"
    rm -f "${DECODERS_FILE}.bak_v46" "${NNX_DECODERS_FILE}.bak_v46"
}
trap restore EXIT
...
v46_total="N/A"; v46_temp="N/A"; v46_tps="N/A"; v46_loss="N/A"
...
run_variant() {
    ...
    local HLO_DIR="${OUTPUT_DIR}/hlo_v46_${name}"
    rm -rf "${HLO_DIR}"
    mkdir -p "${HLO_DIR}"
    unset XLA_FLAGS
    ...
    python -m maxtext.trainers.pre_train.train \
        "${COMMON_ARGS[@]}" \
        run_name="${name}" \
        base_output_directory=/dev/shm/v46/${name} \
        dump_hlo=true \
        dump_hlo_local_dir="${HLO_DIR}" \
        dump_hlo_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/hlo_v46_${name}" \
        dump_hlo_delete_local_after=false \
        dump_jaxpr=true \
        dump_jaxpr_local_dir="${OUTPUT_DIR}/jaxpr_v46_${name}" \
        dump_jaxpr_delete_local_after=false \
        dump_jaxpr_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/jaxpr_v46_${name}" \
        2>&1 | tee "$log_file"
...
echo "================================================================"
echo "V46: Solution 2 — full pipeline_utils.py port"
echo "Date: $(date)"
echo "================================================================"
echo ""
echo "Claim: 6.6 GB gap is from _partial_pack scope isolation."
echo "V46: NNXCircularPipeline routed through pipeline_utils 3-level"
echo "     custom_vjp + create_flax_pipeline_scan(nn.scan(nn.remat(...)))"
echo "     via a real nn.Module adapter (_PipelineAdapter)."
echo ""
echo "If V46 → ~23.3 GB: claim PROVEN"
echo "If V46 → ~29.9 GB: something else matters beyond _partial_pack"
echo ""

run_variant "baseline" "pipeline"
run_variant "v46" "pipeline_v46"
...
printf "%-12s %-12s %-12s %-12s %s\n" "v46" "$v46_total" "$v46_temp" "$v46_tps" "$v46_loss"
echo ""

# Post-run: check HLO while loop count
echo "=== HLO While Loop Count ==="
for name in baseline v46; do
    hlo=$(find "${OUTPUT_DIR}/hlo_v46_${name}" -name "*jit_train_step*before_optimizations*" 2>/dev/null | head -1)
    if [ -n "$hlo" ]; then
