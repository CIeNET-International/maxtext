...
def maybe_shard_with_name(
  ...
  if (
    ...
    if not sharding_desc:
      ...
      if frame is not None:
        ...
        if len(x) > 0:
          ...
          sharding_desc = f"{caller_filename[:-3]}/{x[0]}"
    ...
    if log_key not in _LOGGED_ACTIVATION_SHARDINGS:
      ...
      st = logging._fast_stack_trace()
      # max_logging.info(f"stack_trace: {sti}" for sti in tuple(st))
      i = 1
      for sti in st:
        space = " " * i
        i += 1
        max_logging.info(f"{space} {sti}")
      """
  ...
  if debug_sharding and not sharding_desc:
    ...
    if frame is not None:
      ...
      if len(x) > 0:
        ...
        caller_filename = Path(caller_path_full).name
        sharding_desc = f"{caller_filename[:-3]}/{x[0]}"
        # max_logging.info(f"maybe_shard_with_logical extra_stack_level: {extra_stack_level} x={x}")
