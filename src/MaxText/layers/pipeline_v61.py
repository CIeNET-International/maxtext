...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    outer_nothing_policy = jax.checkpoint_policies.nothing_saveable()
    ...
    # Inner body ALSO gets checkpoint — controls per-microbatch memory
    if self.config.set_remat_policy_on_pipeline_iterations:
    ...
    def outer_body(carry, _):
      """One repeat: remat wraps the WHOLE repeat (prefetch + inner scan)."""
      current_loop_state, current_layer_mutables, w_curr = carry

      # The repeat step: remat wraps this function which CONTAINS jax.lax.scan.
      # During backward, remat recomputation re-executes the inner scan →
      # separate while loop for recomputation (small carry, low peak memory).
      ...
      if self.config.set_remat_policy_on_pipeline_iterations:
        repeat_step = jax.checkpoint(
            repeat_step, policy=outer_nothing_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )
    ...
    if bubble_iterations > 0:
      ...
      if self.config.set_remat_policy_on_pipeline_iterations:
        bubble_step = jax.checkpoint(
            bubble_step, policy=outer_nothing_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )
