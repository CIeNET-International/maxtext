...
"""Pipeline V36: 3-level custom_vjp matching Linen's pipeline_utils.py structure.

HLO analysis showed Linen produces 8 while loops (vs NNX's 4) due to
nn.remat + _partial_pack creating separate recomputation boundaries.
V36 ports the 3-level custom_vjp from pipeline_utils.py to NNX:

  Level 1: Per-microbatch custom_vjp + jax.remat
           → Creates separate recomputation while loops in HLO
  Level 2: Scan over microbatches with d+g gradient accumulation
           → Separates scan backward from forward
  Level 3: Per-repeat weight handling with linear_transpose
           → Prevents replicated weights in residuals

Goal: produce ~8 while loops in HLO → better XLA buffer scheduling → lower temp.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V36: 3-level custom_vjp matching pipeline_utils.py ----
    #
    # Level 1: Per-microbatch custom_vjp + jax.remat
    #   → jax.vjp(jax.remat(run_one_step)) creates separate recomputation while loops
    # Level 2: Scan over microbatches with d+g gradient accumulation on BSW
    #   → jax.vjp(scan(level1_fn)) separates scan forward/backward
    # Level 3: Per-repeat weight handling with linear_transpose
    #   → Prevents replicated weights in scan residuals
    #
    # Goal: produce ~8 HLO while loops (vs baseline's 4) for better XLA scheduling
    num_microbatches = self.config.num_pipeline_microbatches
    remat_policy = self.get_pipeline_remat_policy() if self.config.set_remat_policy_on_pipeline_iterations else None

    # ---- Level 1: Per-microbatch custom_vjp + remat ----
    # Matches pipeline_utils.py::create_gradient_accumulation_scan::run_single_microbatch_custom

    def _run_one_step(lightweight_state, bsw):
      """Pure forward for one microbatch. Closure captures loop-invariant state."""
      ...
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, bsw, layers_graph, layers_metrics,
    ...
    def run_single_microbatch_fwd(lightweight_state, bsw):
      ...
      if remat_policy is not None:
        _run = jax.remat(_run_one_step, policy=remat_policy)
      out, vjp_fun = jax.vjp(_run, lightweight_state, bsw)
      return out, vjp_fun
    ...
    # ---- Level 2: Scan over microbatches with d+g accumulation ----
    # Matches pipeline_utils.py::create_gradient_accumulation_scan::run_pipeline_microbatches_custom

    @jax.custom_vjp
    def run_microbatches(loop_state_and_mutables, bsw):
      return run_microbatches_fwd(loop_state_and_mutables, bsw)[0]

    def run_microbatches_fwd(loop_state_and_mutables, bsw):
      if self.config.scan_pipeline_iterations:
        final_lightweight, scan_vjp_fun = jax.vjp(
            ...
            loop_state_and_mutables, bsw,
        )
      else:
        def _unrolled(ls, b):
          carry = ls
          for _ in range(num_microbatches):
            carry = run_single_microbatch(carry, b)
          return carry

        final_lightweight, scan_vjp_fun = jax.vjp(
            _unrolled, loop_state_and_mutables, bsw,
        )

      return (final_lightweight, bsw), scan_vjp_fun

    def run_microbatches_bwd(residuals, g_final_state):
    ...
    run_microbatches.defvjp(run_microbatches_fwd, run_microbatches_bwd)

    # ---- Level 3: Per-repeat weight handling ----
    # Matches pipeline_utils.py::create_pipeline_stage::execute_pipeline_stage_pure

    @jax.custom_vjp
    def execute_repeat(loop_state_and_mutables, w_curr, pipeline_weights):
      return execute_repeat_fwd(loop_state_and_mutables, w_curr, pipeline_weights)[0]

    def execute_repeat_fwd(loop_state_and_mutables, w_curr, pipeline_weights):
      current_loop_state = loop_state_and_mutables[0]
      iteration = current_loop_state["loop_iteration"]
      ...
      weight_prefetching_t = jax.linear_transpose(p_weight_prefetching, pipeline_weights)

      (final_lightweight, _), scan_microbatches_vjp = jax.vjp(
          run_microbatches, loop_state_and_mutables, bsw,
      )

      return (final_lightweight, w_next), (scan_microbatches_vjp, weight_prefetching_t)

    def execute_repeat_bwd(residuals, g_outputs):
      ...
      g_loop_state_and_mutables, g_bsw = scan_microbatches_vjp((g_lightweight, g_bsw))

      g_w_curr, g_w_next = g_bsw
      (g_pipeline_weights,) = weight_prefetching_t(g_w_next)

      return g_loop_state_and_mutables, g_w_curr, g_pipeline_weights

    execute_repeat.defvjp(execute_repeat_fwd, execute_repeat_bwd)

    # ---- Outer body: one repeat ----
    def outer_body(carry, _):
      current_loop_state, current_layer_mutables, w_curr = carry
      (new_ls_mut, w_next) = execute_repeat(
          (current_loop_state, current_layer_mutables), w_curr, layers_params,
      )
      new_loop_state, new_layer_mutables = new_ls_mut
      return (new_loop_state, new_layer_mutables, w_next), layers_metrics
    ...
    if bubble_iterations > 0:
      def make_bubble_body(bsw):
        def bubble_body(carry, _):
          current_loop_state, current_layer_mutables = carry
          out = _run_one_step((current_loop_state, current_layer_mutables), bsw)
          return out, None
        if remat_policy is not None:
          bubble_body = jax.checkpoint(bubble_body, policy=remat_policy)
        return bubble_body

      bubble_bsw = (final_w_curr, final_w_curr)
      bubble_body = make_bubble_body(bubble_bsw)
    ...
    # Metrics: use layers_metrics placeholder (actual metrics in module state via nnx.update)
