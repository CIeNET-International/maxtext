...
"""Pipeline V11b: Params in carry, NO stop_gradient.

Variant of V11 that tests whether putting layers_params in the scan carry
(making it visible to XLA's buffer planner as an explicit function arg)
helps memory WITHOUT the stop_gradient fence.

V11 achieved 16.4 GB but killed gradients via stop_gradient on layers_params.
V11b tests the hypothesis that carry placement alone changes XLA buffer
scheduling, while letting gradients flow through normally.

Changes from V11:
  - REMOVED: jax.lax.stop_gradient on carried_layers_params (NNXPipeline.scan_body)
  - REMOVED: jax.lax.stop_gradient on carried_layers_params (NNXCircularPipeline.outer_body)
  - KEPT: layers_params in carry tuple (structural change from V11)
  - KEPT: All V10 stop_gradient calls on w_curr and current_layer_mutables
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V11b: layers_params in scan carry so XLA sees it as an explicit arg rather
    # than an opaque closure capture. NO stop_gradient -- gradients flow through
    # the carry path normally. Testing whether carry placement alone helps memory.
    def scan_body(carry, _):
      current_loop_state, current_layer_mutables, carried_layers_params = carry
      # V11b: NO stop_gradient on carried_layers_params -- let gradients flow
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      # V11b: Use carried_layers_params (explicit arg) instead of closure-captured layers_params
      ...
      # V11b: Pass carried_layers_params back unchanged in the output carry
    ...
    if self.config.scan_pipeline_iterations:
      # V11b: layers_params is now part of the initial carry tuple
      ...
      # V11b: layers_params is now part of the carry tuple
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  V11b: layers_params in outer scan carry WITHOUT stop_gradient, testing whether
  carry placement alone changes XLA buffer scheduling while preserving gradients.
  """
  ...
  def __call__(
    ...
    #
    # V11b: layers_params is in outer scan carry WITHOUT stop_gradient.
    #   Unlike V11, gradients flow through the carry path normally.
    #   Testing whether carry placement alone changes XLA buffer scheduling.
    bsw_ref = [None]
    ...
    def outer_body(carry, _):
      ...
      V11b: layers_params is an explicit carry element WITHOUT stop_gradient.
      Gradients flow through carry normally; testing carry placement alone.
      """
      current_loop_state, current_layer_mutables, w_curr, carried_layers_params = carry
      iteration = current_loop_state["loop_iteration"]

      # V11b: NO stop_gradient on carried_layers_params -- let gradients flow
      # through the carry path. Testing whether carry placement alone helps memory.
