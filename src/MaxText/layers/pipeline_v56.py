...
V56: Two-level custom_vjp matching Linen's exact HLO structure (8 while loops).
Level 1 (per-microbatch): jax.remat + jax.vjp around run_one_iteration
  - Returns FULL (loop_state, layer_state) -- no nnx.split inside vjp boundary
  - Gradient cotangent matches naturally -- no State.merge needed
Level 2 (per-repeat): jax.vjp over scan_microbatches + linear_transpose
  - scan_vjp_fn triggers Level 1's remat recomputation in backward
  - Creates SEPARATE while loops for recomputation (the 8-loop pattern)
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  """V56: NNX circular pipeline with two-level custom_vjp matching Linen's 8-loop HLO.

  Produces 8 HLO while loops (matching Linen nn.scan(nn.remat(body)) + custom_vjp):
    - Forward scan (inner microbatches)
    - Tiny remat recomputation scan (5 elements: 2 activations + 3 indices)
    - Backward gradient scan using recomputed activations
    - Above 3 appear TWICE (once per repeat section)
    - Forward scan (outer repeats)
    - Backward scan (outer repeats)

  Two-level custom_vjp structure:
    Level 1 (run_iter_custom): per-microbatch jax.remat + jax.vjp
      - Returns FULL (loop_state, layer_state) -- NO nnx.split inside vjp boundary
      - Gradient cotangent matches naturally -- no State.merge reconstruction
    Level 2 (execute_repeat_pure): per-repeat jax.vjp over scan_microbatches
      - scan_vjp_fn triggers Level 1's remat recomputation in backward
      - linear_transpose for weight_prefetching gradient propagation
      - Creates SEPARATE while loops for recomputation (the 8-loop pattern)

  Key difference from V52: Level 1 _run_iter_bwd passes (g_ls, g_layer_state)
  directly to vjp_fn -- NO nnx.State.merge reconstruction needed because
  new_layer_state flows through the scan body BEFORE being split.
  """
  ...
  def __call__(
    ...
    # ---- V56: Two-level custom_vjp nested scan structure ----
    #
    # Level 1 (per-microbatch): jax.remat + jax.vjp around run_one_iteration
    #   - Creates remat checkpoint for each microbatch iteration
    #   - Returns FULL (loop_state, layer_state) -- no nnx.split inside vjp boundary
    #   - Gradient cotangent matches naturally because output = input to vjp_fn
    #
    # Level 2 (per-repeat): jax.vjp over scan_microbatches + linear_transpose
    #   - When Level 2's _bwd calls scan_vjp_fn(g), it triggers recomputation
    #     via Level 1's jax.remat, creating SEPARATE recomputation while loops
    #   - linear_transpose propagates gradients through weight_prefetching
    #
    # This produces 8 HLO while loops matching Linen's pattern:
    #   Forward inner scan x2, Remat recomputation scan x2,
    #   Backward gradient scan x2, Forward/Backward outer scan x2
    num_microbatches = self.config.num_pipeline_microbatches
    remat_policy = self.get_pipeline_remat_policy()

    # --- Level 1: per-microbatch custom_vjp (matches run_single_microbatch_custom) ---
    # KEY: NO nnx.split inside this boundary. Return full (loop_state, layer_state).
    # The vjp cotangent structure matches naturally because new_layer_state
    # flows through the scan body BEFORE being split.

    @jax.custom_vjp
    def run_iter_custom(loop_state, bsw, mutables):
    ...
    def _run_iter_fwd(loop_state, bsw, mutables):
      ...
      advanced_mutables = _advance_rng_state(mutables, iteration)

      def _run(ls, b):
        return self.run_one_iteration(
            ls, b, layers_graph, layers_metrics,
            advanced_mutables, positions, segment_ids,
    ...
    def _run_iter_bwd(vjp_fn, g_out):
      ...
      # Pass cotangent directly -- structure matches (new_ls, new_layer_state)
      # from jax.vjp(_run_remat, loop_state, bsw). No State.merge needed.
      d_ls, d_bsw = vjp_fn((g_ls, g_layer_state))
      return d_ls, d_bsw, None  # None for mutables (stop_gradient)

    run_iter_custom.defvjp(_run_iter_fwd, _run_iter_bwd)

    # --- Level 2 (MIDDLE): per-scan custom_vjp ---
    # Matches Linen's run_pipeline_microbatches_custom.
    # Wraps the jax.lax.scan itself in @custom_vjp so the scan backward
    # is OPAQUE — forces JAX to create a SEPARATE while loop for
    # recomputation instead of inlining it into the outer backward.
    @jax.custom_vjp
    def scan_microbatches_custom(loop_state, bsw, layer_mutables):
      return _scan_microbatches_fwd(loop_state, bsw, layer_mutables)[0]

    def _scan_microbatches_fwd(loop_state, bsw, layer_mutables):
      def _inner_body(carry, _):
        ls, muts = carry
        muts = jax.lax.stop_gradient(muts)
        new_ls, new_layer_state = run_iter_custom(ls, bsw, muts)
        _, _, new_metrics, new_muts = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        return (new_ls, new_muts), new_metrics

      # jax.vjp over the scan — captures backward closure
      # When _bwd calls scan_vjp_fn, it triggers a SEPARATE scan
      # for recomputation (via Level 1's jax.remat)
      (final_carry, scan_metrics), scan_vjp_fn = jax.vjp(
          lambda ls, b, muts: jax.lax.scan(
              _inner_body, (ls, muts), None, length=num_microbatches,
          ),
          loop_state, bsw, layer_mutables,
      )
      # Return (final_carry, bsw) matching Linen's pattern
      return (final_carry, bsw, scan_metrics), scan_vjp_fn

    def _scan_microbatches_bwd(scan_vjp_fn, g_out):
      (g_carry, g_muts), g_bsw, g_metrics = g_out
      # Trigger recomputation — this creates the separate while loop
      d_ls, d_bsw, d_muts = scan_vjp_fn(((g_carry, g_muts), g_metrics))
      # Accumulate bsw gradients from output + scan backward
      d_bsw_total = jax.tree.map(
          lambda a, b: a + b if hasattr(a, 'shape') else a, d_bsw, g_bsw
      )
      return d_ls, d_bsw_total, d_muts

    scan_microbatches_custom.defvjp(_scan_microbatches_fwd, _scan_microbatches_bwd)

    def scan_microbatches(loop_state, bsw, layer_mutables):
      (final_carry, _, scan_metrics), = scan_microbatches_custom(loop_state, bsw, layer_mutables),
      return final_carry, scan_metrics

    # --- Level 2: per-repeat custom_vjp (matches execute_pipeline_stage_pure) ---
    @jax.custom_vjp
    def execute_repeat_pure(loop_state, layer_mutables, w_curr, pipeline_weights):
      return _execute_repeat_fwd(loop_state, layer_mutables, w_curr, pipeline_weights)[0]

    def _execute_repeat_fwd(loop_state, layer_mutables, w_curr, pipeline_weights):
      iteration = loop_state["loop_iteration"]
      w_next = self.weight_prefetching(pipeline_weights, physical_partition_spec_full, iteration)
      bsw = (w_curr, w_next)

      p_wp = functools.partial(
          self.weight_prefetching,
          physical_partition_spec=physical_partition_spec_full,
          loop_iteration=iteration,
      )
      weight_prefetching_t = jax.linear_transpose(p_wp, pipeline_weights)

      # jax.vjp over inner scan -- THIS creates the separate recomputation loop
      # because scan_microbatches uses Level 1 custom_vjp with jax.remat
      (scan_carry, scan_metrics), scan_vjp_fn = jax.vjp(
          scan_microbatches, loop_state, bsw, layer_mutables
      )
      new_ls, new_muts = scan_carry

      return (new_ls, new_muts, w_next, scan_metrics), \
             (scan_vjp_fn, weight_prefetching_t)

    def _execute_repeat_bwd(residuals, g_outputs):
      scan_vjp_fn, weight_prefetching_t = residuals
      g_ls, g_muts, g_w_next, g_metrics = g_outputs

      # Backprop through inner scan -- triggers Level 1 remat recomputation
      g_init_ls, g_bsw, g_init_muts = scan_vjp_fn(
          ((g_ls, g_muts), g_metrics)
      )
      g_w_curr_from_scan, g_w_next_from_scan = g_bsw

      g_w_next_total = jax.tree.map(lambda a, b: a + b, g_w_next, g_w_next_from_scan)
      (g_pipeline_weights,) = weight_prefetching_t(g_w_next_total)

      return g_init_ls, g_init_muts, g_w_curr_from_scan, g_pipeline_weights

    execute_repeat_pure.defvjp(_execute_repeat_fwd, _execute_repeat_bwd)

    # --- Outer scan ---
    def outer_body(carry, _):
      current_loop_state, current_layer_mutables, w_curr = carry
      new_ls, new_muts, w_next, inner_metrics = execute_repeat_pure(
          current_loop_state, current_layer_mutables, w_curr, layers_params
      )
      return (new_ls, new_muts, w_next), inner_metrics
    ...
    # Use Level 1 directly for bubble (no repeat-level custom_vjp needed)
    if bubble_iterations > 0:
      ...
      def bubble_body(carry, _):
        ls, muts = carry
        muts = jax.lax.stop_gradient(muts)
        new_ls, new_layer_state = run_iter_custom(ls, bsw_bubble, muts)
        _, _, new_metrics, new_muts = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        return (new_ls, new_muts), new_metrics
