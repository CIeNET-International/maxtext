...
V53: Params in outer scan carry + @jax.custom_vjp, matching Linen's exact
architecture. pipeline_weights is the 4th carry element (like Linen's
self.layers.variables). custom_vjp prevents carry-induced backward blowup
by using jax.linear_transpose for reduce_scatter.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  """NNX circular pipeline with params in outer scan carry + custom_vjp.
  ...
  Key design decisions:
    - pipeline_weights in outer scan CARRY (like Linen's self.layers.variables)
    - @jax.custom_vjp on outer body prevents carry-induced backward blowup
    - jax.linear_transpose for reduce_scatter of weight gradients
    - inner scan uses BSW from bsw_ref closure (unchanged from base)

  Paired experiment with base pipeline.py:
    - Base: params in closure, no custom_vjp
    - V53: params in carry, with custom_vjp
    - If V53 matches Linen's memory -> custom_vjp is the key
    - If V53 memory > Linen -> something else in Linen's _partial_pack/nn.scan is doing more
  """
  ...
  def __call__(
    ...
    # ---- Nested scan structure (V53: params in carry + custom_vjp) ----
    #
    # Matches Linen's architecture exactly:
    #   - pipeline_weights in outer scan CARRY (like Linen's self.layers.variables)
    #   - @jax.custom_vjp on outer body prevents carry-induced backward blowup
    #   - jax.linear_transpose for reduce_scatter of weight gradients
    #   - inner scan uses BSW from bsw_ref closure (unchanged from base)
    ...
    # Inner scan takes BSW as explicit arg (not bsw_ref closure) to enable
    # jax.vjp capture in _execute_repeat_fwd. Matches V52 pattern.
    ...
    # --- Per-repeat custom_vjp (params in carry, gradient via linear_transpose) ---
    # Uses jax.vjp over scan_microbatches to capture backward closure,
    # then weight_prefetching_t for reduce_scatter. Params in carry.
    @jax.custom_vjp
    ...
    def _execute_repeat_fwd(loop_state, layer_mutables, w_curr, pipeline_weights):
      ...
      # jax.vjp over inner scan to capture backward closure
    ...
    def _execute_repeat_bwd(residuals, g_outputs):
      ...
      # Combine w_next gradients from output + scan backward
      ...
      # Return real g_w_curr (not zeros — it flows back through outer carry)
    ...
    def outer_body(carry, _):
      """One repeat: params from carry, prefetch w_next, run microbatches."""
      current_loop_state, current_layer_mutables, w_curr, pipeline_weights = carry
      new_ls, new_muts, w_next, inner_metrics = execute_repeat_pure(
          current_loop_state, current_layer_mutables, w_curr, pipeline_weights
      )
      # pipeline_weights passes through unchanged in carry
      return (new_ls, new_muts, w_next, pipeline_weights), inner_metrics
    ...
    if self.config.scan_pipeline_iterations:
      ...
      # NOTE: layers_params is now in carry (4th element), matching Linen
      (loop_state, final_layer_mutables, final_w_curr, _), repeat_metrics = jax.lax.scan(
    ...
    if bubble_iterations > 0:
      ...
      def bubble_body(carry, _):
        ...
        advanced_mutables = _advance_rng_state(muts, iteration)
        new_ls, new_layer_state = self.run_one_iteration(
            ls, bsw_bubble, layers_graph, layers_metrics,
            advanced_mutables, positions, segment_ids, deterministic,
            model_mode, logical_partition_spec_stripped,
        )
        _, _, new_layer_metrics, new_layer_mutables = nnx.split(
      ...
      if self.config.set_remat_policy_on_pipeline_iterations:
        bubble_body = jax.checkpoint(
            bubble_body, policy=remat_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )
