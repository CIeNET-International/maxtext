...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V58: Remat on OUTER body (contains scan) ----
    #
    # Key insight: jax.remat wrapping a function that CONTAINS jax.lax.scan
    # produces SEPARATE recomputation while loops during backward.
    # (ad_checkpoint.py:793-796 — if jaxpr_rematted contains scan_p,
    # recomputation traces a separate scan_p → separate while loop)
    #
    # Base pipeline: jax.checkpoint on INNER body (no scan inside) → inlined
    # V58: jax.remat on OUTER body (scan INSIDE) → separate recomp loops
    # This matches Linen's nn.scan(nn.remat(stage_fn)) where stage_fn
    # internally contains jax.lax.scan.
    bsw_ref = [None]
    num_microbatches = self.config.num_pipeline_microbatches
    remat_policy = self.get_pipeline_remat_policy()

    # Inner body: checkpoint controls per-microbatch memory
    ...
    # --- V60: Different checkpoint policies for inner vs outer ---
    # Inner: nothing_saveable — save ZERO intermediates per microbatch.
    #   Minimal storage per scan iteration. Everything recomputed.
    # Outer: pipeline remat policy — save checkpoint-named tensors.
    #   Wraps scan → separate recomputation while loop.
    #
    # This avoids double-remat overhead: inner saves nothing (cheap),
    # outer recomputes the scan (separate loop with minimal carry).
    inner_nothing_policy = jax.checkpoint_policies.nothing_saveable()

    if self.config.set_remat_policy_on_pipeline_iterations:
      inner_body = jax.checkpoint(
          inner_body, policy=inner_nothing_policy,
    ...
    def outer_body(carry, _):
      ...
      def repeat_step(ls, muts, wc):
        ...
        if self.config.scan_pipeline_iterations:
          (new_ls, new_muts), inner_metrics = jax.lax.scan(
              inner_body, (ls, muts), None, length=num_microbatches,
          )
        else:
          inner_carry = (ls, muts)
          inner_metrics_list = []
      ...
      new_loop_state, new_layer_mutables, w_next, inner_metrics = repeat_step(
    ...
    if bubble_iterations > 0:
      ...
      def bubble_step(ls, muts):
        bsw_ref[0] = (final_w_curr, final_w_curr)
        if self.config.scan_pipeline_iterations:
          return jax.lax.scan(
              inner_body, (ls, muts), None, length=bubble_iterations
          )
        else:
          carry = (ls, muts)
          metrics_list = []
