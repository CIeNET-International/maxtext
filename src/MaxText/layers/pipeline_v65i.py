...
"""V65i: Fixed Jacky pipeline — pure JAX, no custom_vjp at scan level, SPMD-friendly.

Based on pipeline_test_jax_cond.py (21.3 GB) with gradient fixes:
1. Dict mutation fixed (create new dict instead of in-place)
2. jax.lax.cond replaced with unconditional prefetch
3. Level 1 custom_vjp per-microbatch (inside inner scan, doesn't block outer SPMD)
"""
