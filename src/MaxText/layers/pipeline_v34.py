...
"""Pipeline V34: custom_vjp to prevent replicated weights in scan carry residuals.

Per Flax author feedback: the extra memory comes from fully-replicated (post
all-gather) w_curr being stored in the outer scan carry. Scan saves all
intermediate carry values for backward, so replicated weights get saved at
each repeat iteration.

Fix: wrap the outer body in a custom_vjp (matching Linen's
execute_pipeline_stage_pure) that:
1. Forward: runs inner scan, saves only (scan_vjp, weight_prefetching_t)
2. Backward: uses linear_transpose for reduce-scatter, avoids saving replicated weights
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    #
    # outer scan (repeats):
    #   1. All-gather weights once -> BSW (w_curr, w_next)
    #   2. Pass BSW to make_inner_body() which captures it as closure
    #   3. Run inner scan over microbatches
    #
    # inner scan (microbatches):
    #   1. BSW captured via closure from make_inner_body (NOT mutable list)
    #   2. Run one pipeline iteration (forward through decoder)
    #   3. jax.checkpoint wraps this -- recomputes forward during backward
    #
    # V34 fix: bsw_ref[0] = bsw inside custom_vjp/vjp scope leaked
    # LinearizeTracer. Factory pattern captures BSW as proper JAX closure
    # constant — tracer stays within transform scope.
    num_microbatches = self.config.num_pipeline_microbatches

    def make_inner_body(bsw):
      """Factory: inner scan body with BSW captured as closure (not mutable list)."""
    ...
    # V34: custom_vjp boundary matching Linen's execute_pipeline_stage_pure.
    # Prevents replicated (post-all-gather) w_curr/w_next from being saved
    # as outer scan carry residuals. Instead, saves only:
    #   1. scan_vjp (inner scan backward function)
    #   2. weight_prefetching_t (linear transpose for reduce-scatter)

    def _run_inner_scan(lightweight_state, bsw):
      """Run inner microbatch scan. BSW captured via factory closure (no mutable list)."""
      current_loop_state, current_layer_mutables = lightweight_state
      body = make_inner_body(bsw)
      if self.config.scan_pipeline_iterations:
        ...
        for _ in range(num_microbatches):
          inner_carry, step_metrics = body(inner_carry, None)
    ...
    def execute_repeat(loop_state_and_mutables, w_curr, pipeline_weights):
      """One repeat with controlled residuals. Matches Linen execute_pipeline_stage_pure."""
    ...
    def execute_repeat_fwd(loop_state_and_mutables, w_curr, pipeline_weights):
      ...
      # Prefetch next repeat's weights (FSDP all-gather → replicated)
      w_next = self.weight_prefetching(pipeline_weights, physical_partition_spec_full, iteration)
      bsw = (w_curr, w_next)

      # Capture linear_transpose for reduce-scatter in backward
      ...
      # Run inner scan with jax.vjp to capture backward function.
      # _run_inner_scan returns ((loop_state, mutables), inner_metrics).
      # We need the full output for vjp cotangent matching.
      (final_lightweight, inner_metrics), scan_vjp = jax.vjp(
          _run_inner_scan, loop_state_and_mutables, bsw
      )

      # Return: (lightweight_state, w_next) as output
      # Residuals: (scan_vjp, weight_prefetching_t, metrics_template) — NO replicated weights saved
      return (final_lightweight, w_next), (scan_vjp, weight_prefetching_t, inner_metrics)

    def execute_repeat_bwd(residuals, g_outputs):
      g_lightweight, g_w_next = g_outputs
      scan_vjp, weight_prefetching_t, inner_metrics_template = residuals
      ...
      # Zero cotangent for inner_metrics (not differentiated)
      g_inner_metrics = jax.tree.map(jnp.zeros_like, inner_metrics_template)

      # Backpropagate through inner scan.
      # scan_vjp cotangent matches _run_inner_scan output: ((ls, mut), metrics)
      # scan_vjp returns cotangent of inputs: (lightweight_state, bsw)
      g_loop_state_and_mutables, g_bsw_out = scan_vjp((g_lightweight, g_inner_metrics))
      # Now add the incoming BSW gradient from the outer scan carry
      g_bsw_out = jax.tree.map(
          lambda d, g: d + g if hasattr(d, "shape") else d,
          g_bsw_out, g_bsw,
      )

      # Apply linear transpose (reduce-scatter) to get FSDP-sharded gradients
    ...
    def outer_body(carry, _):
      """One repeat: custom_vjp-controlled weight handling."""
      current_loop_state, current_layer_mutables, w_curr = carry

      (new_ls_mut, w_next), = (
          execute_repeat((current_loop_state, current_layer_mutables), w_curr, layers_params),
      )
      new_loop_state, new_layer_mutables = new_ls_mut

      # Metrics: use layers_metrics placeholder (actual metrics in module state)
    ...
    if bubble_iterations > 0:
      bubble_body = make_inner_body((final_w_curr, final_w_curr))
      ...
      # V34: repeat_metrics are placeholders (layers_metrics stacked by scan).
      # Bubble metrics come from actual inner_body runs. Use layers_metrics as final.
