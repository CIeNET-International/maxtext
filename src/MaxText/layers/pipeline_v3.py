...
"""Pipeline V3: Remove _stamp_at_current_trace + jax.vmap + explicit checkpoint args.

Variant of pipeline.py with the following changes:
  1. _stamp_at_current_trace is a true no-op (returns input unchanged)
  2. jax.vmap already in place (baseline has it)
  3. inner_body passes BSW as explicit arg to checkpoint via _checkpointed_step
"""
...
class NNXPipelineBase(nnx.Module):
  ...
  def _stamp_at_current_trace(weights):
    """V3: True no-op — jax.vmap doesn't need trace stamping.

    The original dynamic_slice identity trick created new arrays at the
    current trace level. With jax.vmap (not nnx.vmap), this is unnecessary
    because jax.vmap handles trace levels natively without nnx.State
    threading overhead.
    """
    return weights  # No-op — jax.vmap doesn't need trace stamping
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V3: Separate the checkpointed computation into a function that takes
    # BSW as an explicit argument, making it visible to jax.checkpoint's
    # saved-residual policy instead of hidden in a closure reference.
    def _checkpointed_step(bsw_arg, loop_state, layer_mutables):
      """Core iteration step with BSW as explicit arg for checkpoint visibility."""
      iteration = loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          loop_state, bsw_arg, layers_graph, layers_metrics,
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      _checkpointed_step = jax.checkpoint(
          _checkpointed_step, policy=self.get_pipeline_remat_policy(),
          prevent_cse=not self.config.scan_pipeline_iterations,
      )

    def inner_body(carry, _):
      current_loop_state, current_layer_mutables = carry
      # V3: Pass BSW as explicit arg to the checkpointed step
      return _checkpointed_step(bsw_ref[0], current_loop_state, current_layer_mutables)
