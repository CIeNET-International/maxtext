...
class AttentionOp(nnx.Module):
  ...
  def cudnn_flash_attention(
    ...
    if self.attention_type == AttentionType.LOCAL_SLIDING or using_context_parallelism:
      ...
      dummy_attn_mask = jnp.zeros((1, 1, 1, self.config.max_target_length, self.config.max_target_length), dtype=jnp.uint8)
    ...
    dummy_query_prefill = jnp.zeros((1, self.max_target_length, self.num_query_heads, self.config.head_dim), dtype=self.dtype)
    dummy_key_prefill = jnp.zeros((1, self.max_target_length, self.num_kv_heads, self.config.head_dim), dtype=self.dtype)
    dummy_value_prefill = jnp.zeros((1, self.max_target_length, self.num_kv_heads, self.config.head_dim), dtype=self.dtype)
