...
V51 variant: @jax.custom_vjp on the per-repeat outer_body.

Mechanism: jax.checkpoint always inlines recomputation into the caller's
while loop. @jax.custom_vjp makes _bwd opaque to JAX, so a jax.lax.scan
inside _bwd produces a SEPARATE scan primitive and thus a SEPARATE HLO
while loop. This is the first variant to test explicit recomputation via
custom_vjp on the outer body (per-repeat level), while keeping the existing
jax.checkpoint on the inner body (per-microbatch level).

Expected HLO: 5+ while loops (vs 4 for baseline with jax.checkpoint).
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  """V51: NNX circular pipeline with @jax.custom_vjp on outer_body.
  ...
  V51 change: The outer_body (per-repeat) is wrapped with @jax.custom_vjp.
  The custom _bwd explicitly recomputes the forward via jax.vjp(outer_body_impl),
  which traces jax.lax.scan(inner_body) as a SEPARATE scan primitive. This
  produces an additional HLO while loop for backward recomputation, unlike
  jax.checkpoint which always inlines recomputation into the enclosing loop.
  ...
  Key design decisions:
    ...
    - @jax.custom_vjp on outer_body: _bwd is opaque to JAX, inner scan
      in _bwd traces as separate primitive -> separate HLO while loop.
    - jax.checkpoint on inner_body is KEPT for per-microbatch remat.
  """
  ...
  def __call__(
    ...
    # ---- V51: outer_body with @jax.custom_vjp ----
    #
    # Rationale: jax.checkpoint always inlines recomputation into the
    # enclosing while loop (the outer scan). @jax.custom_vjp makes the
    # backward pass opaque to JAX. Inside _bwd we call jax.vjp on the
    # implementation function, which contains jax.lax.scan(inner_body).
    # That scan traces as a SEPARATE primitive, producing an ADDITIONAL
    # HLO while loop for recomputation. This is the mechanism that could
    # produce the 8-while-loop structure matching Linen's nn.remat.
    #
    # The inner body's jax.checkpoint is kept unchanged -- it controls
    # per-microbatch recomputation within the inner scan.

    def outer_body_impl(carry, _):
    ...
    def outer_body(carry, _):
      return outer_body_impl(carry, _)

    def outer_body_fwd(carry, _):
      """Forward pass: run outer_body_impl, save carry for recomputation."""
      result = outer_body(carry, _)
      # Save the input carry as residuals for backward recomputation.
      # We do NOT save the result -- _bwd will recompute it via jax.vjp.
      return result, carry

    def outer_body_bwd(carry_res, g):
      """Backward pass: EXPLICIT recomputation from saved carry.

      jax.vjp traces through outer_body_impl which contains
      jax.lax.scan(inner_body). This scan traces as a SEPARATE
      primitive (not inlined like jax.checkpoint would), producing
      an additional HLO while loop for the backward recomputation.

      Args:
        carry_res: The saved carry from outer_body_fwd (input to the
            forward pass). Structure: (loop_state, layer_mutables, w_curr).
        g: Gradient of the output. Structure matches
            ((loop_state, layer_mutables, w_next), inner_metrics).
      """
      g_carry_out, g_metrics = g
      # Recompute forward and get VJP function.
      # outer_body_impl takes (carry, _) where _ is the scan input (None).
      # We differentiate w.r.t. the carry argument only.
      _, vjp_fn = jax.vjp(lambda c: outer_body_impl(c, None), carry_res)
      (d_carry,) = vjp_fn((g_carry_out, g_metrics))
      # Return gradients for (carry, _). The _ scan input is None/unused.
      return d_carry, None

    outer_body.defvjp(outer_body_fwd, outer_body_bwd)
