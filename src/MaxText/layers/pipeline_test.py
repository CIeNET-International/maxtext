...
class Pipeline(nnx.Module):
  ...
  def get_current_repeat_from_stages(self, weights, loop_iteration, physical_partition_spec=None):
    ...
    # Calculate which repeat step each stage is currently on
    _, repeat_ids = self.get_microbatch_and_repeat_ids(loop_iteration)

    # Define the gather function
    # We assume the weights coming in (from nnx.state) have the shape:
    # [num_repeats, num_stages, ...param_dims...]
    ...
    # Apply to the PyTree
    # We removed meta.remove_axis and circular_metadata_params entirely.
    if physical_partition_spec is None:
  ...
  def get_weight_sharding(self):
    """Returns the PartitionSpec tree for the model weights."""
    # 1. Extract the parameters/variables
    state = nnx.state(self.layers)

    # 2. Define a helper to extract the sharding spec from nnx.Param
    def get_spec(leaf):
      # In MaxText NNX, params usually have a .sharding attribute
      # containing the PartitionSpec (if set via with_partitioning)
      if hasattr(leaf, "sharding") and isinstance(leaf.sharding, PartitionSpec):
        return leaf.sharding
      # Fallback or default
      return PartitionSpec()

    # 3. Map over the state tree
    # Note: We need to recreate the logical->physical mapping if using logical axes
    # But often in NNX MaxText, the sharding is stored physically on the Param.
    # If using LogicalPartitioner logic, we mimic the Linen behavior:

    def get_partition_spec_linen_style(leaf):
      # If your params utilize the 'LogicallyPartitioned' wrapper or equivalent in NNX port:
      return getattr(leaf, "get_partition_spec", lambda: None)()

    # Using the static helper we preserved
    partition_spec_tree = jax.tree.map(get_spec, state)

    # If the weights don't have the 'stage' dim yet (single layer), we might need to adjust,
    # but usually get_weight_sharding returns the spec for the *logical* model.
  ...
  def run_one_iteration(
    ...
    stages_inputs = jax.checkpoint(lambda x: x, policy=jax.checkpoint_policies.save_anything)(stages_inputs)
    ...
    # --- Handling Circular Repeats & Weight Gathering ---
    if self.config.num_pipeline_repeats > 1:
      _, repeat_ids = self.get_microbatch_and_repeat_ids(loop_iteration)

      # Helper to gather specific repeat weights
      def gather_weights_for_stages_in(w, spec=None):
        # We assume w has shape [num_repeats, num_stages, ...]
        # We gather based on repeat_ids for each stage
        return self.vmap_parallel_gather(
            w, repeat_ids=repeat_ids, repeat_dim_in_weights=0, stages_dim_in_weights=1, physical_partition_spec=spec
        )

      if physical_partition_spec is None:
        stage_weights = jax.tree.map(gather_weights_for_stages_in, pipeline_weights)
      else:
        stage_weights = jax.tree.map(gather_weights_for_stages_in, pipeline_weights, physical_partition_spec)
    else:
      # Simple pipeline: pipeline_weights already has [num_stages, ...] structure
      # or acts as the source for get_current_stage_weights
      stage_weights = self.get_current_stage_weights(
          pipeline_weights, loop_iteration, physical_partition_spec=physical_partition_spec
      )

    # --- JAX VMAP Implementation ---
    # 1. Get the pure function
    stage_fn_pure = self.get_functional_stage_fn()

    # 2. Vmap it using standard jax.vmap
    # in_axes:
    #   weights: 0 (since we gathered weights per stage)
    #   inputs: 0 (stages_inputs has shape [num_stages, ...])
    #   segment_ids: 0
    #   positions: 0
    #   others: None
    vmapped_stage_fn = jax.vmap(
        stage_fn_pure, in_axes=(0, 0, 0, 0, None, None), out_axes=(0, 0)  # (output, updated_weights)
    )
    ...
    # CRITICAL: You must now merge 'updated_stage_weights' back into 'pipeline_weights'
    # This is complex in Pipelining because we only updated the 'current' stage's slice.
    # You need a scatter/update logic here if you support BatchNorm.
  ...
  def get_functional_stage_fn(self):
    """
    Returns a pure function (graph_def, weights, inputs...) -> output
    that can be vmapped by jax.
    """
    ...
    def stage_fn(weights, inputs, segment_ids, positions, deterministic, model_mode):
      ...
      # Extract MoE Loss / Metrics
      # In MaxText/Linen these were "sown" into 'intermediates'
      # In NNX, we look at the 'intermediates' collection in the split state
      _, new_state = nnx.split(model)

      # Return output AND the state (which contains the metrics/losses)
  ...
  def __call__(
    ...
    # The function for Scan/Loop
    def step_fn(carry_state, _):
      # We discard the second arg (scan input) because we iterate via loop_iteration in carry_state
      new_state = self.run_one_iteration(
          carry_state,
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      # prevent_cse is specific to Flax remat, standard jax.checkpoint handles re-computation
    ...
    if self.config.scan_pipeline_iterations:
      # Use jax.lax.scan
      # We need a dummy input array to scan over, length = total_iterations
