...
V65b: NNX circular pipeline -- params in CARRY (not broadcast).

Hypothesis test
---------------
V65 puts params in ``variable_broadcast='params'``, which means params are
KNOWN in partial eval. The hypothesis is that putting params in CARRY (as a
pass-through, KNOWN element returned OUTSIDE the Level 3 custom_vjp) will
produce a 2nd recomp pair in ``_scan_partial_eval_custom``, matching V46's
memory behavior.

Key change from V65
-------------------
1. Params removed from ``variable_broadcast`` -- no longer broadcast.
2. Params added to user carry: ``carry = (loop_state, w_curr, local_layers_params)``
3. Inside ``_stage_fn_for_scope``: params unpacked from carry (NOT read from scope).
4. Params returned unchanged in carry output OUTSIDE the Level 3 custom_vjp
   (identity pass-through), so their carry output is KNOWN in PE.
5. ``'params'`` collection removed from ``linen_variables`` entirely.

This mirrors V46's pattern where ``pipeline_weights`` is repacked AFTER the
``execute_pipeline_stage_pure`` custom_vjp call:
```python
...
All other V65 features preserved:
  - 3-level custom_vjp nesting
  - _PipelineContext for closure safety
  - NNX state <-> Linen collection flattening
"""
...
# ---------------------------------------------------------------------------
# V65b utilities: NNX state <-> flat Linen array collections
# ---------------------------------------------------------------------------
...
# For V65b, we import from pipeline.py and only override NNXCircularPipeline.
# ---------------------------------------------------------------------------
...
class NNXCircularPipeline(NNXPipelineBase):
  """V65b: NNX circular pipeline with params in CARRY (not broadcast).

  Key difference from V65: params are part of the lift.scan carry instead of
  variable_broadcast. They are returned OUTSIDE the Level 3 custom_vjp as an
  identity pass-through, making them KNOWN in partial eval.

  This mirrors V46's pattern where pipeline_weights passes through outside
  execute_pipeline_stage_pure's custom_vjp boundary.

  Memory target: match V46's 23.3 GB by producing 2nd recomp pair in PE.
  """
  ...
  def __call__(
    ...
    """V65b: Uses flax.core.lift.scan + lift.checkpoint with params in CARRY.

    Architecture:
      lift.scan(REPEATS) -> lift.checkpoint(stage_fn containing jax.lax.scan(MICROBATCHES))

    Key difference from V65: params are part of the scan carry (not broadcast).
    They are returned OUTSIDE the Level 3 custom_vjp as an identity pass-through,
    so their carry output is KNOWN in partial eval -- producing the 2nd recomp pair.
    """
    ...
    #
    # V65b CHANGE: params are NOT placed into scope. They go into the carry.
    # Only mutables/metrics/inputs go into scope collections.

    # Params -> flattened for carry (NOT scope)
    ...
    #   - bsw_ref: Python mutable list. SAFE.
    #   - num_microbatches, bubble_iterations: Python int. SAFE.
    #   - deterministic, model_mode: Python bool/str. SAFE.
    #
    # V65b CHANGE: NO 'params' read from scope. Params come from carry.

    def _stage_fn_for_scope(scope, carry):
      """One repeat: prefetch w_next, run microbatch scan, return updated carry.

      V65b: params are part of carry, NOT read from scope.
      Params are returned OUTSIDE the Level 3 custom_vjp (identity pass-through).
      """
      loop_state_carry, w_curr, local_params_collection = carry

      # ---- Reconstruct params from carry collection ----
      local_param_arrays = _linen_collection_to_arrays(local_params_collection, param_keys)
      local_layers_params = _unflatten_nnx_state(
          local_param_arrays, param_treedef, param_is_var, param_var_types, param_meta
      )

      # ---- Read NON-param arrays from scope ----
      ...
      # NOTE: _execute_stage does NOT include params in its output.
      # Params are returned OUTSIDE this custom_vjp in the carry (identity pass-through).
      ...
      def _execute_stage_fwd(loop_state_arg, w_curr_arg, params_arg,
        ...
        # NOTE: returns (loop_state, w_next, layer_mutables) -- NO params.
        # Params are repacked OUTSIDE this custom_vjp in the carry.
      ...
      # ---- V65b KEY CHANGE: params returned OUTSIDE the Level 3 custom_vjp ----
      # local_layers_params passes through as identity in the carry.
      # This mirrors V46's pattern:
      #   return (*execute_pipeline_stage_pure(loop_state, w_curr, pipeline_weights), pipeline_weights), None
      # The custom_vjp returns (loop_state, w_next, layer_mutables) only.
      # Params are repacked here, OUTSIDE the custom_vjp boundary.
      # Because params are unchanged (identity), their carry output is KNOWN in PE.
      ...
      # ---- Return carry with params as identity pass-through ----
      # Carry: (loop_state, w_next, params_collection_unchanged)
      # The params collection is returned unchanged -- identity pass-through OUTSIDE custom_vjp.
      return (new_loop_state, w_next, local_params_collection), inner_metrics
    ...
    # V65b CHANGE: 'params' removed from variable_broadcast.
    # Params are now part of the user carry, not a scope collection.
    scanned_stage = flax_lift.scan(
        checkpointed_stage,
        variable_broadcast=['broadcast_inputs', 'metrics_template'],
    ...
    # V65b CHANGE: NO 'params' in linen_variables. Params go into carry.
    ...
    # Initial params collection for carry
    initial_params_collection = _arrays_to_linen_collection(param_arrays, param_keys)
    ...
    # Run the repeat scan
    # V65b: carry is (loop_state, w_curr, params_collection)
    scan_result, mutated_vars = apply_fn(
        linen_variables,
        (loop_state, initial_w_curr, initial_params_collection),
    )
    # scan_result = (final_carry, stacked_ys) from lift.scan
    (loop_state, final_w_curr, _final_params_collection), repeat_metrics_raw = scan_result
