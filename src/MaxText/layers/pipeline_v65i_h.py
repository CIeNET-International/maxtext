...
"""V65i-H: Precomputed w_curr as scan xs — reduce in-scan all-gathers by half.

Derived from V65i (original NNXCircularPipeline). The key change is in the
outer scan over repeats:

V65i (original):
  - Inside execute_one_repeat, calls weight_prefetching TWICE:
    1. w_curr = weight_prefetching(layers_params, ..., iter_start - 1)  [or zeros for r=0]
    2. w_next = weight_prefetching(layers_params, ..., iter_start)
  - BSW = (w_curr, w_next), both fully gathered

V65i-H (this file):
  - BEFORE the outer scan, precomputes all w_curr for every repeat:
    for r in range(num_repeats):
      w_curr[r] = weight_prefetching(layers_params, ..., r * num_microbatches - 1)
    Stacks them into a pytree with leading dim num_repeats.
  - Passes stacked_w_curr as scan xs. Each repeat body unstacks its w_curr
    from xs[repeat_index], then only calls weight_prefetching ONCE (for w_next).
  - BSW = (w_curr_from_xs, w_next_from_scan_body)

Why this might help memory:
  - xs in jax.lax.scan backward are treated as num_consts (1 copy for the
    entire backward pass), NOT per-iteration storage like carry.
  - Only 1 all-gather inside the scan body (for w_next) vs 2 in V65i.
  - w_curr is NOT in carry, so no per-iteration carry storage for it.

Trade-off:
  - Precomputing all w_curr means num_repeats all-gathers happen upfront.
  - The stacked gathered weights occupy O(num_repeats * gathered_weight_size)
    memory outside the scan.
  - For typical configs (2-4 repeats), this is modest.
"""
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    if self.config.scan_pipeline_iterations:
      (loop_state, final_rng_mutables), stacked_metrics = jax.lax.scan(
      ...
      loop_state, final_rng_mutables = current_carry
      stacked_metrics = jax.tree.map(lambda *xs: jnp.stack(xs), *metrics_history) if metrics_history else layers_metrics

    final_layer_state = nnx.State.merge(layers_params, layers_metrics, final_rng_mutables)
...
class NNXCircularPipeline(NNXPipelineBase):
  """V65i-H: NNX circular pipeline with precomputed w_curr as scan xs.

  Modification of V65i: instead of computing w_curr inside the outer scan
  body (which requires an all-gather per repeat), w_curr for ALL repeats is
  precomputed BEFORE the scan and passed as xs (scan inputs).

  Memory hypothesis:
    - jax.lax.scan xs are treated as num_consts in backward (single copy),
      unlike carry which is stored per-iteration.
    - Only 1 all-gather inside scan (for w_next) vs 2 in V65i.
    - No w_curr in carry -> no per-iteration carry storage for it.

  Trade-offs:
    - num_repeats all-gathers happen upfront (before scan starts).
    - stacked_w_curr occupies O(num_repeats * gathered_weight_size) outside scan.
    - For typical configs (2-4 repeats), this is modest overhead.

  Key design decisions (inherited from V65i):
  ...
  def __call__(
    ...
    # ---- Nested scan structure (V65i-H: precomputed w_curr as scan xs) ----
    ...
    # ---- V65i-H: Precompute w_curr (gathered) for ALL repeats OUTSIDE the scan ----
    # In V65i, execute_one_repeat calls weight_prefetching twice:
    #   w_curr = prefetch(iter_start - 1)  and  w_next = prefetch(iter_start)
    # Here we precompute all w_curr upfront and pass them as scan xs.
    # xs in jax.lax.scan backward are num_consts (1 copy), not per-iteration carry.
    #
    # For repeat 0: w_curr is zeros (no previous repeat to prefetch from)
    # For repeat r>0: w_curr = weight_prefetching(layers_params, ..., r * num_microbatches - 1)

    all_w_curr_list = []
    # Repeat 0: w_curr is zeros (no previous repeat to prefetch from).
    # weight_prefetching returns gathered weights with shape [stages, ...param_dims_no_fsdp...].
    # Call weight_prefetching once at iteration 0 to get the correct output shape,
    # then zero it out for repeat 0's w_curr slot.
    w_curr_shape_ref = self.weight_prefetching(layers_params, physical_partition_spec_full, 0)
    all_w_curr_list.append(jax.tree.map(jnp.zeros_like, w_curr_shape_ref))

    for r in range(1, num_repeats):
      iter_start = r * num_microbatches
      w_curr_r = self.weight_prefetching(layers_params, physical_partition_spec_full, iter_start - 1)
      all_w_curr_list.append(w_curr_r)

    # Stack all w_curr into a pytree with leading dim num_repeats.
    # Each leaf: [num_repeats, stages, ...param_dims...]
    stacked_w_curr = jax.tree.map(lambda *xs: jnp.stack(xs), *all_w_curr_list)

    def execute_one_repeat(carry, xs):
      """Outer scan body: one repeat. w_curr comes from xs (precomputed), w_next computed inside."""
      loop_state, rng_mutables = carry
      w_curr_for_repeat, repeat_index = xs

      total_iteration_at_repeat_start = repeat_index * num_microbatches

      # w_curr comes from precomputed xs — NO all-gather needed here!
      gathered_weights_current = w_curr_for_repeat

      # w_next: single all-gather inside scan (same as V65i)
      gathered_weights_next = self.weight_prefetching(
          layers_params, physical_partition_spec_full, total_iteration_at_repeat_start
      )
      ...
      return (new_loop_state, updated_rng_mutables), repeat_step_metrics
    ...
    # ---- Execute: outer scan over repeats with precomputed w_curr as xs ----
    scan_xs = (stacked_w_curr, jnp.arange(num_repeats))

    (loop_state, final_rng_mutables), repeat_metrics = jax.lax.scan(
