...
V63: V62 + @jax.custom_vjp per microbatch INSIDE the scope-isolated lift.scan body.
The custom_vjp acts as opaque barrier to partial eval (zero internal residuals),
matching Linen's run_single_microbatch_custom pattern.

Based on V62 which uses flax.core.apply + lift.scan + lift.checkpoint to get _partial_pack
scope isolation for the NNX circular pipeline's inner scan, WITHOUT nn.Module.

The approach:
  1. Convert NNX layers state to Linen-style variable dicts (nested dicts of arrays)
  2. Use flax.core.apply to create a Scope
  3. Inside the apply function, use lift.scan + lift.checkpoint which go through
     _partial_pack to provide scope isolation
  4. The _partial_pack provides scope isolation -> enables custom_vjp inside
     checkpoint inside scan without tracer leaks
  5. Convert results back to NNX state
"""
...
# ---------------------------------------------------------------------------
# V62 utilities: NNX state <-> Linen variable dict conversion
# ---------------------------------------------------------------------------

def _nnx_state_to_linen_vars(state, collection_name='params'):
  """Convert an nnx.State (or partition thereof) to a Linen-style variable dict.

  NNX State is a nested dict with nnx.Variable wrappers at leaves.
  Linen variables are {'collection_name': {nested_dict_of_raw_arrays}}.

  The conversion:
    nnx.State({'layers': {'0': {'kernel': Param(array)}}})
    ->
    {'params': {'layers': {'0': {'kernel': array}}}}

  Args:
    state: An nnx.State instance (possibly a filtered partition).
    collection_name: The Linen collection name to use (default: 'params').

  Returns:
    A dict of the form {collection_name: nested_dict_of_arrays}.
  """
  ...
  raw_dict = jax.tree.map(
      _unwrap_variable, state,
      is_leaf=lambda x: isinstance(x, nnx.Variable),
  )
  # nnx.State is a frozen-dict-like structure; convert to plain dict recursively
  raw_dict = _to_plain_dict(raw_dict)
  return {collection_name: raw_dict}


def _to_plain_dict(x):
  """Recursively convert nnx.State / FrozenDict to plain dicts."""
  if isinstance(x, (nnx.State, dict)):
    return {k: _to_plain_dict(v) for k, v in x.items()}
  return x


def _linen_vars_to_nnx_state(linen_vars, reference_state, collection_name='params'):
  """Convert Linen-style variable dict back to an nnx.State matching a reference.

  The reference_state provides the tree structure and nnx.Variable types.
  We replace only the raw array values.

  Args:
    linen_vars: Dict of form {collection_name: nested_dict_of_arrays}.
    reference_state: The original nnx.State whose structure to match.
    collection_name: Which collection to extract.

  Returns:
    An nnx.State with the same structure as reference_state but updated values.
  """
  updated_arrays = linen_vars[collection_name]
  # Reconstruct by mapping over reference to preserve Variable types
  def _rewrap(ref_leaf, new_val):
    if isinstance(ref_leaf, nnx.Variable):
      return ref_leaf.replace(value=new_val)
    return new_val

  # We need to flatten both trees in the same order
  ref_leaves, ref_treedef = jax.tree.flatten(
      reference_state, is_leaf=lambda x: isinstance(x, nnx.Variable)
  )
  # Flatten the updated arrays using the same path structure
  # The updated_arrays has the same nested dict structure but without Variable wrappers
  updated_dict = _to_nnx_state_dict(updated_arrays, reference_state)
  new_leaves, _ = jax.tree.flatten(
      updated_dict, is_leaf=lambda x: isinstance(x, nnx.Variable)
  )

  rewrapped = [_rewrap(r, n) for r, n in zip(ref_leaves, new_leaves)]
  return ref_treedef.unflatten(rewrapped)


def _to_nnx_state_dict(arrays_dict, reference_state):
  """Map raw arrays back into the reference state structure (preserving Variable wrappers)."""
  def _map_fn(ref_leaf, path):
    # Navigate arrays_dict by path
    val = arrays_dict
    for key in path:
      key_str = getattr(key, 'key', str(key))
      val = val[key_str]
    if isinstance(ref_leaf, nnx.Variable):
      return ref_leaf.replace(value=val)
    return val
  ...
  new_leaves = []
  for path, ref_leaf in leaves_with_path:
    val = arrays_dict
    for key in path:
      key_str = getattr(key, 'key', str(key))
      val = val[key_str]
    if isinstance(ref_leaf, nnx.Variable):
      new_leaves.append(ref_leaf.replace(value=val))
    else:
      new_leaves.append(val)
  return treedef.unflatten(new_leaves)


def _nnx_state_to_flat_linen(state, collection_name='params'):
  """Simpler conversion: flatten nnx.State to {collection: {flat_key: array}}.

  This avoids nested-dict path issues by using a flat key scheme.
  Each leaf gets a string key derived from its tree path.

  Returns:
    (linen_vars, unflatten_fn) where unflatten_fn(flat_arrays) -> nnx.State
  """
  def _is_var(x):
  ...
  for path, leaf in leaves_with_path:
    # Create a string key from the path
    key_parts = []
    for p in path:
      key_parts.append(getattr(p, 'key', str(p)))
    flat_key = '/'.join(key_parts)
    path_keys.append(flat_key)
    flat_dict[flat_key] = leaf.value if isinstance(leaf, nnx.Variable) else leaf

  linen_vars = {collection_name: flat_dict}

  # Create unflatten function
  def unflatten_fn(updated_flat_dict):
    new_leaves = []
    for (path, ref_leaf), flat_key in zip(leaves_with_path, path_keys):
      val = updated_flat_dict[flat_key]
      if isinstance(ref_leaf, nnx.Variable):
        new_leaves.append(ref_leaf.replace(value=val))
      else:
        new_leaves.append(val)
  ...
  return linen_vars, path_keys, treedef, leaves_with_path, unflatten_fn
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    """V62: Uses flax.core.apply + lift.scan + lift.checkpoint for scope isolation.

    The key change from the original NNXCircularPipeline.__call__:
    - The inner scan (over microbatches) is replaced with lift.scan(lift.checkpoint(...))
    - This goes through _partial_pack, which provides scope isolation
    - Scope isolation enables custom_vjp inside checkpoint inside scan without tracer leaks
    - NNX state is converted to Linen-style variable dicts for the Scope-based path
    - Results are converted back to NNX state after the scan completes
    """
    inputs = inputs.reshape(
        (
            self.config.num_pipeline_microbatches,
    ...
    # Two spec variants needed (same as original):
    ...
    # ---- V62: Nested scan with flax.core Scope isolation ----
    #
    # Architecture (same outer structure, new inner implementation):
    #
    # outer scan (repeats):
    #   1. All-gather weights once -> BSW
    #   2. Store BSW in bsw_ref[0] (closure, NOT carry -- carry would OOM)
    #   3. Run inner scan over microbatches using lift.scan + lift.checkpoint
    #
    # inner scan (microbatches) -- NOW via lift.scan(lift.checkpoint(...)):
    #   1. Read BSW from bsw_ref[0] (set by outer_body)
    #   2. lift.checkpoint wraps the body -> _partial_pack -> scope isolation
    #   3. The scope isolation prevents tracer leaks from custom_vjp
    #   4. Run one pipeline iteration (forward through decoder)
    #
    # The key insight: lift.scan and lift.checkpoint both call `pack()` which
    # calls `_partial_pack()`. _partial_pack creates isolated Scope instances
    # that have their own variable dicts detached from the parent scope. This
    # prevents tracer leaks when custom_vjp (used by e.g. all_gather gradient
    # interception) is nested inside checkpoint inside scan.
    #
    # State flow:
    #   NNX mutables -> Linen {'carry_state': flat_arrays} (carried through scan)
    #   NNX params -> closed over via bsw_ref (NOT in Linen vars, since
    #                 _partial_pack would try to stack/broadcast them incorrectly)
    #   loop_state -> plain JAX carry (not in Linen vars)
    #   metrics -> Linen {'metrics': flat_arrays} (scan output axis)

    bsw_ref = [None]
    num_microbatches = self.config.num_pipeline_microbatches

    # ---- Convert NNX mutables state to Linen-style variable dicts ----
    # We need to flatten the NNX state into a form that Linen's Scope system
    # can manage. The key requirement: Linen variables must be nested dicts
    # of raw arrays (not nnx.Variable wrappers).
    #
    # We use two Linen collections:
    #   'carry_state' - mutable, carried through scan iterations (mutables)
    #   'metrics' - mutable, output from each scan iteration (stacked)

    def _flatten_nnx_state(state):
      """Flatten nnx.State to (list_of_arrays, treedef, is_var_flags).

      Returns raw arrays and metadata needed to reconstruct the state.
      """
      def _is_var(x):
        return isinstance(x, nnx.Variable)
      ...
      return arrays, treedef, is_var_flags, var_types, leaves_with_path

    def _unflatten_nnx_state(arrays, treedef, is_var_flags, var_types, ref_leaves_with_path):
      """Reconstruct nnx.State from flattened arrays."""
      new_leaves = []
      for arr, is_var, vtype, (path, ref_leaf) in zip(
          arrays, is_var_flags, var_types, ref_leaves_with_path
      ):
        if is_var and isinstance(ref_leaf, nnx.Variable):
          new_leaves.append(ref_leaf.replace(value=arr))
        else:
          new_leaves.append(arr)
      return treedef.unflatten(new_leaves)

    # Pre-flatten mutables and metrics for reuse
    mut_arrays, mut_treedef, mut_is_var, mut_var_types, mut_lwp = _flatten_nnx_state(layers_mutables)
    met_arrays, met_treedef, met_is_var, met_var_types, met_lwp = _flatten_nnx_state(layers_metrics)

    # Create indexed names for Linen variable dicts
    ...
    def _arrays_to_linen_collection(arrays, keys):
      """Pack arrays into a flat dict for use as a Linen collection."""
      return {k: a for k, a in zip(keys, arrays)}

    def _linen_collection_to_arrays(collection, keys):
      """Unpack arrays from a Linen collection dict."""
      return [collection[k] for k in keys]

    # ---- Define the inner scan body for lift.scan ----
    # This function has the Linen Scope signature: (scope, carry, *xs) -> (carry, ys)
    # The scope provides isolated variable access via _partial_pack

    # --- V63: custom_vjp per microbatch (opaque barrier to partial eval) ---
    # Matches Linen's run_single_microbatch_custom: wraps run_one_iteration
    # with jax.remat + jax.vjp inside @jax.custom_vjp.
    # Since we're inside lift.scan + lift.checkpoint (_partial_pack active),
    # the custom_vjp closures are at the correct trace level — no leaks.
    remat_policy = self.get_pipeline_remat_policy()

    @jax.custom_vjp
    def _run_iter_custom(loop_state, bsw, mutables):
      return _run_iter_fwd(loop_state, bsw, mutables)[0]

    def _run_iter_fwd(loop_state, bsw, mutables):
      def _run(ls, b):
        return self.run_one_iteration(
            ls, b, layers_graph, layers_metrics,
            mutables, positions, segment_ids,
            deterministic, model_mode, logical_partition_spec_stripped,
        )

      _run_remat = jax.remat(_run, policy=remat_policy)
      (new_ls, new_layer_state), vjp_fn = jax.vjp(_run_remat, loop_state, bsw)
      return (new_ls, new_layer_state), vjp_fn

    def _run_iter_bwd(vjp_fn, g_out):
      g_ls, g_layer_state = g_out
      d_ls, d_bsw = vjp_fn((g_ls, g_layer_state))
      return d_ls, d_bsw, None

    _run_iter_custom.defvjp(_run_iter_fwd, _run_iter_bwd)

    def _inner_scan_body_for_scope(scope, carry):
      """Inner scan body with custom_vjp barrier inside scope isolation.

      The custom_vjp makes this step OPAQUE to partial eval — JAX cannot
      decompose its internals, producing ZERO residuals from inside.
      This is the key memory optimization from Linen's architecture.
      """
      current_loop_state = carry

      # Read current mutables from scope's carry_state collection
      carry_state_dict = {}
      for k in mut_keys:
        carry_state_dict[k] = scope.get_variable('carry_state', k)

      current_mut_arrays = _linen_collection_to_arrays(carry_state_dict, mut_keys)
      current_layer_mutables = _unflatten_nnx_state(
          current_mut_arrays, mut_treedef, mut_is_var, mut_var_types, mut_lwp
      )

      # Apply stop_gradient and advance RNG state
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)

      # V63: custom_vjp wraps run_one_iteration (opaque to partial eval)
      new_loop_state, new_layer_state = _run_iter_custom(
          current_loop_state, bsw_ref[0], advanced_mutables
      )
      _, _, new_layer_metrics, new_layer_mutables = nnx.split(
          new_layer_state, _is_static_param, nnx.Intermediate, ...
      )

      # Write updated mutables back to scope's carry_state collection
      ...
      # Flatten metrics for scan output
      new_met_arrays, _, _, _, _ = _flatten_nnx_state(new_layer_metrics)

      return new_loop_state, _arrays_to_linen_collection(new_met_arrays, met_keys)

    # ---- Build the lifted scan with checkpoint ----
    # lift.checkpoint wraps the body -> goes through _partial_pack -> scope isolation
    # lift.scan wraps the checkpointed body -> goes through _partial_pack again

    if self.config.set_remat_policy_on_pipeline_iterations:
      checkpointed_body = flax_lift.checkpoint(
          _inner_scan_body_for_scope,
          variables=True,  # all variable collections
          rngs=True,        # all RNG sequences
          prevent_cse=not self.config.scan_pipeline_iterations,
          policy=self.get_pipeline_remat_policy(),
      )
    else:
      checkpointed_body = _inner_scan_body_for_scope

    # lift.scan wraps the (possibly checkpointed) body
    # variable_broadcast: params are NOT in scope vars (they're in bsw_ref closure)
    # variable_carry: 'carry_state' is carried (mutables)
    # variable_axes: 'metrics' are stacked along axis 0
    scanned_fn = flax_lift.scan(
        checkpointed_body,
        variable_broadcast=False,          # nothing is broadcast in scope vars
        variable_carry='carry_state',      # mutables are carried
        variable_axes={},                  # metrics returned via ys, not scope vars
        split_rngs={},                     # no Linen RNGs (NNX handles its own)
        length=num_microbatches,
    )

    # ---- Build the flax.core.apply wrapper ----
    def _run_inner_scan_via_scope(scope, init_loop_state):
      """Execute the inner scan within a Linen Scope context.

      This function is called via flax.core.apply, which creates a Scope
      with the provided variables. The lift.scan inside will call _partial_pack
      to create isolated sub-scopes for each iteration.
      """
      return scanned_fn(scope, init_loop_state)

    apply_fn = flax_scope.apply(
        _run_inner_scan_via_scope,
        mutable=['carry_state'],  # only carry_state is in scope vars
    )

    # ---- Define the outer body (repeats) ----
    # The outer loop remains as jax.lax.scan (no scope isolation needed here)

    def outer_body(carry, _):
      """One repeat: prefetch w_next (1 all-gather), inner scan via Scope."""
      ...
      if self.config.scan_pipeline_iterations:
        # ---- V62 path: lift.scan + lift.checkpoint via flax.core.apply ----
        # Convert current NNX mutables to Linen variable dicts
        cur_mut_arrays, _, _, _, _ = _flatten_nnx_state(current_layer_mutables)
        carry_state_init = _arrays_to_linen_collection(cur_mut_arrays, mut_keys)

        linen_variables = {
            'carry_state': carry_state_init,
        }

        # Execute inner scan via flax.core.apply
        # apply_fn returns (fn_result, mutated_vars) where fn_result = (carry, ys)
        # from lift.scan: carry = final_loop_state, ys = stacked scan outputs
        scan_result, mutated_vars = apply_fn(
            linen_variables,
            current_loop_state,
        )
        # scan_result is (final_carry, stacked_ys) from lift.scan
        new_loop_state, stacked_scan_ys = scan_result

        # Extract updated mutables from Linen carry_state
        updated_carry_state = mutated_vars['carry_state']
        updated_mut_arrays = _linen_collection_to_arrays(updated_carry_state, mut_keys)
        new_layer_mutables = _unflatten_nnx_state(
            updated_mut_arrays, mut_treedef, mut_is_var, mut_var_types, mut_lwp
        )

        # Extract stacked metrics from scan ys (returned by body)
        inner_met_arrays = _linen_collection_to_arrays(stacked_scan_ys, met_keys)
        inner_metrics = _unflatten_nnx_state(
            inner_met_arrays, met_treedef, met_is_var, met_var_types, met_lwp
        )
      else:
        # Unrolled path: use plain Python loop (no scope isolation needed)
        inner_carry_loop_state = current_loop_state
        inner_carry_mutables = current_layer_mutables
        inner_metrics_list = []
        for _ in range(num_microbatches):
          # Use the non-scope inner body for the unrolled case
          cur_mut_arrays_step, _, _, _, _ = _flatten_nnx_state(inner_carry_mutables)
          carry_state_step = _arrays_to_linen_collection(cur_mut_arrays_step, mut_keys)
          metrics_step = _arrays_to_linen_collection(met_arrays, met_keys)

          linen_vars_step = {
              'carry_state': carry_state_step,
              'metrics': metrics_step,
          }

          # For unrolled, we use a single-step apply with the checkpointed body
          single_step_apply = flax_scope.apply(
              lambda scope, ls: checkpointed_body(scope, ls),
              mutable=['carry_state', 'metrics'],
          )
          step_loop_state, step_mutated = single_step_apply(
              linen_vars_step, inner_carry_loop_state,
          )
          inner_carry_loop_state = step_loop_state

          # Update mutables from step
          step_carry = step_mutated['carry_state']
          step_mut_arrays = _linen_collection_to_arrays(step_carry, mut_keys)
          inner_carry_mutables = _unflatten_nnx_state(
              step_mut_arrays, mut_treedef, mut_is_var, mut_var_types, mut_lwp
          )

          # Collect metrics
          step_met_collection = step_mutated['metrics']
          step_met_arrays = _linen_collection_to_arrays(step_met_collection, met_keys)
          step_metrics = _unflatten_nnx_state(
              step_met_arrays, met_treedef, met_is_var, met_var_types, met_lwp
          )
          inner_metrics_list.append(step_metrics)

        new_loop_state = inner_carry_loop_state
        new_layer_mutables = inner_carry_mutables
        inner_metrics = (
    ...
    # ---- Inner body for bubble iterations (uses lift.checkpoint only) ----
    def _bubble_body_for_scope(scope, carry):
      """Bubble iteration body with scope isolation (same as inner scan body)."""
      current_loop_state = carry
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      checkpointed_bubble_body = flax_lift.checkpoint(
          _bubble_body_for_scope,
          variables=True,
          rngs=True,
          prevent_cse=not self.config.scan_pipeline_iterations,
          policy=self.get_pipeline_remat_policy(),
      ...
      checkpointed_bubble_body = _bubble_body_for_scope

    # Build bubble scan using lift.scan
    bubble_scanned_fn = flax_lift.scan(
        checkpointed_bubble_body,
        variable_broadcast=False,
        variable_carry='carry_state',
        variable_axes={},
        split_rngs={},
        length=bubble_iterations if bubble_iterations > 0 else 1,
    )

    bubble_apply_fn = flax_scope.apply(
        lambda scope, ls: bubble_scanned_fn(scope, ls),
        mutable=['carry_state'],
    )
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        # V62: bubble iterations also go through lift.scan + lift.checkpoint
        bubble_mut_arrays, _, _, _, _ = _flatten_nnx_state(final_layer_mutables)
        bubble_carry_init = _arrays_to_linen_collection(bubble_mut_arrays, mut_keys)

        bubble_linen_vars = {
            'carry_state': bubble_carry_init,
        }

        bubble_scan_result, bubble_mutated = bubble_apply_fn(
            bubble_linen_vars,
            loop_state,
        )
        # bubble_scan_result = (final_carry, stacked_ys)
        loop_state, bubble_scan_ys = bubble_scan_result

        # Extract final mutables
        bubble_carry_out = bubble_mutated['carry_state']
        bubble_mut_out = _linen_collection_to_arrays(bubble_carry_out, mut_keys)
        final_layer_mutables = _unflatten_nnx_state(
            bubble_mut_out, mut_treedef, mut_is_var, mut_var_types, mut_lwp
        )

        # Extract bubble metrics from scan ys
        bubble_met_out = _linen_collection_to_arrays(bubble_scan_ys, met_keys)
        bubble_metrics = _unflatten_nnx_state(
            bubble_met_out, met_treedef, met_is_var, met_var_types, met_lwp
        )
      else:
        bubble_carry = (loop_state, final_layer_mutables)
        bubble_metrics_list = []
        for _ in range(bubble_iterations):
          # Unrolled bubble with scope isolation
          bub_mut_arrays, _, _, _, _ = _flatten_nnx_state(bubble_carry[1])
          bub_carry_state = _arrays_to_linen_collection(bub_mut_arrays, mut_keys)
          bub_met_state = _arrays_to_linen_collection(met_arrays, met_keys)
          bub_linen_vars = {'carry_state': bub_carry_state, 'metrics': bub_met_state}

          bub_step_apply = flax_scope.apply(
              lambda scope, ls: checkpointed_bubble_body(scope, ls),
              mutable=['carry_state', 'metrics'],
          )
          bub_loop_state, bub_mutated = bub_step_apply(bub_linen_vars, bubble_carry[0])

          bub_carry_out = bub_mutated['carry_state']
          bub_mut_out = _linen_collection_to_arrays(bub_carry_out, mut_keys)
          bub_layer_mutables = _unflatten_nnx_state(
              bub_mut_out, mut_treedef, mut_is_var, mut_var_types, mut_lwp
          )
          bubble_carry = (bub_loop_state, bub_layer_mutables)

          bub_met_out = _linen_collection_to_arrays(bub_mutated['metrics'], met_keys)
          bub_metrics = _unflatten_nnx_state(
              bub_met_out, met_treedef, met_is_var, met_var_types, met_lwp
          )
