  ...
  def __call__(
    ...
    def _stage_fn_for_scope(scope, carry):
      ...
      def _execute_stage_fwd(loop_state_arg, w_curr_arg, params_arg,
        ...
        def _full_stage(ls, w_curr, params):
          iteration = ls["loop_iteration"]
          w_next = ctx.weight_prefetching(
              params, physical_partition_spec_full, iteration
          )
          bsw = (w_curr, w_next)
          (final_ls, final_muts, _bsw_out), metrics = _run_microbatches(
              ls, bsw, muts_arg, met_arg, pos_arg, seg_arg
          )
          return (final_ls, w_next, final_muts), metrics

        (result, metrics), stage_vjp_fn = jax.vjp(
            _full_stage, loop_state_arg, w_curr_arg, params_arg
        )

        return (result, metrics), stage_vjp_fn

      def _execute_stage_bwd(stage_vjp_fn, g_out):
        (g_result, g_metrics) = g_out
        g_ls_w_muts, = g_result,  # g_result = (g_ls, g_w_next, g_muts)

        d_ls, d_w_curr, d_params = stage_vjp_fn((g_ls_w_muts, g_metrics))
