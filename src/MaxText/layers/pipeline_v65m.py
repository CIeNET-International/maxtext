...
V65m: V65j base (lift.scan + lift.checkpoint, NO w_curr in carry) with Level 2
and Level 3 custom_vjp REMOVED. Only Level 1 (per-microbatch custom_vjp) kept.

Architecture
------------
Same outer structure as V65j:
  - ``flax.core.lift.scan`` + ``flax.core.lift.checkpoint`` over REPEATS
  - ``jax.lax.scan`` over microbatches INSIDE the remat body

Key difference from V65j
------------------------
V65j has three levels of custom_vjp:
  - Level 1: per-microbatch (_run_iter_local)
  - Level 2: microbatch scan wrapper (_run_microbatches with d+g accumulation)
  - Level 3: BSW computation + scan wrapper (_execute_stage with linear_transpose)

V65m REMOVES Level 2 and Level 3 entirely. The stage function directly:
  1. Derives repeat_idx from loop_state["loop_iteration"] // num_microbatches
  2. Computes w_curr and w_next from scope-provided params (jnp.where for iter 0)
  3. Runs raw jax.lax.scan for inner microbatch loop with Level 1 only
  4. Returns (new_loop_state,) as carry -- NO w_curr in carry

Rationale: Level 2 and Level 3 add complexity for gradient routing. By removing
them, we let JAX's autodiff handle BSW gradient flow naturally. This tests
whether the explicit d+g accumulation and linear_transpose reduce-scatter in
Level 2/3 are necessary for correctness or memory, or if Level 1 alone suffices.

Closure safety audit
--------------------
Same as V65j. All closures capture only Python objects and scope-read JAX arrays.
...
class NNXCircularPipeline(NNXPipelineBase):
  """V65m: NNX circular pipeline -- Level 1 custom_vjp only.
  ...
  Key changes from V65j:
    - Level 2 (_run_microbatches custom_vjp with d+g accumulation) REMOVED
    - Level 3 (_execute_stage custom_vjp with linear_transpose) REMOVED
    - Direct computation: derive repeat_idx, compute BSW, raw jax.lax.scan
    - Carry is just (loop_state,) -- NO w_curr
  """
  ...
  def __call__(
    ...
    """V65m: Uses flax.core.lift.scan + lift.checkpoint with Level 1 custom_vjp only.

    Architecture:
      lift.scan(REPEATS) -> lift.checkpoint(stage_fn containing jax.lax.scan(MICROBATCHES))

    Key changes from V65j:
      - Level 2 (_run_microbatches custom_vjp) REMOVED
      - Level 3 (_execute_stage custom_vjp with linear_transpose) REMOVED
      - Direct BSW computation + raw jax.lax.scan with Level 1 only
      - Carry is (loop_state,) only -- NO w_curr
    """
    ...
    #
    # REMOVED from V65j's closure:
    #   - bsw_ref: not needed inside stage (only used for bubble iterations)
    #   - bubble_iterations: not needed inside stage
    #
    # VERDICT: ZERO JAX pytree objects in closure. All JAX data from scope reads.

    def _stage_fn_for_scope(scope, carry):
      """One repeat: compute BSW fresh, run microbatch scan with Level 1 only, return updated carry.

      V65m KEY CHANGES from V65j:
        - Level 2 (_run_microbatches custom_vjp) REMOVED
        - Level 3 (_execute_stage custom_vjp with linear_transpose) REMOVED
        - Direct BSW computation + raw jax.lax.scan with Level 1 only
        - Carry is just (loop_state,) -- no w_curr
      ...
      # ---- V65m: Compute BSW fresh from sharded params ----
      ...
      # ---- Level 1 ONLY: Per-microbatch custom_vjp (opaque barrier to partial eval) ----
      ...
      # ---- V65m: DIRECT inner microbatch scan (NO Level 2 or Level 3 custom_vjp) ----
      # Raw jax.lax.scan over microbatches with Level 1 custom_vjp only.
      # JAX's autodiff handles BSW gradient flow naturally through the scan.

      def inner_body(inner_carry, _):
        """Single microbatch step inside the inner scan.

        CLOSURE AUDIT:
          - _run_iter_local: locally defined function with custom_vjp -- SAFE.
          - bsw: JAX arrays computed from scope reads above -- SAFE (in same trace scope).
          - local_layers_metrics: NNX state from scope read -- SAFE.
          - local_positions, local_segment_ids: JAX arrays from scope read -- SAFE.
          - met_keys: list of strings -- SAFE.
        """
        ls, muts = inner_carry
        ...
        new_ls, new_layer_state = _run_iter_local(
            ls, bsw, advanced_muts,
            local_layers_metrics, local_positions, local_segment_ids
        )
      ...
      if scan_pipeline_iters:
        (new_loop_state, new_layer_mutables), inner_metrics = jax.lax.scan(
            inner_body, (loop_state_carry, current_layer_mutables),
            None, length=num_microbatches,
        )
      else:
        carry = (loop_state_carry, current_layer_mutables)
        metrics_list = []
        for _ in range(num_microbatches):
          carry, step_met = inner_body(carry, None)
          metrics_list.append(step_met)
        new_loop_state, new_layer_mutables = carry
        met_arrays_local, _, _, _, _ = _flatten_nnx_state(local_layers_metrics)
      ...
      # ---- V65m: carry is just (loop_state,) -- no w_curr or w_next ----
    ...
    # V65m: No initial_w_curr needed in carry
    ...
    # V65m: carry is just (loop_state,) -- no initial_w_curr
    ...
    if bubble_iterations > 0:
      # V65m: For bubble iterations, BSW = (final_w, final_w) where final_w
