...
"""Pipeline V2: Single custom_vjp + jax.vmap + axes_scan.

Variant of pipeline.py with the following changes to NNXCircularPipeline.__call__:
  1. Import flax.core.axes_scan for broadcast-aware scanning
  2. Wrap inner_body in a single-level custom_vjp for d+g gradient accumulation
  3. Use axes_scan for the outer scan with layers_params as broadcast
"""
...
from flax.core.axes_scan import scan as axes_scan, broadcast as scan_broadcast
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V2: Raw inner body without custom_vjp wrapping
    def inner_body_raw(carry, _):
      current_loop_state, current_layer_mutables = carry
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
    ...
    # V2: Wrap inner_body in a single-level custom_vjp for d+g gradient accumulation.
    # This gives explicit control over backward pass gradient flow.
    @jax.custom_vjp
    def inner_with_vjp(carry, xs):
      return inner_body_raw(carry, xs)

    def inner_with_vjp_fwd(carry, xs):
      out = inner_body_raw(carry, xs)
      # Save inputs as residuals for backward pass
      return out, (carry, xs)

    def inner_with_vjp_bwd(res, g_out):
      carry, xs = res
      # Use jax.vjp to compute gradients through the raw inner body
      _, vjp_fn = jax.vjp(inner_body_raw, carry, xs)
      return vjp_fn(g_out)

    inner_with_vjp.defvjp(inner_with_vjp_fwd, inner_with_vjp_bwd)

    # Use the custom_vjp-wrapped version as inner_body
    inner_body = inner_with_vjp
    ...
    # V2: Outer body takes broadcast_params as explicit first arg for axes_scan
    def outer_body(broadcast_params, carry, _):
      """One repeat: prefetch w_next (1 all-gather), dual BSW.
      broadcast_params is passed through unchanged by axes_scan (not stacked)."""
      ...
      return broadcast_params, (new_loop_state, new_layer_mutables, w_next), inner_metrics

    # ---- Execute: outer scan (repeats) using axes_scan + bubble scan ----
    ...
    if self.config.scan_pipeline_iterations:
      ...
      # V2: Use axes_scan with layers_params as broadcast (not stacked across repeats)
      outer_scanned = axes_scan(
          outer_body,
          in_axes=(scan_broadcast, 0, 0),
          out_axes=(scan_broadcast, 0, 0),
          length=num_repeats,
          unroll=unroll_repeats,
      )
      _, (loop_state, final_layer_mutables, final_w_curr), repeat_metrics = outer_scanned(
          layers_params, (loop_state, layers_mutables, initial_w_curr), None,
      )
      ...
      for _ in range(num_repeats):
        _, outer_carry, rep_metrics = outer_body(layers_params, outer_carry, None)
