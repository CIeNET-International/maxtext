...
V65j: NNX circular pipeline -- V65 architecture WITHOUT w_curr in outer scan carry.

Architecture
------------
Same as V65: ``flax.core.lift.scan`` + ``flax.core.lift.checkpoint`` over REPEATS,
with ``jax.lax.scan`` over microbatches INSIDE the remat body.

Key difference from V65
-----------------------
V65 carries ``(loop_state, w_curr)`` in the outer lift.scan carry. ``w_curr`` is
ALL-GATHERED (unsharded), making the carry 4x larger than needed.

V65j removes ``w_curr`` from carry entirely. Instead, BSW is computed fresh each
iteration inside ``_stage_fn_for_scope`` using the repeat index derived from
``loop_state["loop_iteration"]``:

    repeat_idx = loop_iteration // num_microbatches

At the START of each outer iteration:
  - Repeat 0: loop_iteration = 0                    -> repeat_idx = 0
  - Repeat 1: loop_iteration = num_microbatches      -> repeat_idx = 1
  - Repeat k: loop_iteration = k * num_microbatches  -> repeat_idx = k

BSW construction (using jnp.where to avoid jax.lax.cond tracer issues):
  - w_prev = weight_prefetching(params, spec, max(repeat_idx - 1, 0) * num_microbatches)
  - w_curr = tree_map(where(repeat_idx == 0, zeros_like, w_prev))
  - w_next = weight_prefetching(params, spec, repeat_idx * num_microbatches)
  - bsw = (w_curr, w_next)

This matches Jacky's approach (21.3 GB) which computes BSW fresh each iteration.

Closure safety audit
--------------------
Same as V65. All closures capture only Python objects and scope-read JAX arrays.
No nnx.Module with JAX array attributes crosses transform boundaries.
"""
...
# Reuse V65 utilities (no duplication)
...
class NNXCircularPipeline(NNXPipelineBase):
  """V65j: NNX circular pipeline -- V65 architecture WITHOUT w_curr in carry.
  ...
  Key change from V65: the outer scan carry is just ``(loop_state,)`` -- no
  ``w_curr``. BSW is computed fresh each iteration from sharded params via
  scope broadcast, using ``loop_iteration // num_microbatches`` as repeat index.

  This reduces carry size by ~4x (no all-gathered weights in carry).
  """
  ...
  def __call__(
    ...
    """V65j: Uses flax.core.lift.scan + lift.checkpoint with all state in scope.

    Architecture:
      lift.scan(REPEATS) -> lift.checkpoint(stage_fn containing jax.lax.scan(MICROBATCHES))

    Key change from V65: outer scan carry is (loop_state,) only -- no w_curr.
    BSW computed fresh each iteration from sharded params in scope broadcast.
    """
    ...
    # ---- BSW mutable ref (closure, NOT carry -- used for bubble iterations) ----
    bsw_ref = [None]
    ...
    #   - param_meta/mut_meta/met_meta: lists of dicts (Python metadata only). SAFE.
    ...
    #
    # REMOVED from V65's closure:
    #   - initial_w_curr: no longer needed (BSW computed fresh)
    #
    # VERDICT: ZERO JAX pytree objects in closure. All JAX data from scope reads.

    def _stage_fn_for_scope(scope, carry):
      """One repeat: compute BSW fresh, run microbatch scan, return updated carry.

      V65j KEY CHANGE: carry is just (loop_state,) -- no w_curr.
      BSW computed fresh each iteration using repeat_idx from loop_iteration.

      All JAX arrays read from scope. No NNX module accessed.
      """
      (loop_state_carry,) = carry
      ...
      # ---- V65j: Compute BSW fresh from sharded params ----
      # Derive repeat index from loop_iteration (which starts at k * num_microbatches
      # for repeat k, because the inner scan increments it).
      #
      # At the START of each outer iteration:
      #   Repeat 0: loop_iteration = 0
      #   Repeat 1: loop_iteration = num_microbatches
      #   Repeat k: loop_iteration = k * num_microbatches
      #
      # So: repeat_idx = loop_iteration // num_microbatches
      #
      # For BSW:
      #   w_curr = weights for repeat_idx (all-gathered from sharded params)
      #            BUT for repeat 0, w_curr should be zeros (no previous prefetch)
      #   w_next = weights for repeat_idx + 1 (prefetched)
      #
      # We use jnp.where instead of jax.lax.cond to avoid tracer issues inside
      # lift.scan + lift.checkpoint. jnp.where evaluates both branches but is
      # safe under all JAX transforms.

      iteration = loop_state_carry["loop_iteration"]
      repeat_idx = iteration // num_microbatches

      # Compute w_prev: weights that WOULD have been prefetched in the previous repeat.
      # For repeat 0, we clamp to 0 and then mask with zeros via jnp.where.
      # The loop_iteration for "previous repeat's prefetch" is:
      #   max(repeat_idx - 1, 0) * num_microbatches
      prev_iteration = jnp.maximum(repeat_idx - 1, 0) * num_microbatches
      w_prev = ctx.weight_prefetching(
          local_layers_params, physical_partition_spec_full, prev_iteration
      )

      # For repeat 0: w_curr = zeros (no previous prefetch happened)
      # For repeat k>0: w_curr = w_prev (the previous repeat's prefetched weights)
      w_curr = jax.tree.map(
          lambda w: jnp.where(repeat_idx == 0, jnp.zeros_like(w), w),
          w_prev,
      )

      # w_next: prefetch for the current repeat
      ...
      # ---- Level 3 custom_vjp: wraps BSW computation + microbatch scan ----
      # V65j: Both w_curr and w_next computed from params in fwd.
      # Backward: reduce-scatter via linear_transpose for BOTH.

      @jax.custom_vjp
      def _execute_stage_v65j(loop_state_arg, w_curr_local, w_next_local, params_arg,
                               muts_arg, met_arg, pos_arg, seg_arg):
        return _execute_stage_v65j_fwd(loop_state_arg, w_curr_local, w_next_local,
                                        params_arg, muts_arg, met_arg, pos_arg, seg_arg)[0]

      def _execute_stage_v65j_fwd(loop_state_arg, w_curr_local, w_next_local, params_arg,
                                    muts_arg, met_arg, pos_arg, seg_arg):
        bsw = (w_curr_local, w_next_local)

        # Build linear transposes for weight_prefetching backward
        iteration_fwd = loop_state_arg["loop_iteration"]

        # Transpose for w_next (current iteration's prefetch)
        p_weight_prefetching = functools.partial(
            ctx.weight_prefetching,
            physical_partition_spec=physical_partition_spec_full,
            loop_iteration=iteration_fwd,
        )
        weight_prefetching_t = jax.linear_transpose(
            p_weight_prefetching, params_arg
        )

        # Transpose for w_curr (previous iteration's prefetch)
        repeat_idx_fwd = iteration_fwd // num_microbatches
        prev_iteration_fwd = jnp.maximum(repeat_idx_fwd - 1, 0) * num_microbatches
        p_weight_prefetching_prev = functools.partial(
            ctx.weight_prefetching,
            physical_partition_spec=physical_partition_spec_full,
            loop_iteration=prev_iteration_fwd,
        )
        weight_prefetching_prev_t = jax.linear_transpose(
            p_weight_prefetching_prev, params_arg
        )

        # Forward through Level 2 (microbatch scan)
        ...
        # Store bsw zero template for backward
        bsw_zero = jax.tree.map(jnp.zeros_like, bsw)

        return ((final_ls, final_muts), metrics), (
            scan_microbatches_vjp, weight_prefetching_t, weight_prefetching_prev_t,
            repeat_idx_fwd, bsw_zero,
        )

      def _execute_stage_v65j_bwd(residuals, g_out):
        (scan_microbatches_vjp, weight_prefetching_t, weight_prefetching_prev_t,
         repeat_idx_fwd, bsw_zero) = residuals
        (g_ls_muts, g_metrics) = g_out
        g_ls, g_muts = g_ls_muts

        # Build cotangent for Level 2's output: ((ls, muts, bsw), metrics)
        # No incoming gradient on bsw from outer carry (it's not in carry).
        g_bsw = bsw_zero  # (zeros_like(w_curr), zeros_like(w_next))
        ...
        # d_bsw = (d_w_curr, d_w_next) -- gradients w.r.t. BSW halves
        d_w_curr, d_w_next = d_bsw

        # Reduce-scatter for w_next gradients via linear transpose
        (g_params_from_next,) = weight_prefetching_t(d_w_next)

        # Reduce-scatter for w_curr gradients via linear transpose
        # For repeat 0, w_curr was zeros so d_w_curr should be zero too.
        # But we still apply the transpose -- the zero gradient will produce
        # zero param updates, which is correct.
        # For repeat k>0, d_w_curr flows through weight_prefetching_prev_t.
        (g_params_from_curr,) = weight_prefetching_prev_t(d_w_curr)

        # Mask out w_curr gradient contribution for repeat 0
        # (w_curr was zeros_like, so gradient should not flow to params)
        g_params_from_curr = jax.tree.map(
            lambda g: jnp.where(repeat_idx_fwd == 0, jnp.zeros_like(g), g),
            g_params_from_curr,
        )

        # Total params gradient = sum of both paths
        g_params = jax.tree.map(
            lambda a, b: a + b,
            g_params_from_next, g_params_from_curr,
        )

        # Gradients for w_curr_local and w_next_local: these are intermediate
        # computations from params. Their custom_vjp gradients flow back via
        # the linear transposes above. Return zeros for them since their
        # contribution is captured in g_params.
        g_w_curr_local = jax.tree.map(jnp.zeros_like, bsw_zero[0])
        g_w_next_local = jax.tree.map(jnp.zeros_like, bsw_zero[1])

        return d_ls, g_w_curr_local, g_w_next_local, g_params, None, None, None, None

      _execute_stage_v65j.defvjp(_execute_stage_v65j_fwd, _execute_stage_v65j_bwd)

      # ---- Execute the stage ----
      (new_loop_state, new_layer_mutables), inner_metrics = _execute_stage_v65j(
          loop_state_carry, w_curr, w_next, local_layers_params,
      ...
      # ---- V65j: carry is just (loop_state,) -- no w_next ----
      return (new_loop_state,), inner_metrics
    ...
    # V65j: No initial_w_curr needed in carry
    ...
    # V65j: carry is just (loop_state,) -- no initial_w_curr
    scan_result, mutated_vars = apply_fn(
        linen_variables,
        (loop_state,),
    )
    # scan_result = (final_carry, stacked_ys) from lift.scan
    (loop_state,), repeat_metrics_raw = scan_result
    ...
    if bubble_iterations > 0:
      # V65j: For bubble iterations, BSW = (final_w, final_w) where final_w
      # is the last repeat's weights. Compute fresh from params.
      final_iteration = num_repeats * num_microbatches
      final_w = self.weight_prefetching(
          layers_params, physical_partition_spec_full,
          (num_repeats - 1) * num_microbatches,
      )
      bsw_ref[0] = (final_w, final_w)

      # Bubble custom_vjp -- defined outside scope since bubble runs without lift.scan.
      # CLOSURE AUDIT:
      #   - ctx: _PipelineContext -- SAFE.
      #   - layers_graph: GraphDef -- SAFE.
      #   - layers_metrics: NNX state from OUTER scope. OK for bubble iterations
      #     (runs AFTER lift.scan, at outer trace level).
      ...
      #   - bsw_ref: Python list, contents set to (final_w, final_w) -- SAFE.
