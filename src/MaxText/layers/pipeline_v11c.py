...
"""Pipeline V11c: Params in carry, stop_gradient only on carry output.

Builds on V11 with one key change to the stop_gradient placement:
  6c. (Finding 6c) `layers_params` remains in the explicit scan carry (same as V11),
      but stop_gradient is applied ONLY on the carry OUTPUT, NOT at the top of the
      body. This means:

      - Inside the body, `carried_layers_params` is fully differentiable and used
        normally for weight_prefetching, BSW creation, and layer computation.
      - Gradients flow through the forward computation path (weight_prefetching ->
        BSW -> layer compute) unimpeded.
      - The stop_gradient on the carry OUTPUT tells XLA "params don't change across
        iterations" (loop-invariant hint) while still allowing gradient flow through
        the body's computation graph.
      - This potentially enables XLA buffer reuse since the carry output is marked
        non-differentiable, but the forward path retains full gradient flow.
     - For NNXPipeline (non-circular), layers_params is added to the flat scan carry.
     - For NNXCircularPipeline, layers_params is added to the outer scan carry
       alongside (loop_state, layers_mutables, w_curr).

All V10 fixes are preserved:
  1. (Finding 1) vmap returns only non-param state.
  2. (Finding 2) stop_gradient on current_layer_mutables.
  3. (Finding 3) stop_gradient on w_curr in outer scan carry.
  4. (Finding 4) No nnx.split inside checkpoint boundary.
  5. (Finding 5) Remove metric scatter-update, return per-stage metrics directly.
"""
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V11 Finding 6: Move layers_params into the scan carry so XLA sees it as an
    # explicit arg rather than an opaque closure capture. stop_gradient fence at
    # the top of scan_body prevents gradient accumulation through the carry path;
    # gradients for params flow through the forward computation instead.
    def scan_body(carry, _):
      current_loop_state, current_layer_mutables, carried_layers_params = carry
      # V11c: NO stop_gradient here -- carried_layers_params is fully differentiable
      # inside the body for forward computation. Gradients flow through the
      # computation path (merge -> run_one_iteration -> layer compute).
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      # V11c: Use carried_layers_params (explicit arg) -- differentiable inside body
      current_layer_state = nnx.State.merge(carried_layers_params, layers_metrics, advanced_mutables)
      ...
      # V11c: stop_gradient ONLY on carry output -- tells XLA params are loop-invariant
      # while allowing gradient flow through the body computation above.
      return (new_loop_state, new_layer_mutables, jax.lax.stop_gradient(carried_layers_params)), new_layer_metrics
    ...
    if self.config.scan_pipeline_iterations:
      # V11: layers_params is now part of the initial carry tuple
      (loop_state, final_layer_mutables, _final_layers_params), stacked_metrics = jax.lax.scan(
          scan_body, (loop_state, layers_mutables, layers_params), None, length=total_iterations
      )
    else:
      # V11: layers_params is now part of the carry tuple
      current_carry = (loop_state, layers_mutables, layers_params)
      metrics_history = []
      for _ in range(total_iterations):
        current_carry, step_metrics = scan_body(current_carry, None)
        metrics_history.append(step_metrics)
      loop_state, final_layer_mutables, _final_layers_params = current_carry
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  V11c addition: layers_params in explicit outer scan carry; stop_gradient applied
  ONLY on carry output (not at body top), enabling gradient flow through forward
  computation while hinting XLA that params are loop-invariant for buffer reuse.
  """
  ...
  def __call__(
    ...
    #
    # V11c: layers_params is in outer scan carry (same as V11). stop_gradient
    #   is applied ONLY on carry output, NOT at body top. This allows gradient
    #   flow through forward computation (weight_prefetching -> BSW -> layer)
    #   while hinting XLA that carry output params are loop-invariant.
    bsw_ref = [None]
    ...
    def inner_body(carry, _):
      ...
      # V10 Finding 4: nnx.split is outside the checkpoint boundary here (inner_body
    ...
    def outer_body(carry, _):
      ...
      V11c: layers_params is an explicit carry element. stop_gradient is applied
      ONLY on the carry output, NOT at the top of the body. This allows full
      gradient flow through weight_prefetching -> BSW -> layer compute, while
      telling XLA the carry output is loop-invariant for buffer reuse.
      """
      current_loop_state, current_layer_mutables, w_curr, carried_layers_params = carry
      iteration = current_loop_state["loop_iteration"]

      # V11c: NO stop_gradient on carried_layers_params here -- it is fully
      # differentiable inside the body. Gradients flow through
      # weight_prefetching -> BSW -> run_one_iteration -> layer compute.

      # V10 Finding 3: stop_gradient on w_curr. Gradients should flow through the
      # custom BSW/all-gather path (w_next computed from layers_params), not through
      ...
      # V11c: Use carried_layers_params (explicit arg, differentiable) for prefetching
      w_next = self.weight_prefetching(
          carried_layers_params, physical_partition_spec_full, iteration
      )
      ...
      # V11c: stop_gradient ONLY on carry output -- tells XLA params are loop-invariant
      # while allowing gradient flow through the body computation above.
      return (new_loop_state, new_layer_mutables, w_next, jax.lax.stop_gradient(carried_layers_params)), inner_metrics
    ...
    if self.config.scan_pipeline_iterations:
      ...
      # V11: layers_params is now part of the initial carry tuple
      (loop_state, final_layer_mutables, final_w_curr, _final_layers_params), repeat_metrics = jax.lax.scan(
          outer_body, (loop_state, layers_mutables, initial_w_curr, layers_params), None,
      ...
      loop_state, final_layer_mutables, final_w_curr, _final_layers_params = outer_carry
      repeat_metrics = (
