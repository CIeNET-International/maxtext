...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V31: Explicit-params checkpoint boundary ----
    #
    # Linen's nn.remat wraps `rematted(variable_groups, rng_groups, *args)`
    # where params are an EXPLICIT first argument.  This lets jax.remat's
    # partial evaluator see params as regular inputs, not closure captures.
    #
    # V31 replicates this: the checkpointed function receives
    # (bsw, graph, metrics_template) as its FIRST positional argument,
    # so they are never captured by the closure of the remat'd function.
    # `deterministic`, `model_mode`, `positions`, `segment_ids` remain
    # closures because they are strings / static / xs-broadcast tensors.

    def _inner_body_impl(params_bundle, carry, _):
      """Pure checkpoint-interior: params are EXPLICIT arg[0], not closure.

      Args:
        params_bundle: (bsw, graph, metrics_template) -- moved out of closure.
        carry: (loop_state, layer_mutables) -- normal scan carry.
        _: unused xs slot for jax.lax.scan.

      Returns:
        (new_carry, metrics) matching inner scan signature.
      """
      bsw, graph, metrics_template = params_bundle
      current_loop_state, current_layer_mutables = carry
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, bsw, graph, metrics_template,
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      # Checkpoint wraps _inner_body_impl WITH params as first arg.
      # This matches Linen's `rematted(variable_groups, ...)` pattern.
      _inner_body_ckpt = jax.checkpoint(
          _inner_body_impl, policy=self.get_pipeline_remat_policy(),
          prevent_cse=not self.config.scan_pipeline_iterations,
      )
    else:
      _inner_body_ckpt = _inner_body_impl

    def inner_body(carry, _):
      """Scan-compatible wrapper: passes params explicitly to checkpoint."""
      params_bundle = (bsw_ref[0], layers_graph, layers_metrics)
      return _inner_body_ckpt(params_bundle, carry, _)
