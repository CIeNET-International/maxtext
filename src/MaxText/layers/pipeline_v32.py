...
Based on Flax Issue #5116: nnx.clone breaks trace-level links on non-diff
state (metrics, mutables) before merging, preventing checkpoint from saving
unnecessary state copies as residuals.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      ...
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, bsw_ref[0], layers_graph, cloned_metrics,
          cloned_mutables, positions, segment_ids, deterministic,
