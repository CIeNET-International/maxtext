...
"""Pipeline V27: Use jax.remat with prevent_cse=True on both inner and outer body.

Variant targeting scan body equation count. Replaces jax.checkpoint on
inner_body with jax.remat using prevent_cse=True and explicit static_argnums.
Also wraps the OUTER body in jax.checkpoint too (Linen has remat at both levels).
prevent_cse=True prevents CSE from merging recomputed values with saved ones,
which can change buffer lifetimes and reduce peak memory.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V27: Use jax.remat with prevent_cse=True on inner_body.
    # prevent_cse=True prevents common-subexpression elimination from merging
    # recomputed values with saved ones during backward, which can change
    # buffer lifetimes and reduce peak memory. Also use explicit static_argnums.
    if self.config.set_remat_policy_on_pipeline_iterations:
      inner_body = jax.remat(
          inner_body,
    ...
    # V27: Also wrap outer_body in jax.checkpoint with prevent_cse=True.
    # Linen has remat at both scan levels. This ensures the outer repeat scan
    # also benefits from checkpoint boundaries for buffer lifetime management.
    if self.config.set_remat_policy_on_pipeline_iterations:
