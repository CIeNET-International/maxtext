...
"""V65i-E: V65i-A with Python for-loop outer (no scan carry for w_curr).

w_next from each repeat becomes w_curr for the next via Python variable.
Expected: ~21 GB memory + ~114 tok/s throughput.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- Execute: Python for-loop over repeats (no scan carry) ----
    ...
    for repeat_idx in range(num_repeats):
      (loop_state, layers_mutables, w_curr), rep_metrics = checkpointed_repeat_body(
          (loop_state, layers_mutables, w_curr), jnp.int32(repeat_idx)
      )
      repeat_metrics_list.append(rep_metrics)
    final_rng_mutables = layers_mutables
    final_w_curr = w_curr
    repeat_metrics = jax.tree.map(lambda *xs: jnp.stack(xs), *repeat_metrics_list)
