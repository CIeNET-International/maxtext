...
"""Pipeline V5: FrozenDict + carry promotion + stop_gradient.

Variant of pipeline.py with the following changes to NNXCircularPipeline.__call__:
  1. FrozenDict wrapping on broadcast vars (same as V1)
  2. layers_params added to outer scan carry (passed through for promotion)
  3. stop_gradient on mutables inside inner_body
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V5: Freeze broadcast variables to prevent JAX from treating them as
    # mutable state during scan. FrozenDict is immutable and signals to JAX
    # that these pytrees should not be differentiated through or carried.
    layers_params = freeze(layers_params)
    layers_metrics = freeze(layers_metrics)
    ...
    #
    # V5: outer scan carry now includes layers_params (carry promotion).
    # This makes the weight dependency explicit to JAX's AD system instead
    # of relying on closure capture, which can cause trace-level issues.
    #
    # outer scan (repeats):
    #   1. All-gather weights once -> BSW (using carried_params, not closure)
    ...
    def inner_body(carry, _):
      current_loop_state, current_layer_mutables = carry
      # V5: Stop gradients through mutables (RNG state) — they are not
      # trainable and carrying gradients through them inflates memory.
    ...
    def outer_body(carry, _):
      ...
      V5: carry includes carried_params for weight_prefetching (promotion)."""
      current_loop_state, current_layer_mutables, w_curr, carried_params = carry
      iteration = current_loop_state["loop_iteration"]

      # V5: Use carried_params from carry instead of layers_params closure
      w_next = self.weight_prefetching(
          carried_params, physical_partition_spec_full, iteration
      )
      ...
      # V5: carried_params passes through unchanged in carry
      return (new_loop_state, new_layer_mutables, w_next, carried_params), inner_metrics
    ...
    if self.config.scan_pipeline_iterations:
      ...
      # V5: Initial carry includes layers_params as 4th element (carry promotion)
      ...
      # V5: outer_carry includes layers_params as 4th element
