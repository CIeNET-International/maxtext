...
"""Pipeline V40: Use pipeline_utils functions (custom_vjp gradient accumulation + pipeline stage).

Uses Linen's pipeline_utils.create_gradient_accumulation_scan() and
create_pipeline_stage() inside NNXCircularPipeline.__call__(). These functions
define custom VJP rules that separate lightweight state from heavy parameters,
enabling memory-efficient gradient accumulation.

The outer scan structure uses jax.lax.scan with jax.checkpoint (mirroring the
nn.scan(nn.remat(...)) pattern in Linen's create_flax_pipeline_scan).

Adapter bridges NNX run_one_iteration to Linen's expected signature so that
pipeline_utils functions can call model.run_one_iteration(...) transparently.

KNOWN ISSUES:
- create_flax_pipeline_scan() (nn.scan + nn.remat + _partial_pack) cannot be used
  directly because nn.scan requires a Linen module with .variables/scope. The NNX
  adapter does not have these. Instead, we use jax.lax.scan + jax.checkpoint which
  is functionally equivalent but may not trigger _partial_pack's XLA scheduling.
- The adapter's mutable state (layers_mutables) is a Python-level side effect
  within the scan body. Under jax.lax.scan this works because the state is carried
  explicitly; the adapter merely provides a compatible call interface.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V40: Use pipeline_utils for custom_vjp gradient accumulation ----
    #
    # Strategy: use pipeline_utils.create_gradient_accumulation_scan() and
    # pipeline_utils.create_pipeline_stage() which define custom VJP rules
    # for memory-efficient gradient accumulation. These are pure JAX functions
    # (no Linen scope needed).
    #
    # For the outer scan over repeats, we use jax.lax.scan + jax.checkpoint
    # which is the JAX equivalent of nn.scan(nn.remat(...)). We cannot use
    # create_flax_pipeline_scan() directly because it calls nn.scan which
    # requires a Linen module with .variables/scope.
    #
    # Adapter bridges NNX's run_one_iteration signature to the Linen signature
    # expected by pipeline_utils functions:
    #   Linen: model.run_one_iteration(loop_state, bsw, pos, seg, det, mode, logical_partition_spec=...)
    #   NNX:   self.run_one_iteration(loop_state, bsw, graph, metrics, mutables, pos, seg, det, mode, spec)

    # Mutable state container for the adapter -- carried explicitly through
    # the scan carry and updated by the adapter after each run_one_iteration.
    # This avoids Python-level side effects inside the scan body.
    mutables_ref = [layers_mutables]

    class _PipelineAdapter:
      """Adapter to make NNX pipeline compatible with pipeline_utils functions.

      pipeline_utils.create_gradient_accumulation_scan() and
      create_pipeline_stage() call model.run_one_iteration(...) with
      Linen's signature. This adapter translates to NNX's signature.

      The adapter also provides get_pipeline_remat_policy() and
      weight_prefetching() which pipeline_utils calls on the model.
      """

      def __init__(self, pipeline, p_layers_graph, p_layers_metrics):
        self.pipeline = pipeline
        self.config = pipeline.config
        self.mesh = pipeline.mesh
        self._layers_graph = p_layers_graph
        self._layers_metrics = p_layers_metrics

      def run_one_iteration(self, loop_state, bsw, pos_or_positions, seg_or_segment_ids,
                            deterministic_arg, model_mode_arg, logical_partition_spec=None):
        """Linen-compatible signature called by pipeline_utils functions.

        pipeline_utils.create_gradient_accumulation_scan() calls:
          model.run_one_iteration(l, b, pos_arg, seg_arg, deterministic, model_mode,
                                  logical_partition_spec=logical_partition_spec)
        where l=lightweight_state (loop_state), b=bsw.
        """
        iteration = loop_state["loop_iteration"]
        current_mutables = jax.lax.stop_gradient(mutables_ref[0])
        advanced_mutables = _advance_rng_state(current_mutables, iteration)

        new_loop_state, new_layer_state = self.pipeline.run_one_iteration(
            loop_state, bsw, self._layers_graph, self._layers_metrics,
            advanced_mutables, pos_or_positions, seg_or_segment_ids,
            deterministic_arg, model_mode_arg, logical_partition_spec,
        )

        # Extract updated mutables and store for next iteration
        _, _, _new_metrics, new_mutables = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        mutables_ref[0] = new_mutables

        return new_loop_state

      def get_pipeline_remat_policy(self):
        return self.pipeline.get_pipeline_remat_policy()

      def weight_prefetching(self, weights_state, physical_partition_spec, loop_iteration):
        return self.pipeline.weight_prefetching(weights_state, physical_partition_spec, loop_iteration)

    adapter = _PipelineAdapter(self, layers_graph, layers_metrics)

    # ---- Build the pipeline stage function via pipeline_utils ----
    #
    # create_pipeline_stage() returns execute_pipeline_stage_flax(model, carry)
    # which:
    #   1. Calls create_gradient_accumulation_scan() for the inner microbatch loop
    #   2. Wraps weight prefetching in a custom_vjp with linear_transpose for
    #      reduce-scatter in backward
    #   3. Returns (loop_state, w_next, pipeline_weights), None
    num_microbatches = self.config.num_pipeline_microbatches
    num_repeats = self.config.num_pipeline_repeats

    pipeline_stage_fn = pipeline_utils.create_pipeline_stage(
    ...
    # ---- Outer scan body: one repeat ----
    # Mirrors create_flax_pipeline_scan's nn.scan(nn.remat(pipeline_stage_fn))
    # but uses jax.lax.scan + jax.checkpoint since we lack a Linen module.

    def outer_body(carry, _):
      """One repeat: calls pipeline_stage_fn which handles BSW + gradient accum."""
      loop_state_carry, layer_mutables_carry, w_curr, p_weights = carry

      # Update mutables_ref so the adapter sees current mutables
      mutables_ref[0] = layer_mutables_carry

      # pipeline_stage_fn(model, carry) -> ((loop_state, w_next, pipeline_weights), None)
      (new_loop_state, w_next, p_weights_out), _ = pipeline_stage_fn(
          adapter, (loop_state_carry, w_curr, p_weights)
      )

      # Retrieve updated mutables from the adapter (set during run_one_iteration calls)
      new_layer_mutables = mutables_ref[0]

      return (new_loop_state, new_layer_mutables, w_next, p_weights_out), None

    if self.config.set_remat_policy_on_pipeline_iterations:
      outer_body = jax.checkpoint(
          outer_body, policy=self.get_pipeline_remat_policy(),
    ...
    if self.config.scan_pipeline_iterations:
      ...
      (loop_state, final_layer_mutables, final_w_curr, _), _ = jax.lax.scan(
          outer_body, (loop_state, layers_mutables, initial_w_curr, layers_params),
          None, length=num_repeats, unroll=unroll_repeats,
      )
    else:
    ...
    # During bubbles, no weight prefetching needed -- reuse final weights.
    # Build a separate pipeline_stage_fn for bubble length.
    if bubble_iterations > 0:
      bubble_stage_fn = pipeline_utils.create_pipeline_stage(
      ...
      def bubble_body(carry, _):
        ...
        (new_loop_state, w_next, p_weights_out), _ = bubble_stage_fn(
            adapter, (loop_state_carry, w_curr, p_weights)
        )
      ...
      # Bubbles run once (length=1 outer) with bubble_iterations inner microbatches
      bubble_carry = (loop_state, final_layer_mutables, final_w_curr, layers_params)

      if self.config.scan_pipeline_iterations:
        (loop_state, final_layer_mutables, _, _), _ = jax.lax.scan(
            bubble_body, bubble_carry, None, length=1
        )
      else:
        bubble_carry, _ = bubble_body(bubble_carry, None)
        loop_state, final_layer_mutables, _, _ = bubble_carry

    # ---- Finalize: merge state and reshape output ----
    # Use placeholder metrics (actual metrics flow through module state).
    # Since pipeline_utils' custom_vjp functions don't return per-step metrics
    # in the same way as the baseline scan, we use the initial metrics as
    # the stacked placeholder.
