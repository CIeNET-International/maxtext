...
"""V66: From-scratch NNX pipeline with two-level memory control.

Based on V65i (21.3 GB, correct gradients). Two structural changes:
1. Outer checkpoint: nothing_saveable — BSW recomputed in backward, never stored
2. Inner microbatch: custom_vjp saves only (loop_state, rng) — not checkpoint-determined residuals

Expected: <=21 GB memory, ~85 tok/s (same throughput as V65i, potentially less memory).
"""
  ...
  def __call__(
    ...
    def execute_one_repeat(carry, repeat_index):
      ...
      def run_microbatch_scan(loop_state_in, rng_mutables_in, bsw):
        @jax.custom_vjp
        def microbatch_step_cvjp(carry_ls, carry_rng):
          return run_single_microbatch((carry_ls, carry_rng), None, bsw)

        def microbatch_step_cvjp_fwd(carry_ls, carry_rng):
          result = microbatch_step_cvjp(carry_ls, carry_rng)
          residuals = (carry_ls, carry_rng)
          return result, residuals

        def microbatch_step_cvjp_bwd(residuals, g_out):
          saved_ls, saved_rng = residuals
          remat_fn = jax.checkpoint(
              lambda ls, rm: run_single_microbatch((ls, rm), None, bsw),
              policy=self.get_pipeline_remat_policy(),
          )
          _, vjp_fn = jax.vjp(remat_fn, saved_ls, saved_rng)
          d_ls, d_rng = vjp_fn(g_out)
          return d_ls, d_rng

        microbatch_step_cvjp.defvjp(microbatch_step_cvjp_fwd, microbatch_step_cvjp_bwd)

        def microbatch_step(microbatch_carry, _unused):
          ls, rng = microbatch_carry
          (new_ls, new_rng), metrics = microbatch_step_cvjp(ls, rng)
          return (new_ls, new_rng), metrics
