...
V62: Uses flax.core.apply + lift.scan + lift.checkpoint to get _partial_pack
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def _inner_scan_body_for_scope(scope, carry):
      """Inner scan body operating with Linen Scope isolation.

      Args:
        scope: Linen Scope providing isolated variable access.
               Variables in 'carry_state' collection are carried through scan.
               Variables in 'metrics' collection are stacked as output.
        carry: The plain JAX carry = loop_state dict (not in scope variables).

      Returns:
        (new_carry, scan_output) where scan_output is the metrics arrays.
      """
      ...
      # Apply stop_gradient and advance RNG state (same as original)
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)

      # Run one pipeline iteration (identical to original inner_body)
      new_loop_state, new_layer_state = self.run_one_iteration(
    ...
    scanned_fn = flax_lift.scan(
        ...
        variable_axes={'metrics': 0},      # metrics stacked on axis 0
    ...
    apply_fn = flax_scope.apply(
        ...
        mutable=['carry_state', 'metrics'],  # both collections are mutable
    )
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        ...
        # Initialize metrics collection with zeros (scan will populate it)
        metrics_init = _arrays_to_linen_collection(met_arrays, met_keys)
        ...
        # apply_fn signature: (variables, *args, rngs=...) -> (result, mutated_vars)
        new_loop_state, mutated_vars = apply_fn(
            linen_variables,
            current_loop_state,
        )
        ...
        # Extract stacked metrics from Linen metrics collection
        stacked_met_collection = mutated_vars['metrics']
        inner_met_arrays = _linen_collection_to_arrays(stacked_met_collection, met_keys)
        # inner_met_arrays are already stacked by lift.scan along axis 0
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        ...
        bubble_metrics_init = _arrays_to_linen_collection(met_arrays, met_keys)

        bubble_linen_vars = {
            'carry_state': bubble_carry_init,
            'metrics': bubble_metrics_init,
        }

        loop_state, bubble_mutated = bubble_apply_fn(
            bubble_linen_vars,
            loop_state,
        )
        ...
        # Extract bubble metrics
        bubble_met_collection = bubble_mutated['metrics']
        bubble_met_out = _linen_collection_to_arrays(bubble_met_collection, met_keys)
