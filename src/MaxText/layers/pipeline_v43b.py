...
"""Pipeline V43b: Author's pattern extended — ALL large state as explicit remat args.

V43 passed params+metrics explicitly to remat_body. V43b goes further:
passes layers_graph AND all large state explicitly. Minimizes closure
captures of the checkpoint function — only self methods and small Python
values (deterministic, model_mode, spec) remain as closures.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V43b: ALL large state as explicit remat args ----
    #
    # V43 passed params+metrics explicitly. V43b passes EVERYTHING large:
    # params, metrics, graph, positions, segment_ids, bsw — all explicit args.
    # Only self methods and small Python values remain as closures.
    num_microbatches = self.config.num_pipeline_microbatches

    def make_inner_body(bsw):
      """Factory: scan wrapper + remat_body with all large state explicit."""

      def remat_body(params, metrics, graph, pos, seg, bsw_arg, carry_state):
        """Checkpoint body. ALL large JAX arrays are EXPLICIT positional args."""
        ...
        new_loop_state, new_layer_state = self.run_one_iteration(
            current_loop_state, bsw_arg, graph, metrics,
            advanced_mutables, pos, seg, deterministic,
