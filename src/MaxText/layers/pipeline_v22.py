...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      current_loop_state, current_layer_mutables, _prev_metrics = carry
      ...
      return (new_loop_state, new_layer_mutables, new_layer_metrics), None
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        (new_loop_state, new_layer_mutables, last_metrics), _ = jax.lax.scan(
            inner_body, (current_loop_state, current_layer_mutables, layers_metrics),
            None, length=num_microbatches,
        )
      else:
        inner_carry = (current_loop_state, current_layer_mutables, layers_metrics)
        for _ in range(num_microbatches):
          inner_carry, _ = inner_body(inner_carry, None)
        new_loop_state, new_layer_mutables, last_metrics = inner_carry

      return (new_loop_state, new_layer_mutables, w_next), None
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        (loop_state, final_layer_mutables, _bubble_metrics), _ = jax.lax.scan(
            inner_body, (loop_state, final_layer_mutables, layers_metrics),
            None, length=bubble_iterations
        )
      else:
        bubble_carry = (loop_state, final_layer_mutables, layers_metrics)
        for _ in range(bubble_iterations):
          bubble_carry, _ = inner_body(bubble_carry, None)
        loop_state, final_layer_mutables, _bubble_metrics = bubble_carry

    # V22: metrics from final carry state only (not stacked from ys)
    # Use layers_metrics as placeholder — actual metrics are in module Intermediate state
