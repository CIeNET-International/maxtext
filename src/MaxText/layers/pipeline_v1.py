...
"""Pipeline V1: Triple Freeze (jax.vmap + FrozenDict + stop_gradient).

Variant of pipeline.py with the following changes to NNXCircularPipeline.__call__:
  1. FrozenDict wrapping on layers_params and layers_metrics after nnx.split
  2. stop_gradient on current_layer_mutables inside inner_body
  3. jax.vmap in get_main_vmap_func_for_iterations (baseline already has this)
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V1: Freeze broadcast variables to prevent JAX from treating them as
    ...
    def inner_body(carry, _):
      current_loop_state, current_layer_mutables = carry
      # V1: Stop gradients through mutables (RNG state) — they are not
