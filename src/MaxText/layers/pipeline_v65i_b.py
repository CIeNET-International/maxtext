...
"""V65i-B: Carry FSDP-sharded w_curr to reduce scan carry size.

Based on V65i-A (29.7 GB, 113.6 tok/s, correct gradients).
V65i-A carries the ALL-GATHERED (unsharded) w_curr in the outer scan carry,
which is ~4x larger than necessary due to FSDP replication. JAX stores the
carry per iteration for backward, so this inflates memory.

V65i-B carries the PRE-all-gather SHARDED weights instead. Each repeat:
  1. All-gather w_curr_sharded -> w_curr_gathered (1 all-gather)
  2. Call weight_prefetching for w_next (slice + all-gather = 1 all-gather)
  3. Extract w_next_sharded via from_all_variables_to_repeat_weights (no all-gather)
  4. Return w_next_sharded as carry for the next repeat

This is still 2 all-gathers per repeat (same as V65i), but the carry is ~4x
smaller than V65i-A. Expected memory: between V65i (21.3 GB) and V65i-A (29.7 GB).

Comparison:
  V65i:   21.3 GB, 85.6 tok/s  — no carry, 2 all-gathers/repeat (recompute both)
  V65i-A: 29.7 GB, 113.6 tok/s — carry gathered w_curr, 1 all-gather/repeat
  V65i-B: ~23-25 GB, ~90-100 tok/s — carry sharded w_curr, 2 all-gathers/repeat
  Linen:  23.3 GB, 116.5 tok/s — target
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def execute_one_repeat(carry, repeat_index):
      loop_state, rng_mutables, w_curr_sharded = carry

      total_iteration_at_repeat_start = repeat_index * num_microbatches

      # Step 1: All-gather the sharded carry to get full w_curr for BSW
      w_curr_gathered = self.from_repeat_weights_to_bsw(w_curr_sharded, physical_partition_spec_full)

      # Step 2: Prefetch w_next (slice + all-gather = 1 all-gather)
      w_next_gathered = self.weight_prefetching(
          layers_params, physical_partition_spec_full, total_iteration_at_repeat_start
      )

      # Step 3: Extract sharded form of w_next for carry (no all-gather, just slice)
      w_next_sharded = self.from_all_variables_to_repeat_weights(
          layers_params, total_iteration_at_repeat_start + 1
      )

      buffer_sliding_window = (w_curr_gathered, w_next_gathered)
      ...
      return (new_loop_state, updated_rng_mutables, w_next_sharded), repeat_step_metrics
    ...
    # Init carry with SHARDED zeros (from_all_variables_to_repeat_weights returns
    # the pre-all-gather sharded form, which is ~4x smaller than gathered).
    init_w_curr_sharded = self.from_all_variables_to_repeat_weights(layers_params, 0)
    init_w_curr_sharded = jax.tree.map(jnp.zeros_like, init_w_curr_sharded)

    (loop_state, final_rng_mutables, final_w_curr_sharded), repeat_metrics = jax.lax.scan(
        checkpointed_repeat_body,
        (loop_state, layers_mutables, init_w_curr_sharded),
    ...
    if bubble_iterations > 0:
      ...
      # All-gather the final sharded carry to get full weights for bubble BSW
      last_repeat_weights = self.from_repeat_weights_to_bsw(
          final_w_curr_sharded, physical_partition_spec_full
      )
