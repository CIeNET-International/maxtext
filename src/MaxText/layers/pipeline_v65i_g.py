...
"""V65i-G: V65i-A with nothing_saveable checkpoint policy on outer repeat body.

Based on V65i-A (w_curr carried in outer scan to halve all-gathers).
Change: replace get_pipeline_remat_policy() with jax.checkpoint_policies.nothing_saveable
for the outer repeat body checkpoint. Forces recomputation of w_curr in backward -- not
saved as checkpoint. This means w_curr is never stored, but recomputed on demand via
weight_prefetching from the layers_params closure.
Expected: reduced memory at cost of recomputation time (1 extra all-gather per repeat in backward).
"""
