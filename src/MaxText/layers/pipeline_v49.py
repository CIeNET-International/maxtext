...
V49 (Solution 6 -- HLO computation boundary)
--------------------------------------------
The NNX circular pipeline compiles to ~29.9 GB vs ~23.3 GB for Linen. The 6.6 GB
gap comes from scope isolation: Linen's ``_partial_pack`` emits 8 separate XLA
``while`` loops, giving buffer assignment disjoint live ranges; the NNX port's
``jax.checkpoint`` + ``jax.lax.scan`` collapses to 4, so XLA fuses regions whose
buffers would otherwise not overlap.

V49 inserts a *hard HLO boundary* -- ``jax.lax.optimization_barrier`` via
``maxtext.utils.hlo_boundary`` -- around the scan carry at the top of every scan
body (``inner_body`` and ``outer_body``). An optimization barrier lowers to the
``stablehlo.optimization_barrier`` instruction, which XLA's algebraic simplifier,
CSE, fusion and buffer-assignment passes are all forbidden to move data flow
across. It is the same primitive XLA uses to fence rematerialized regions, so it
*survives every optimization pass* -- there is no foldable identity op for XLA to
remove. It is a true identity for both forward values and gradients (registered
identity JVP / transpose / batching rules), so loss must match the baseline
exactly. The boundary forces XLA to keep the pre- and post-boundary buffers in
separate scheduling regions, approximating ``_partial_pack``'s scope isolation.
See ``maxtext/utils/hlo_boundary.py`` for the full rationale.
"""
...
from maxtext.utils.hlo_boundary import hlo_boundary
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  Key design decisions (from commit bb87194 through current):
    ...
    - V49: an HLO computation boundary (jax.lax.optimization_barrier, via
      hlo_boundary()) is inserted on the scan carry at the top of inner_body
      and outer_body. This forces XLA to keep the pre- and post-boundary
      buffers in separate scheduling regions -- mimicking the scope isolation
      that Linen's _partial_pack achieves -- to close the ~6.6 GB compile-time
      memory gap. The barrier is a true identity for values and gradients.
  """
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      # V49: hard HLO boundary on the incoming carry. optimization_barrier
      # forces XLA to treat buffers produced before this point and those
      # produced after as separate scheduling regions (no fusion / CSE across
      # the boundary), approximating Linen _partial_pack scope isolation.
      # It is a value- and gradient-identity, so loss is unaffected.
      carry = hlo_boundary(carry)
      current_loop_state, current_layer_mutables = carry
    ...
    def outer_body(carry, _):
      ...
      # V49: hard HLO boundary on the incoming carry (loop_state, mutables,
      # w_curr). Keeps each repeat's weight buffers (w_curr) and loop-state
      # buffers in a distinct XLA scheduling region from the previous repeat,
      # so XLA buffer assignment sees disjoint live ranges. Value/grad identity.
      carry = hlo_boundary(carry)
