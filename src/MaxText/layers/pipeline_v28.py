...
"""Pipeline V28: Pure functional scan body (no self, no closures over module).

Variant targeting scan body equation count. Rewrites inner_body to be a PURE
function with NO closures over `self` or module state. All methods needed
(run_one_iteration etc.) are pre-bound and passed as explicit references.
The inner_body_pure function is defined at the class level as a static method,
ensuring JAX traces it as a pure function without implicit closure references
to `self`. This may reduce the number of equations JAX captures in the scan
body trace by eliminating closure-captured state traversals.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    #
    # V28: Pure functional scan body. All methods pre-bound, inner_body_pure
    # defined without closures over `self`. Only bsw_ref remains as a mutable
    # closure (unavoidable -- BSW in carry would OOM).
    #
    # outer scan (repeats):
    ...
    # V28: Pre-bind all methods needed by inner_body as plain function references.
    # This eliminates closure-captured `self` traversals during JAX tracing.
    _bound_run_one_iteration = self.run_one_iteration
    _bound_weight_prefetching = self.weight_prefetching
    _remat_policy = self.get_pipeline_remat_policy()
    _set_remat = self.config.set_remat_policy_on_pipeline_iterations
    _scan_iterations = self.config.scan_pipeline_iterations

    # V28: Pure functional inner body -- no `self` reference inside.
    # All dependencies passed as pre-bound references or explicit args via lambda wrapper.
    def _inner_body_pure(carry, _, run_iter_fn, bsw, graph, metrics, pos, seg, det, mode, lps):
      """Pure inner scan body with no closure over self or module state."""
      current_loop_state, current_layer_mutables = carry
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = run_iter_fn(
          current_loop_state, bsw, graph, metrics,
          advanced_mutables, pos, seg, det,
          mode, lps,
      )
    ...
    # Lambda wrapper: binds non-scan args (bsw_ref is still mutable closure)
    def inner_body(carry, _):
      return _inner_body_pure(
          carry, _,
          _bound_run_one_iteration,
          bsw_ref[0], layers_graph, layers_metrics,
          positions, segment_ids, deterministic,
          model_mode, logical_partition_spec_stripped,
      )

    if _set_remat:
      inner_body = jax.checkpoint(
          inner_body, policy=_remat_policy,
          prevent_cse=not _scan_iterations,
      )
    ...
    def outer_body(carry, _):
      ...
      w_next = _bound_weight_prefetching(
