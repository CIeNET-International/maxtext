...
"""V65i-C: V65i-A with stop_gradient on w_curr carry to prevent backward storage.

Based on V65i-A (29.7 GB, 113.6 tok/s, correct gradients).
Change: apply jax.lax.stop_gradient to w_curr after unpacking from carry.
Rationale: w_curr is only needed in the forward pass to build BSW = (w_curr, w_next).
Weight gradients flow entirely through w_next -> layers_params (closure path).
Linen's custom_vjp confirms this: _execute_stage_bwd returns g_w_curr = zeros_like(g_w_next),
meaning the carry gradient for w_curr is always zero. stop_gradient makes this explicit,
so JAX does not store w_curr per-iteration for the backward pass.
Expected: V65i-A throughput (~113 tok/s) with much less memory (~21-25 GB).
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def execute_one_repeat(carry, repeat_index):
      loop_state, rng_mutables, w_curr = carry

      # V65i-C: stop_gradient on w_curr prevents JAX from storing it at every
      # scan iteration for the backward pass.  Weight gradients flow entirely
      # through w_next (freshly computed each repeat via closure over
      # layers_params).  This mirrors Linen's custom_vjp which returns
      # g_w_curr = zeros_like(g_w_next) in _execute_stage_bwd.
      w_curr = jax.tree.map(jax.lax.stop_gradient, w_curr)
      ...
      return (new_loop_state, updated_rng_mutables, w_next), repeat_step_metrics
    ...
    init_w_curr = self.weight_prefetching(layers_params, physical_partition_spec_full, -1)
    init_w_curr = jax.tree.map(jnp.zeros_like, init_w_curr)

    (loop_state, final_rng_mutables, final_w_curr), repeat_metrics = jax.lax.scan(
        checkpointed_repeat_body,
        (loop_state, layers_mutables, init_w_curr),
    ...
    if bubble_iterations > 0:
      ...
      last_repeat_weights = final_w_curr
