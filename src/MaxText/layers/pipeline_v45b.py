...
"""Pipeline V45b: Double checkpoint (nested remat) for NNX scope isolation.

Design agent found: nested jax.checkpoint creates TWO remat_p primitives in
the jaxpr, each lowered to a separate XLA computation with its own while loop.

Pattern:
  OUTER jax.checkpoint wraps scan_body → broadcast_state as closure → remat_p #1
  INNER jax.checkpoint wraps inner_body → broadcast_state as explicit arg → remat_p #2

This produces nested remat boundaries matching nn.scan(nn.remat(body))'s
double _partial_pack isolation. Expected: 6-8 HLO while loops vs baseline's 4.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V45b: Double checkpoint (nested remat) for scope isolation ----
    #
    # Design agent found: nested jax.checkpoint creates TWO remat_p primitives
    # in the jaxpr, each lowered to a separate XLA computation → more while loops.
    #
    # Pattern:
    #   OUTER jax.checkpoint wraps scan_body → remat_p #1
    #     broadcast_state (layers_params) as closure → convert_constvars_jaxpr → invar
    #   INNER jax.checkpoint wraps inner_compute → remat_p #2
    #     broadcast_state as EXPLICIT positional arg → separate invar
    #
    # This creates nested remat boundaries matching nn.scan(nn.remat(body))'s
    # double _partial_pack. Expected: 6-8 HLO while loops vs baseline's 4.
    ...
    bsw_ref = [None]  # mutable reference to hold BSW across inner loop iterations without putting it in the carry

    def make_inner_body(bsw):
      """Factory: inner body with INNER checkpoint boundary."""

      # INNER checkpoint: broadcast params as EXPLICIT positional arg
      # This creates remat_p #2 with params as invar (not constvar)
      def inner_compute(broadcast_params, carry_state):
        """Inner remat boundary. broadcast_params is explicit arg."""
        current_loop_state, current_layer_mutables = carry_state
      ...
      if remat_policy is not None:
        inner_compute = jax.checkpoint(inner_compute, policy=remat_policy)

      def inner_body(carry, _):
        """Scan body. Calls inner_compute with params as EXPLICIT arg."""
        current_loop_state, current_layer_mutables = carry
        new_ls, new_mut, new_metrics = inner_compute(
            layers_params, (current_loop_state, current_layer_mutables)
        )
    ...
    def outer_body(carry, _):
      """One repeat. Will be wrapped in OUTER checkpoint."""
      ...
      if self.config.scan_pipeline_iterations:
        ...
        for _ in range(num_microbatches):
          inner_carry, step_m = body(inner_carry, None)
    ...
    # OUTER checkpoint: wraps outer_body → remat_p #1
    # layers_params is closure here → convert_constvars_jaxpr makes it invar
    # Combined with INNER checkpoint, creates nested remat_p boundaries
    if remat_policy is not None:
