...
"""Pipeline V41: Expose per-repeat method for decoder-level nn.scan.

NNXCircularPipeline.__call__() wraps the outer repeat loop in
nn.scan(nn.remat(...)) to invoke _partial_pack for better XLA buffer
scheduling. The inner microbatch scan and bubble iterations remain inside
the pipeline module.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V41: nn.scan(nn.remat(...)) for outer repeat loop ----
    #
    # Wraps the outer repeat body in a Linen nn.Module subclass so that
    # nn.scan(nn.remat(RepeatStage)) invokes _partial_pack at the scan
    # boundary, potentially improving XLA buffer scheduling.
    #
    # Inner microbatch scan uses jax.lax.scan (same as baseline).
    # BSW is stored in bsw_ref (closure, NOT carry -- carry would OOM).
    bsw_ref = [None]
    num_microbatches = self.config.num_pipeline_microbatches
    num_repeats = self.config.num_pipeline_repeats

    # Capture pipeline self for use inside Linen module closure
    pipeline_self = self
    ...
    # -- Linen Module wrapping one repeat for nn.scan(nn.remat(...)) --
    class RepeatStage(nn.Module):
      """One pipeline repeat as a Linen module for nn.scan wrapping.

      All pipeline data (layers_params, layers_graph, etc.) flows through
      closures, not Linen variables. The nn.scan + nn.remat wrapper creates
      the _partial_pack boundary that may improve XLA scheduling.
      """
    ...
    # ---- Build nn.scan(nn.remat(RepeatStage)) ----
    remat_policy = (
    ...
    # nn.remat wraps the RepeatStage CLASS, adding checkpoint boundaries.
    # nn.scan wraps the result, creating a scan over repeats that invokes
    # _partial_pack to isolate Linen variables at the scan boundary.
    RemattedRepeatStage = nn.remat(
        RepeatStage,
        policy=remat_policy,
        prevent_cse=not self.config.scan_pipeline_iterations,
    ) if remat_policy is not None else RepeatStage
    ...
    # nn.scan creates an instance and calls it; carry flows through __call__.
    # _partial_pack is triggered by nn.scan variable management.
    scanned_stage = ScanRepeatStage(name="repeat_scan")
    (loop_state, final_layer_mutables, final_w_curr), repeat_metrics = scanned_stage(initial_carry)

    # Reshape stacked metrics: nn.scan stacks [num_repeats, ...] on top of
    # the inner scan's [num_microbatches, ...], giving [num_repeats, num_microbatches, ...].
    if self.config.scan_pipeline_iterations:
      ...
      # Unrolled inner: metrics are already [num_microbatches, ...] per repeat,
      # nn.scan stacks to [num_repeats, num_microbatches, ...].
      repeat_metrics = jax.tree.map(
          lambda x: x.reshape((num_repeats * num_microbatches,) + x.shape[2:]),
          repeat_metrics,
      )
