...
"""Pipeline V42: nn.scan with ACTUAL Linen variables at scan boundary.

V41 proved empty nn.scan (no Linen vars) doesn't help. V42 injects NNX pipeline
weights into a Linen variable scope so _partial_pack has real variables to manage.

Hybrid approach: NNX for inner computation, Linen variables at scan boundary.
RepeatStage Linen module holds pipeline_params as self.variable() — managed by
_partial_pack via variable_broadcast (broadcast as const, not scan carry).
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    #
    # V41 showed empty nn.scan (no Linen vars) doesn't help.
    # V42 injects NNX pipeline weights into a Linen module's variable scope
    # so _partial_pack has real variables to manage at the scan boundary.
    #
    # Hybrid: NNX for inner computation, Linen variables for scan boundary.
    num_microbatches = self.config.num_pipeline_microbatches
    num_repeats = self.config.num_pipeline_repeats
    bsw_ref = [None]
    pipeline_self = self

    # Inner body for microbatch scan (pure JAX, same as baseline)
    def make_inner_body(bsw):
      ...
      if pipeline_self.config.set_remat_policy_on_pipeline_iterations:
        inner_body = jax.checkpoint(
            inner_body, policy=pipeline_self.get_pipeline_remat_policy(),
            prevent_cse=not pipeline_self.config.scan_pipeline_iterations,
        )
      return inner_body

    # Linen module for one repeat — holds pipeline weights as ACTUAL Linen variables.
    # _partial_pack will manage these via variable_broadcast at scan boundary.
    class RepeatStage(nn.Module):
      """One pipeline repeat with pipeline weights as Linen variables."""

      @nn.compact
      def __call__(self_linen, carry):
        # Pipeline weights as Linen variable — this is what _partial_pack manages!
        # Initialized from NNX layers_params, stored in Linen's scope system.
        # variable_broadcast=["pipeline_params"] will broadcast these as consts.
        pp_weights = self_linen.variable(
            "pipeline_params", "weights",
            lambda: jax.tree.map(lambda x: x, layers_params),
        )
        ...
        w_next = pipeline_self.weight_prefetching(
            pp_weights.value, physical_partition_spec_full, iteration
        )
        bsw = (w_curr, w_next)
        ...
        if pipeline_self.config.scan_pipeline_iterations:
          (new_loop_state, new_layer_mutables), inner_metrics = jax.lax.scan(
              body, (current_loop_state, current_layer_mutables),
    ...
    # Wrap RepeatStage in nn.scan(nn.remat(...))
    # _partial_pack manages "pipeline_params" collection via variable_broadcast
    remat_policy = (
    ...
    ScanRepeatStage = nn.scan(
        nn.remat(RepeatStage, policy=remat_policy) if remat_policy else RepeatStage,
        variable_broadcast=["pipeline_params"],
        variable_axes={"intermediates": 0},
        split_rngs={"random": True},
        length=num_repeats,
        unroll=num_repeats if not self.config.scan_pipeline_repeats else 1,
    )

    # Initialize and run the scanned repeat stage
    initial_w_curr = jax.tree.map(lambda x: jnp.zeros(x.shape[1:], dtype=x.dtype), layers_params)
    initial_carry = (loop_state, layers_mutables, initial_w_curr)

    # Init the Linen module to create the variable scope
    scan_stage = ScanRepeatStage()
    rng_init = jax.random.key(0)
    scan_variables = jax.eval_shape(
        lambda: scan_stage.init(rng_init, initial_carry)
    )
    # Inject actual NNX weights into the Linen variable scope
    scan_variables = scan_stage.init(rng_init, initial_carry)
    scan_variables = scan_variables.copy({"pipeline_params": {"weights": layers_params}})

    # Run the scanned repeats
    (loop_state, final_layer_mutables, final_w_curr), repeat_metrics = scan_stage.apply(
        scan_variables,
        initial_carry,
        rngs={"random": jax.random.key(1)},
    )

    if self.config.scan_pipeline_iterations and num_repeats > 1:
