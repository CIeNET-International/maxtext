...
"""Pipeline V12: Explicit params for jax.checkpoint (V10 + Finding 6).

All V10 fixes preserved (Findings 1-5), plus:
  6. (Finding 6) BSW weights passed as EXPLICIT ARGUMENT to jax.checkpoint instead
     of closure capture. This mimics what Flax's _partial_pack does at the nn.remat
     level -- making params visible to jax.remat's partial evaluator.

     In V10, inner_body closes over bsw_ref[0] (the BSW dual-buffer). jax.checkpoint
     sees this as opaque closed-over data and cannot reason about which intermediates
     depend on params vs carry. By passing BSW as an explicit function argument,
     jax.remat's partial evaluator can see the dependency structure and potentially
     make better rematerialization decisions (e.g., not saving param-dependent
     intermediates that can be cheaply recomputed from the explicit param input).

     The computational semantics are identical to V10 -- only the mechanism by which
     params reach the checkpointed function changes.

Previous findings retained:
  1. (Finding 1) vmap returns only non-param state, no param stacking across stages.
  2. (Finding 2) stop_gradient on current_layer_mutables -- no gradient tape on RNG state.
  3. (Finding 3) stop_gradient on w_curr in outer scan carry -- gradients flow through
     the custom BSW/all-gather path, not through w_curr carried across repeats.
  4. (Finding 4) No nnx.split inside checkpoint boundary -- Variable IDs change between
     fwd/bwd causing recomputation. Move split outside the checkpointed region.
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    #
    # V12 Finding 6: BSW is passed as an EXPLICIT argument to the checkpointed
    # function instead of being captured via closure (bsw_ref). This makes params
    # visible to jax.remat's partial evaluator, mirroring Flax's _partial_pack
    # pattern. bsw_ref is still used as a mutable container to communicate BSW
    # from outer_body to inner_body, but the checkpointed function itself receives
    # BSW as a proper argument.
    #
    ...
    def _inner_body_with_explicit_bsw(bsw, carry, _):
      """Core inner body that takes BSW as an explicit argument.

      V12: By making BSW an explicit function argument instead of a closure
      capture, jax.checkpoint's partial evaluator can see the dependency
      between intermediates and the param tensor. This mirrors what Flax's
      _partial_pack does at the nn.remat level.

      Args:
        bsw: The BSW dual-buffer (w_curr, w_next) for this repeat.
        carry: Tuple of (loop_state, layer_mutables).
        _: Unused scan xs slot.

      Returns:
        Same as inner_body: ((new_loop_state, new_layer_mutables), new_layer_metrics).
      """
      current_loop_state, current_layer_mutables = carry
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      _inner_body_with_explicit_bsw = jax.checkpoint(
          _inner_body_with_explicit_bsw, policy=self.get_pipeline_remat_policy(),
          prevent_cse=not self.config.scan_pipeline_iterations,
      )

    def inner_body(carry, xs):
      """Thin wrapper that reads BSW from bsw_ref and passes it explicitly
      to the checkpointed function. This is the function that jax.lax.scan sees.

      V12: bsw_ref[0] is still used to communicate BSW from outer_body, but
      the actual checkpointed computation receives BSW as an explicit first
      argument rather than capturing it via closure.
      """
      return _inner_body_with_explicit_bsw(bsw_ref[0], carry, xs)
