...
"""Pipeline V13: Flax _partial_pack variable-group pattern in pure JAX.

Reproduces Flax's _partial_pack mechanism (flax.core.lift) where variables are
split into explicit groups (broadcast, carry, scan_axes) and threaded as
POSITIONAL ARGUMENTS through both scan and checkpoint boundaries.

Flax's pattern (from lift.py:_partial_pack + scan):
  1. _partial_pack partitions scope variables into groups via filters:
     - broadcast_vars (params): frozen, loop-invariant, no gradient through carry
     - carry_vars (mutables): threaded through iterations, mutable
     - scan_vars (metrics): stacked along scan axis
  2. Creates scope_fn(variable_groups, rng_groups) -> reconstructed scope
  3. Creates repack_fn(scope) -> extracts updated variable groups
  4. The scan body signature becomes:
       scanned(broadcast_vars, carry, scan_vars, rng_groups, args)
     where ALL variable groups are explicit positional args to the JAX primitive.

V13 adaptation for NNX pipeline:
  - broadcast_vars = layers_params (frozen via stop_gradient, no carry gradient)
  - carry_vars = layers_mutables (RNG state, threaded through scan carry)
  - scan_vars = layers_metrics (intermediate outputs, stacked along scan axis)
  - Both scan body AND checkpoint body receive these as explicit arguments,
    not via closure capture. This ensures JAX sees the same variable topology
    as Flax's nn.scan would produce.

Key differences from V10:
  - V10: layers_params captured via closure in scan_body/inner_body
  - V13: layers_params passed as explicit broadcast_vars argument through
    both jax.lax.scan and jax.checkpoint function boundaries
  - V10: checkpoint wraps inner_body which captures BSW via bsw_ref[0]
  - V13: checkpoint function receives (broadcast_params, carry_state, xs)
    as explicit positional args, mirroring Flax's scope_fn/repack_fn pattern

All V10 fixes are retained:
  1. (Finding 1) vmap returns only non-param state
  2. (Finding 2) stop_gradient on mutables
  3. (Finding 3) stop_gradient on broadcast params (w_curr)
  4. (Finding 4) nnx.split outside checkpoint boundary
  5. (Finding 5) No metric scatter-update
"""
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V13: _partial_pack variable-group pattern ----
    #
    # Flax's _partial_pack splits variables into groups and makes them
    # explicit positional args to the scan body:
    #   scanned(broadcast_vars, carry, scan_vars, rng_groups, args)
    #
    # V13 NNXPipeline adaptation:
    #   broadcast_vars = layers_params (frozen, stop_gradient prevents carry gradient)
    #   carry_vars = layers_mutables (RNG state threaded through iterations)
    #   scan_vars = layers_metrics (stacked along scan axis as ys output)
    #
    # The scan body receives broadcast_vars as an explicit positional arg,
    # NOT via closure. This makes the variable topology visible to JAX's
    # scan primitive, matching what Flax's nn.scan produces.

    def scan_body(carry, xs, broadcast_params):
      """V13 scan body with explicit variable groups.

      Args:
        carry: (loop_state, carry_vars) -- mutable state threaded through iterations
        xs: scan inputs (None for this pipeline)
        broadcast_params: layers_params passed as explicit positional arg,
                         NOT captured via closure. Mirrors Flax's broadcast_vars
                         in _partial_pack's scanned() function.
      """
      current_loop_state, current_layer_mutables = carry
      iteration = current_loop_state["loop_iteration"]

      # V13: broadcast_params is an explicit arg, apply stop_gradient
      # to prevent gradient accumulation through scan carry.
      # This mirrors Flax's freezing of broadcast variable collections
      # in _partial_pack (lift.py:168: variable_group[col_name] = freeze(collection))
      broadcast_params = jax.lax.stop_gradient(broadcast_params)

      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      current_layer_state = nnx.State.merge(broadcast_params, layers_metrics, advanced_mutables)
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      # V13: checkpoint wraps a function that receives broadcast_params as
      # explicit arg, NOT via closure. This ensures JAX's checkpoint sees
      # the same variable topology as the scan body.
      def checkpointed_scan_body(carry, xs, broadcast_params):
        return jax.checkpoint(
            scan_body,
            policy=self.get_pipeline_remat_policy(),
            prevent_cse=not self.config.scan_pipeline_iterations,
        )(carry, xs, broadcast_params)
      body_fn = checkpointed_scan_body
    else:
      body_fn = scan_body

    if self.config.scan_pipeline_iterations:
      # V13: Use functools.partial to bind broadcast_params as explicit arg
      # to the scan body. This is the NNX equivalent of Flax's pattern where
      # broadcast_vars are positional args to the scanned() function
      # (lift.py:1016: scanned(broadcast_vars, carry, scan_variable_groups, ...))
      bound_body = functools.partial(body_fn, broadcast_params=layers_params)
      (loop_state, final_layer_mutables), stacked_metrics = jax.lax.scan(
          bound_body, (loop_state, layers_mutables), None, length=total_iterations
      )
    else:
      current_carry = (loop_state, layers_mutables)
      metrics_history = []
      for _ in range(total_iterations):
        current_carry, step_metrics = body_fn(current_carry, None, layers_params)
...
class NNXCircularPipeline(NNXPipelineBase):
  """V13 NNX circular pipeline with Flax _partial_pack variable-group pattern.

  Reproduces Flax's _partial_pack mechanism where variables are split into
  explicit groups (broadcast/carry/scan_axes) and threaded as POSITIONAL
  ARGUMENTS through both scan and checkpoint boundaries.

  Key V13 changes from V10:
    - broadcast_vars (layers_params): passed as explicit positional arg to
      both outer_body and inner_body, NOT captured via closure
    - carry_vars (layers_mutables): threaded through scan carry as before
    - scan_vars (layers_metrics): returned as ys from scan body
    - jax.checkpoint receives broadcast_params as explicit positional arg,
      ensuring the checkpoint boundary sees the same variable topology
      as Flax's nn.remat would produce via _partial_pack

  All V10 fixes retained:
    1. vmap returns only non-param state
    2. stop_gradient on mutables
    3. stop_gradient on broadcast params (w_curr)
    4. nnx.split outside checkpoint boundary
    5. No metric scatter-update

  BSW (Buffer Sliding Window) architecture unchanged from V10.
  """

  def get_main_vmap_func_for_iterations(self):
    """V13 override: vmap returns only non-param state to avoid stacking params across stages.
  ...
  def __call__(
    ...
    # ---- V13: Flax _partial_pack variable-group pattern ----
    #
    # Flax's _partial_pack (lift.py:125) splits scope variables into groups:
    #   broadcast_vars = frozen params (no gradient through carry)
    #   carry_vars = mutable state (threaded through iterations)
    #   scan_vars = metrics (stacked along scan axis)
    #
    # The scan body receives these as EXPLICIT positional args:
    #   scanned(broadcast_vars, carry, scan_variable_groups, rng_groups, args)
    #
    # V13 adaptation for NNX circular pipeline:
    #   - broadcast_vars = layers_params (explicit arg, stop_gradient applied)
    #   - carry_vars = layers_mutables (in scan carry tuple)
    #   - scan_vars = layers_metrics (returned as ys)
    #   - BSW (w_curr, w_next) derived from broadcast_vars inside outer_body
    #
    # CRITICAL: Both inner_body AND its jax.checkpoint wrapper receive
    # broadcast_params as an explicit positional arg. This makes the
    # variable topology visible to both JAX primitives (scan + checkpoint),
    # exactly as Flax's _partial_pack achieves via scope_fn/repack_fn.

    num_microbatches = self.config.num_pipeline_microbatches

    def inner_body_fn(carry, xs, broadcast_params, bsw_tuple):
      """V13 inner scan body with explicit variable groups.

      Mirrors Flax's _partial_pack pattern where ALL variable groups are
      explicit positional args to the scan/checkpoint body, not closures.

      Args:
        carry: (loop_state, carry_vars) -- mutable state threaded through iterations
        xs: scan inputs (None for this pipeline)
        broadcast_params: layers_params as explicit positional arg (broadcast group).
                         stop_gradient applied to prevent carry gradient accumulation.
                         Mirrors Flax's freeze() on in-only variable collections
                         (_partial_pack lift.py:168).
        bsw_tuple: (w_curr, w_next) BSW dual-buffer, passed explicitly instead
                   of via bsw_ref closure.
      """
      current_loop_state, current_layer_mutables = carry
      # V10 Finding 2: stop_gradient on mutables to prevent gradient tape on RNG state
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, bsw_tuple, layers_graph, layers_metrics,
          advanced_mutables, positions, segment_ids, deterministic,
          model_mode, logical_partition_spec_stripped,
      )
      # V10 Finding 4: nnx.split is outside the checkpoint boundary here (inner_body_fn
      # itself is what gets wrapped by jax.checkpoint). The split result is returned
      # as scan output, not recomputed inside checkpoint. This avoids the Variable ID
      # mismatch between fwd/bwd that occurs when nnx.split runs inside checkpoint.
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      # V13: jax.checkpoint wraps a function that receives broadcast_params
      # and bsw_tuple as EXPLICIT positional args, not via closure.
      # This ensures the checkpoint boundary sees the same variable topology
      # as Flax's nn.remat would produce via _partial_pack's scope_fn/repack_fn.
      def inner_body(carry, xs, broadcast_params, bsw_tuple):
        return jax.checkpoint(
            inner_body_fn,
            policy=self.get_pipeline_remat_policy(),
            prevent_cse=not self.config.scan_pipeline_iterations,
        )(carry, xs, broadcast_params, bsw_tuple)
    else:
      inner_body = inner_body_fn

    def outer_body(carry, xs, broadcast_params):
      """V13 outer scan body with explicit broadcast_params.

      One repeat: prefetch w_next (1 all-gather), dual BSW.
      broadcast_params is an EXPLICIT positional arg, mirroring Flax's
      _partial_pack pattern where broadcast_vars are positional args to
      the scanned() function (lift.py:1016).

      Args:
        carry: (loop_state, carry_vars, w_curr) -- mutable scan state
        xs: scan inputs (None)
        broadcast_params: layers_params as explicit positional arg.
                         The full parameter tree from which BSW is derived.
      """
      current_loop_state, current_layer_mutables, w_curr = carry
      iteration = current_loop_state["loop_iteration"]

      # V10 Finding 3: stop_gradient on w_curr. Gradients should flow through the
      # custom BSW/all-gather path (w_next computed from broadcast_params), not through
      # w_curr carried in the outer scan. w_curr is just a cache of the previous
      # repeat's gathered weights -- its backward path duplicates gradient computation.
      w_curr = jax.lax.stop_gradient(w_curr)

      # V13: weight_prefetching uses broadcast_params (explicit arg) instead of
      # layers_params (closure). broadcast_params IS layers_params -- the difference
      # is that it's a positional arg to the scan body, making it visible to JAX's
      # scan primitive as a proper input rather than a captured variable.
      w_next = self.weight_prefetching(
          broadcast_params, physical_partition_spec_full, iteration
      )
      bsw_tuple = (w_curr, w_next)

      if self.config.scan_pipeline_iterations:
        # V13: bind broadcast_params and bsw_tuple as explicit args to inner_body
        # via functools.partial. This preserves the _partial_pack pattern through
        # the inner scan level: broadcast_params flows as a positional arg through
        # outer_body -> inner_body -> jax.checkpoint, never as a closure.
        bound_inner = functools.partial(inner_body, broadcast_params=broadcast_params, bsw_tuple=bsw_tuple)
        (new_loop_state, new_layer_mutables), inner_metrics = jax.lax.scan(
            bound_inner, (current_loop_state, current_layer_mutables),
        ...
        for _ in range(num_microbatches):
          inner_carry, step_metrics = inner_body(inner_carry, None, broadcast_params, bsw_tuple)
    ...
    if self.config.scan_pipeline_iterations:
      ...
      # V13: bind broadcast_params (layers_params) as explicit arg to outer_body
      # via functools.partial. This is the outermost level of the _partial_pack
      # pattern: layers_params flows as a positional arg to the scan body,
      # not captured via closure.
      bound_outer = functools.partial(outer_body, broadcast_params=layers_params)
      (loop_state, final_layer_mutables, final_w_curr), repeat_metrics = jax.lax.scan(
          bound_outer, (loop_state, layers_mutables, initial_w_curr), None,
      ...
      for _ in range(num_repeats):
        outer_carry, rep_metrics = outer_body(outer_carry, None, layers_params)
    ...
    if bubble_iterations > 0:
      # V13: bubble uses final_w_curr as both BSW slots (no more prefetch needed)
      # broadcast_params is still layers_params, passed explicitly
      bsw_bubble = (final_w_curr, final_w_curr)

      if self.config.scan_pipeline_iterations:
        bound_bubble = functools.partial(inner_body, broadcast_params=layers_params, bsw_tuple=bsw_bubble)
        (loop_state, final_layer_mutables), bubble_metrics = jax.lax.scan(
            bound_bubble, (loop_state, final_layer_mutables), None, length=bubble_iterations
        )
      else:
        bubble_carry = (loop_state, final_layer_mutables)
        bubble_metrics_list = []
        for _ in range(bubble_iterations):
          bubble_carry, bub_metrics = inner_body(bubble_carry, None, layers_params, bsw_bubble)
