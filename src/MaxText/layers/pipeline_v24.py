...
"""Pipeline V24: Wrap run_one_iteration in jax.named_call.

Variant targeting scan body equation count. Wraps the self.run_one_iteration()
call inside inner_body with jax.named_call to create a named_call primitive
that groups equations into a sub-computation, potentially affecting XLA's
buffer scheduling and reducing visible equations in the scan body trace.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      ...
      # V24: Wrap run_one_iteration in jax.named_call to group equations
      # into a sub-computation. This creates a named_call primitive that
      # may affect XLA's buffer scheduling by isolating the iteration's
      # equations from the scan body's top-level trace.
      @jax.named_call
      def _run_iteration(ls, bsw, graph, metrics, mutables, pos, seg, det, mode, lps):
        return self.run_one_iteration(
            ls, bsw, graph, metrics,
            mutables, pos, seg, det,
            mode, lps,
        )

      new_loop_state, new_layer_state = _run_iteration(
