...
"""Pipeline V21: Remove _stamp_at_current_trace to test residual forwarding.

_stamp_at_current_trace creates dynamic_slice_in_dim ops that break JAX's
residual forwarding (ad.py checks id(res)==id(input)). Removing tests impact.
"""
...
class NNXPipelineBase(nnx.Module):
  ...
  def _stamp_at_current_trace(weights):
    """V21: Identity passthrough — no dynamic_slice_in_dim.
    Tests whether removing the slice ops improves residual forwarding.
    """
    return weights
