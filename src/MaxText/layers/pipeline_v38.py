...
"""Pipeline V38: Zero-closure scan body (full variable lifting).

Eliminates ALL closure captures from scan body, making every dependency
an explicit argument via carry. Mirrors _partial_pack's effect of making
all variables explicit positional arguments at scan boundary.

Goal: num_consts=0 in scan → different jaxpr → different HLO scheduling.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V38: Zero-closure scan — all dependencies explicit in carry ----
    #
    # _partial_pack makes all variables explicit positional args.
    # We replicate this by putting EVERYTHING in the scan carry,
    # eliminating all closure captures (num_consts=0).
    num_microbatches = self.config.num_pipeline_microbatches
    set_remat = self.config.set_remat_policy_on_pipeline_iterations
    scan_iters = self.config.scan_pipeline_iterations
    remat_policy = self.get_pipeline_remat_policy() if set_remat else None

    # Pack ALL loop-invariant values into a context tuple.
    # These would normally be closure captures (num_consts).
    # By putting them in carry, they become explicit args (num_carry).
    context = (
        layers_graph,
        layers_metrics,
        positions,
    ...
    def inner_body_explicit(carry, _):
      """Inner body with ZERO closure captures. All deps from carry."""
      (current_loop_state, current_layer_mutables, bsw,
       ctx_params, ctx_graph, ctx_metrics, ctx_positions, ctx_segment_ids) = carry
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, bsw, ctx_graph, ctx_metrics,
          advanced_mutables, ctx_positions, ctx_segment_ids, deterministic,
      ...
      return (new_loop_state, new_layer_mutables, bsw,
              ctx_params, ctx_graph, ctx_metrics, ctx_positions, ctx_segment_ids), new_layer_metrics

    if set_remat:
      inner_body_explicit = jax.checkpoint(
          inner_body_explicit, policy=remat_policy,
          prevent_cse=not scan_iters,
      )

    def outer_body_explicit(carry, _):
      """Outer body with zero closure captures. All deps explicit."""
      (current_loop_state, current_layer_mutables, w_curr,
       ctx_params, ctx_graph, ctx_metrics, ctx_positions, ctx_segment_ids) = carry
      iteration = current_loop_state["loop_iteration"]

      w_next = self.weight_prefetching(
          ctx_params, physical_partition_spec_full, iteration
      )
      bsw = (w_curr, w_next)

      if scan_iters:
        inner_init = (current_loop_state, current_layer_mutables, bsw,
                      ctx_params, ctx_graph, ctx_metrics, ctx_positions, ctx_segment_ids)
        inner_final, inner_metrics = jax.lax.scan(
            inner_body_explicit, inner_init, None, length=num_microbatches,
        )
        new_loop_state = inner_final[0]
        new_layer_mutables = inner_final[1]
      else:
        inner_carry = (current_loop_state, current_layer_mutables, bsw,
                       ctx_params, ctx_graph, ctx_metrics, ctx_positions, ctx_segment_ids)
        inner_metrics_list = []
        for _ in range(num_microbatches):
          inner_carry, step_metrics = inner_body_explicit(inner_carry, None)
          inner_metrics_list.append(step_metrics)
        new_loop_state = inner_carry[0]
        new_layer_mutables = inner_carry[1]
        inner_metrics = (
            jax.tree.map(lambda *xs: jnp.stack(xs), *inner_metrics_list)
            if inner_metrics_list else ctx_metrics
        )

      return (new_loop_state, new_layer_mutables, w_next,
              ctx_params, ctx_graph, ctx_metrics, ctx_positions, ctx_segment_ids), inner_metrics
    ...
    outer_init = (loop_state, layers_mutables, initial_w_curr,
                  layers_params, *context)

    if scan_iters:
      unroll_repeats = num_repeats if not self.config.scan_pipeline_repeats else 1
      outer_final, repeat_metrics = jax.lax.scan(
          outer_body_explicit, outer_init, None,
          length=num_repeats, unroll=unroll_repeats,
      )
      loop_state = outer_final[0]
      final_layer_mutables = outer_final[1]
      final_w_curr = outer_final[2]
      repeat_metrics = jax.tree.map(
          lambda x: x.reshape((num_repeats * num_microbatches,) + x.shape[2:]),
          repeat_metrics,
      )
    else:
      outer_carry = outer_init
      repeat_metrics_list = []
      for _ in range(num_repeats):
        outer_carry, rep_metrics = outer_body_explicit(outer_carry, None)
        repeat_metrics_list.append(rep_metrics)
      loop_state = outer_carry[0]
      final_layer_mutables = outer_carry[1]
      final_w_curr = outer_carry[2]
      repeat_metrics = (
    ...
    if bubble_iterations > 0:
      ...
      if scan_iters:
        bubble_final, bubble_metrics = jax.lax.scan(
            inner_body_explicit, bubble_init, None, length=bubble_iterations
        )
        loop_state = bubble_final[0]
        final_layer_mutables = bubble_final[1]
      else:
        bubble_carry = bubble_init
        bubble_metrics_list = []
        for _ in range(bubble_iterations):
          bubble_carry, bub_metrics = inner_body_explicit(bubble_carry, None)
          bubble_metrics_list.append(bub_metrics)
        loop_state = bubble_carry[0]
        final_layer_mutables = bubble_carry[1]
        bubble_metrics = (
