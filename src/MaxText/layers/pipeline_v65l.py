...
V65l: V65 base with Level 2 and Level 3 custom_vjp REMOVED.
...
Difference from V65:
  - Level 1 custom_vjp (_run_iter_local) KEPT: per-microbatch remat barrier.
  - Level 2 custom_vjp (_run_microbatches with jax.vjp(scan_fn)) REMOVED.
  - Level 3 custom_vjp (_execute_stage with linear_transpose) REMOVED.
  - Weight prefetching called DIRECTLY (auto-diff, no custom_vjp wrapper).
  - Inner microbatch scan is raw jax.lax.scan with Level 1 inside body.
  - BSW constructed inline, passed to inner scan via explicit arg.
...
------------------------------------
  ...
  - V65l: removes Level 2 (d+g gradient accumulation) and Level 3
    (linear_transpose for weight prefetching backward). Simpler body,
    relies on JAX AD for all gradient flow through prefetching and scan.
"""
...
# ---------------------------------------------------------------------------
# V65l utilities: NNX state <-> flat Linen array collections
# ---------------------------------------------------------------------------
...
# For V65l, we import from pipeline.py and only override NNXCircularPipeline.
# ---------------------------------------------------------------------------
...
class NNXCircularPipeline(NNXPipelineBase):
  """V65l: NNX circular pipeline using inline lifted scan with scope isolation.
  ...
  Difference from V65: Level 2 and Level 3 custom_vjp REMOVED.
  Only Level 1 (per-microbatch remat) remains.
  ...
  def __call__(
    ...
    """V65l: Uses flax.core.lift.scan + lift.checkpoint with all state in scope.

    Architecture:
      lift.scan(REPEATS) -> lift.checkpoint(stage_fn containing jax.lax.scan(MICROBATCHES))

    Level 2 and Level 3 custom_vjp REMOVED. Only Level 1 (per-microbatch remat) remains.
    Weight prefetching is auto-differentiated (no custom_vjp wrapper).
    Inner microbatch scan is raw jax.lax.scan with Level 1 custom_vjp per step.
    ...
    #
    # REMOVED from closure (vs V65):
    #   - bsw_ref: no longer needed (BSW constructed inline).
    #   - bubble_iterations: not needed inside stage fn.
    #
    ...
    def _stage_fn_for_scope(scope, carry):
      ...
      V65l: NO Level 2/3 custom_vjp. Direct computation with Level 1 only.
      ...
      # ---- Weight prefetching (DIRECT -- no custom_vjp wrapper) ----
      # In V65, this was inside Level 3 custom_vjp (_execute_stage_fwd).
      # In V65l, we call it directly and let JAX AD handle the backward.
      ...
      # This is the ONLY custom_vjp in V65l.
      #
      # CLOSURE AUDIT for _run_iter_local:
      #   - ctx (_PipelineContext): NOT a JAX pytree -- SAFE.
      ...
      # ---- Inner microbatch scan (DIRECT -- no Level 2 custom_vjp) ----
      # In V65, this was wrapped in _run_microbatches custom_vjp with
      # explicit d+g gradient accumulation.
      # In V65l, we use raw jax.lax.scan and let JAX AD handle gradient flow.

      def inner_body(inner_carry, _):
        """Single microbatch step with Level 1 custom_vjp.

        CLOSURE AUDIT:
          - _run_iter_local: locally defined function with Level 1 custom_vjp.
            Captures only Python objects + scope-read arrays. SAFE.
          - bsw: tuple of JAX arrays from current trace level (w_curr from carry,
            w_next from weight_prefetching). At CURRENT trace level. SAFE.
          - local_layers_metrics: reconstructed from scope reads. At CURRENT trace level. SAFE.
          - local_positions, local_segment_ids: from scope reads. At CURRENT trace level. SAFE.
    ...
    if bubble_iterations > 0:
      ...
      #   - bsw_bubble: tuple of (final_w_curr, final_w_curr) -- same trace level -- SAFE.
