...
V65 fix6: V65 fix2 + jax.linear_transpose for weight gradients.
Replaces auto-diff through all_gather with analytical transpose (reduce-scatter).
This avoids storing gathered weight intermediates in vjp residuals.
Expected: ~25-27 GB (down from 29.4 GB).
...
Fix6 change (vs fix2)
---------------------
  Level 3 ``_execute_stage_fwd`` now computes ``jax.linear_transpose`` of
  ``weight_prefetching`` and saves it as a residual alongside ``scan_vjp_fn``.
  ``_execute_stage_bwd`` uses the transposed function to compute weight
  gradients via reduce-scatter instead of auto-diffing through the all-gather.
  This matches Linen's ``execute_pipeline_stage_pure_bwd`` pattern exactly.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  """V65 fix6: NNX circular pipeline with jax.linear_transpose for weight gradients.
  ...
  Key change vs fix2: Level 3 uses jax.linear_transpose(weight_prefetching) in
  backward pass instead of auto-diff through all_gather. This avoids storing
  gathered weight intermediates in vjp residuals.

  Memory target: ~25-27 GB (down from fix2's 29.4 GB).
  """
  ...
  def __call__(
    ...
    """V65 fix6: Uses flax.core.lift.scan + lift.checkpoint with all state in scope.
    ...
    Fix6: Level 3 uses jax.linear_transpose(weight_prefetching) for backward
    instead of auto-diff through all_gather.
    """
    ...
    def _stage_fn_for_scope(scope, carry):
      ...
      def _run_microbatches_fwd(loop_state_arg, bsw_arg, muts_arg,
        ...
        def scan_fn(ls_arg, b_arg):
          # inner_body MUST use b_arg (positional arg), NOT bsw_arg (closure).
      ...
      #
      # Fix6 KEY CHANGE: Instead of auto-diffing through weight_prefetching
      # (which stores the all-gather intermediates in vjp residuals), we:
      # 1. In fwd: compute weight_prefetching_t = jax.linear_transpose(weight_prefetching)
      # 2. In bwd: use weight_prefetching_t(g_w_next) to get g_params analytically
      # This matches Linen's execute_pipeline_stage_pure_bwd exactly.
      ...
      def _execute_stage_fwd(loop_state_arg, w_curr_arg, params_arg,
                              muts_arg, met_arg, pos_arg, seg_arg):
        # Compute w_next via weight_prefetching (forward only -- no vjp through this)
        ...
        # Compute jax.linear_transpose of weight_prefetching, partialed over
        # physical_partition_spec and loop_iteration. Only params_arg remains
        # as the differentiable input.
        # This is the analytical dual: all-gather -> reduce-scatter.
        ...
        # vjp over the microbatch scan only (NOT through weight_prefetching)
        def _scan_only(ls, bsw_arg):
          (final_ls, final_muts, _bsw_out), metrics = _run_microbatches(
              ls, bsw_arg, muts_arg, met_arg, pos_arg, seg_arg
          )
          return (final_ls, final_muts), metrics

        (scan_result, metrics), scan_vjp_fn = jax.vjp(
            _scan_only, loop_state_arg, bsw,
        )
        final_ls, final_muts = scan_result

        # Return: outputs + residuals (scan_vjp_fn, weight_prefetching_t, w_next)
      ...
      def _execute_stage_bwd(residuals, g_out):
        ...
        # Initialize zero cotangents for w_curr (consumed in forward, no direct output grad)
        g_w_curr = jax.tree.map(jnp.zeros_like, g_w_next)
        g_bsw = (g_w_curr, g_w_next)

        # Backprop through microbatch scan only
        d_ls, d_bsw = scan_vjp_fn(((g_ls, g_muts), g_metrics))

        # Split BSW gradients
        d_w_curr, d_w_next = d_bsw

        # Apply linear transpose of weight_prefetching to compute param gradients
        # This executes reduce-scatter (dual of all-gather) analytically
        (d_params,) = weight_prefetching_t(d_w_next)

        return d_ls, d_w_curr, d_params, None, None, None, None
