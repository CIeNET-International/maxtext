...
V48 -- Solution 5: Manual loop splitting.
====================================================================
Problem: NNX circular pipeline uses 29.9 GB compile-time memory vs Linen's
23.3 GB (6.6 GB gap). Linen emits 8 HLO ``while`` loops, NNX only 4. Each
``jax.lax.scan`` compiles to one real HLO ``while`` loop, and more while
loops give XLA more scheduling boundaries -> better buffer packing.

V48 mechanism: split each scan into TWO ``jax.lax.scan`` calls, each
processing roughly half the iterations, threading the carry between them.

  * INNER microbatch scan (inside ``outer_body``): split into scan_a over
    ``half = num_microbatches // 2`` steps and scan_b over the remaining
    ``num_microbatches - half`` steps. carry_b init = carry_a final.
  * OUTER repeat scan: split into scan over ``num_repeats // 2`` repeats
    and a second scan over the remainder, threading the
    ``(loop_state, layer_mutables, w_curr)`` carry.

This converts the baseline's 2 logical scans (inner + outer) -> 4 logical
scans, i.e. 8 HLO while loops (forward + backward each), matching Linen.

Correctness: the second scan's init carry is bound to the first scan's
final carry, so ``loop_state["loop_iteration"]`` keeps incrementing
monotonically across the split -- gradients and BSW selection are
unaffected. Metrics are stacked along axis 0 by each scan and
``jnp.concatenate``'d in iteration order before the final reshape to
``(num_repeats * num_microbatches, ...)``.

Edge cases:
  * ``num_microbatches`` odd  -> split ``half`` / ``num_microbatches-half``.
  * ``num_microbatches < 2``  -> fall back to a single inner scan.
  * ``num_repeats < 2``       -> fall back to a single outer scan.
  * unrolled path (``scan_pipeline_iterations=False``) unchanged.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        # V48: split the inner microbatch scan into TWO jax.lax.scan calls.
        # Each scan compiles to one HLO while loop; threading the carry keeps
        # loop_iteration incrementing correctly across the split.
        inner_carry = (current_loop_state, current_layer_mutables)
        if num_microbatches >= 2:
          half = num_microbatches // 2
          inner_carry, inner_metrics_a = jax.lax.scan(
              inner_body,
              inner_carry,
              None,
              length=half,
          )
          (new_loop_state, new_layer_mutables), inner_metrics_b = jax.lax.scan(
              inner_body,
              inner_carry,
              None,
              length=num_microbatches - half,
          )
          # Stacked along axis 0 by each scan; concatenate in iteration order.
          ...
          # Fall back to a single scan when there is < 2 microbatches.
    ...
    if self.config.scan_pipeline_iterations:
      # V48: split the outer repeat scan into TWO jax.lax.scan calls, mirroring
      # the inner split. The second scan's init carry is the first's final
      # carry, so loop_iteration / w_curr thread correctly across the split.
      outer_carry = (loop_state, layers_mutables, initial_w_curr)
      if num_repeats >= 2:
        repeats_a = num_repeats // 2
        repeats_b = num_repeats - repeats_a
        unroll_a = repeats_a if not self.config.scan_pipeline_repeats else 1
        unroll_b = repeats_b if not self.config.scan_pipeline_repeats else 1
        outer_carry, repeat_metrics_a = jax.lax.scan(
            outer_body,
            outer_carry,
            None,
            length=repeats_a,
        ...
        (loop_state, final_layer_mutables, final_w_curr), repeat_metrics_b = jax.lax.scan(
            outer_body,
            outer_carry,
            None,
            length=repeats_b,
        ...
        # Each scan stacks metrics as (repeats_x, num_microbatches, ...);
        # concatenate along axis 0 (repeats) to restore iteration order.
        ...
        # Fall back to a single scan when there is < 2 repeats.
