...
"""Pipeline V4: Inner axes_scan + Outer jax.lax.scan.
...
Variant of pipeline.py with the following changes to NNXCircularPipeline.__call__:
  ...
  2. Keep outer scan as jax.lax.scan (unchanged)
  3. Change INNER microbatch scan to use axes_scan with BSW as broadcast
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V4: Inner body takes BSW as explicit broadcast arg for axes_scan
    def inner_body_with_broadcast(broadcast_bsw, carry, _):
      """Inner iteration with BSW as explicit broadcast (not closure/carry).
      broadcast_bsw is passed through unchanged by axes_scan."""
      current_loop_state, current_layer_mutables = carry
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, broadcast_bsw, layers_graph, layers_metrics,
      ...
      return broadcast_bsw, (new_loop_state, new_layer_mutables), new_layer_metrics

    if self.config.set_remat_policy_on_pipeline_iterations:
      inner_body_with_broadcast = jax.checkpoint(
          inner_body_with_broadcast, policy=self.get_pipeline_remat_policy(),
          prevent_cse=not self.config.scan_pipeline_iterations,
      )

    # V4: Also keep a closure-based inner_body for bubble iterations (no axes_scan)
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        # V4: Use axes_scan for inner microbatch loop with BSW as broadcast
        inner_scanned = axes_scan(
            inner_body_with_broadcast,
            in_axes=(scan_broadcast, 0, 0),
            out_axes=(scan_broadcast, 0, 0),
            length=num_microbatches,
        )
        _, (new_loop_state, new_layer_mutables), inner_metrics = inner_scanned(
            bsw, (current_loop_state, current_layer_mutables), None,
        )
      else:
        bsw_ref[0] = bsw
