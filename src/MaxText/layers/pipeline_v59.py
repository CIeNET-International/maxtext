...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V59: custom_vjp memory barrier + outer remat (scan inside) ----
    #
    # Two mechanisms:
    # 1. Inner: @custom_vjp per microbatch with jax.remat — MEMORY BARRIER.
    #    Scan stores only vjp closure (not full activations).
    #    Matches Linen's run_single_microbatch_custom.
    # 2. Outer: jax.checkpoint wrapping repeat_step (CONTAINS jax.lax.scan)
    #    — separate recomputation while loop during backward.
    #    Matches Linen's nn.remat(stage_fn).
    #
    # NO double jax.checkpoint. Inner = custom_vjp. Outer = checkpoint.
    num_microbatches = self.config.num_pipeline_microbatches
    remat_policy = self.get_pipeline_remat_policy()

    # --- Per-microbatch custom_vjp: memory barrier ---
    @jax.custom_vjp
    ...
    # --- Outer body: weight prefetching OUTSIDE checkpoint, scan INSIDE ---
    def outer_body(carry, _):
      current_loop_state, current_layer_mutables, w_curr = carry
      iteration = current_loop_state["loop_iteration"]

      # Weight prefetching OUTSIDE checkpoint — avoids tracer leak
      w_next = self.weight_prefetching(
          layers_params, physical_partition_spec_full, iteration
      )
      bsw = (w_curr, w_next)

      # Checkpoint wraps the scan-containing function.
      # BSW passed as explicit arg (not bsw_ref mutable closure).
      # Inner body defined inside repeat_step to capture bsw_arg from closure.
      def repeat_step(ls, muts, bsw_arg):
        def _inner_body(carry, _):
          c_ls, c_muts = carry
          c_muts = jax.lax.stop_gradient(c_muts)
          new_ls, new_layer_state = run_iter_custom(c_ls, bsw_arg, c_muts)
        ...
        if self.config.scan_pipeline_iterations:
          (new_ls, new_muts), inner_metrics = jax.lax.scan(
          ...
          for _ in range(num_microbatches):
            inner_carry, step_metrics = _inner_body(inner_carry, None)
            inner_metrics_list.append(step_metrics)
          new_ls, new_muts = inner_carry
          inner_metrics = (
              jax.tree.map(lambda *xs: jnp.stack(xs), *inner_metrics_list)
              if inner_metrics_list else layers_metrics
          )
        return new_ls, new_muts, inner_metrics

      if self.config.set_remat_policy_on_pipeline_iterations:
        repeat_step = jax.checkpoint(
            repeat_step, policy=remat_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )

      new_ls, new_muts, inner_metrics = repeat_step(
    ...
    if bubble_iterations > 0:
      ...
      def bubble_step(ls, muts, bsw_arg):
        ...
        if self.config.scan_pipeline_iterations:
          return jax.lax.scan(
              _bubble_inner, (ls, muts), None, length=bubble_iterations
          )
        else:
          carry = (ls, muts)
          metrics_list = []
          for _ in range(bubble_iterations):
            carry, m = _bubble_inner(carry, None)
      ...
      if self.config.set_remat_policy_on_pipeline_iterations:
        bubble_step = jax.checkpoint(
            bubble_step, policy=remat_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )

      (loop_state, final_layer_mutables), bubble_metrics = bubble_step(
          loop_state, final_layer_mutables, bsw_bubble
      )
