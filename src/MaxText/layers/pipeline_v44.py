...
"""Pipeline V44: nn.scan(nn.remat(...)) with REAL Linen params + variable_broadcast.

Validates the claim: 6.6 GB gap is from _partial_pack scope isolation.

RepeatStage Linen module registers pipeline weights as self.variable("params", ...).
nn.scan with variable_broadcast=["params"] triggers _partial_pack to:
  1. Extract params from scope
  2. Freeze them (immutable)
  3. Create inner scope with parent=None
  4. Pass frozen params into isolated scope

This should produce 8 HLO while loops (vs baseline's 4) and ~23.3 GB.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V44: nn.scan(nn.remat(RepeatStage)) with REAL Linen params ----
    #
    # _partial_pack scope isolation requires ACTUAL Linen variables.
    # RepeatStage registers pipeline weights as self.variable("params", ...).
    # nn.scan with variable_broadcast=["params"] triggers _partial_pack to:
    #   1. Extract "params" collection from scope
    #   2. Freeze them (immutable FrozenDict)
    #   3. Create inner scope with parent=None
    #   4. Pass frozen params into isolated scope
    # This produces 8 HLO while loops (vs baseline's 4).
    num_microbatches = self.config.num_pipeline_microbatches
    num_repeats = self.config.num_pipeline_repeats
    bsw_ref = [None]
    pipeline_self = self

    # Strip nnx.Variable wrappers → raw arrays for Linen variable storage
    ...
    # Linen module for one repeat — holds pipeline weights as REAL Linen params
    class RepeatStage(nn.Module):
      """One pipeline repeat. Params registered as Linen variables for _partial_pack."""

      @nn.compact
      def __call__(self_linen, carry):
        # Register pipeline weights as Linen "params" collection variable.
        # variable_broadcast=["params"] will make _partial_pack:
        #   - freeze these (immutable)
        #   - NOT include in scan carry
        #   - pass through isolated scope (parent=None)
        stored_params = self_linen.variable(
            "params", "pipeline_weights",
            lambda: layers_params_raw,
        )

        current_loop_state, current_layer_mutables, w_curr = carry
        iteration = current_loop_state["loop_iteration"]

        # Use stored_params.value (from Linen scope, frozen by _partial_pack)
        w_next = pipeline_self.weight_prefetching(
            stored_params.value, physical_partition_spec_full, iteration
        )
        bsw = (w_curr, w_next)

        # Inner microbatch scan — uses stored_params from scope, not closure
        def inner_body(inner_carry, _):
          cur_loop_state, cur_layer_mutables = inner_carry
          cur_layer_mutables = jax.lax.stop_gradient(cur_layer_mutables)
          iter_val = cur_loop_state["loop_iteration"]
          advanced_mutables = _advance_rng_state(cur_layer_mutables, iter_val)
          new_loop_state, new_layer_state = pipeline_self.run_one_iteration(
              cur_loop_state, bsw, layers_graph, layers_metrics,
    ...
    scan_vars = scan_stage.init(jax.random.key(0), initial_carry)
    # Inject actual NNX weights into the Linen "params" collection
    scan_vars = {
        **scan_vars,
        "params": {"pipeline_weights": layers_params_raw},
    }
    ...
    if bubble_iterations > 0:
      ...
      def bubble_body(carry, _):
        ...
        advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
        new_loop_state, new_layer_state = self.run_one_iteration(
            current_loop_state, bubble_bsw, layers_graph, layers_metrics,
            advanced_mutables, positions, segment_ids, deterministic,
            model_mode, logical_partition_spec_stripped,
        )
        _, _, new_layer_metrics, new_layer_mutables = nnx.split(
