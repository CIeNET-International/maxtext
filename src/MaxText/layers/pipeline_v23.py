...
"""Pipeline V23: 3-level custom_vjp pattern ported from Linen pipeline_utils.py.

This is a clean re-implementation of the Linen circular pipeline's gradient
mechanism (pipeline_utils.py lines 166-361) adapted for NNX state management.

Architecture (mirrors Linen exactly):

  Level 1 - run_single_microbatch:
    custom_vjp wrapping ONE microbatch step.
    Forward: jax.remat(run_one_iteration) + jax.vjp -> captures vjp_fun
    Backward: vjp_fun(g_out) -> (d_lightweight, d_bsw)

  Level 2 - run_pipeline_microbatches:
    custom_vjp wrapping jax.lax.scan over Level 1.
    Forward: jax.vjp(lambda: scan(Level1), lightweight, bsw) -> scan_vjp
    Backward: scan_vjp(g_lightweight) -> d+g accumulation on BSW gradients

  Level 3 - execute_pipeline_stage:
    custom_vjp wrapping ONE repeat.
    Forward: weight_prefetching -> BSW, jax.vjp(Level2) -> captures scan_vjp
             + jax.linear_transpose(weight_prefetching) for reduce-scatter
    Backward: zero-init g_w_curr, scan_vjp -> g_bsw, weight_prefetching_t -> g_params

Key differences from pipeline.py (current NNX):
  - NO bsw_ref mutable list: BSW is an explicit function argument at every level
  - NO stop_gradient on w_curr: gradient flows naturally through Level 3's backward
  - d+g accumulation in Level 2: prevents BSW gradient stacking across microbatches
  - jax.remat in Level 1: controls per-step activation memory
  - jax.linear_transpose in Level 3: reduce-scatter for param gradients
  - ys=None for ALL scans: metrics NOT returned as scan output (like Linen)
"""
...
class NNXPipeline(NNXPipelineBase):
  ...
  def run_one_iteration(
    ...
    """Executes the logic for a single microbatch iteration."""
...
class NNXCircularPipeline(NNXPipelineBase):
  """NNX circular pipeline V23: 3-level custom_vjp matching Linen pipeline_utils.py.

  Implements the exact gradient mechanism from pipeline_utils.py:
    Level 1: run_single_microbatch - custom_vjp wrapping one microbatch step with remat
    Level 2: run_pipeline_microbatches - custom_vjp wrapping scan over Level 1 with d+g
    Level 3: execute_pipeline_stage - custom_vjp wrapping one repeat with weight prefetch

  Key differences from the bsw_ref closure approach:
    - BSW is an explicit function argument, not a closure reference
    - jax.linear_transpose for weight prefetching backward (reduce-scatter)
    - d+g accumulation in Level 2 prevents BSW gradient stacking
    - ys=None for all scans (metrics collected separately)
  """
  ...
  def weight_prefetching(self, weights_state, physical_partition_spec, loop_iteration):
    ...
    matching the Linen sliding-window pattern.
    """
  ...
  def fetch_active_stage_weights(self, bsw, loop_iteration, physical_partition_spec=None):
    """Fetches actively prefetched weights from the BSW."""
  ...
  def get_current_weights_from_bsw(self, bsw, loop_iteration, physical_partition_spec):
    ...
    if bsw_pps is not None:
      ...
      # No partition spec: select via tree_map (no shard_map needed)
      def select_weights_from_bsw(bsw_inner, repeat_id):
        return jax.tree.map(
  ...
  def __call__(
    ...
    """V23: 3-level custom_vjp pipeline execution matching Linen pipeline_utils.py.

    Architecture:
      Level 1 (run_single_microbatch): remat + vjp per microbatch step
      Level 2 (run_pipeline_microbatches): scan over Level 1 + d+g accumulation
      Level 3 (execute_pipeline_stage): weight prefetch + linear_transpose per repeat

    BSW is an explicit function argument at every level -- NO closure ref.
    ys=None for all scans -- metrics collected at the end from module state.
    """
    inputs = inputs.reshape(
        (
            self.config.num_pipeline_microbatches,
    ...
    # - Full spec (with circular_repeats axis) -> weight_prefetching uses it
    # - Stripped logical spec (circular_repeats removed) -> BSW consumption in run_one_iteration
    ...
    # layers_mutables catch-all should contain ONLY RngState variables
    assert all(
        isinstance(v, nnx.RngState)
        for v in jax.tree.leaves(layers_mutables, is_leaf=lambda x: isinstance(x, nnx.Variable))
        if isinstance(v, nnx.Variable)
    ...
    # ========================================================================
    # Capture self and closed-over variables for use inside custom_vjp
    # functions. custom_vjp functions must be pure (no self reference in
    # the differentiated args), so we close over these values.
    # ========================================================================
    pipeline_self = self
    ...
    # Level 1: run_single_microbatch
    #
    # Wraps ONE microbatch step in custom_vjp.
    # Forward: jax.remat(run_one_iteration) + jax.vjp -> captures vjp_fun
    # Backward: vjp_fun(g_out) -> (d_lightweight, d_bsw)
    #
    # lightweight_state = (loop_state, layer_mutables)
    # BSW is a separate argument -- NOT in the carry.
    #
    # Mirrors pipeline_utils.py:197-218 (run_single_microbatch_custom)
    # ========================================================================

    @jax.custom_vjp
    def run_single_microbatch(lightweight_state, bsw):
      """One microbatch step. lightweight_state = (loop_state, mutables). BSW separate."""
      return run_single_microbatch_fwd(lightweight_state, bsw)[0]

    def run_single_microbatch_fwd(lightweight_state, bsw):
      loop_state_inner, layer_mutables_inner = lightweight_state
      # Stop gradient on mutables -- they are RNG state, not trainable
      layer_mutables_inner = jax.lax.stop_gradient(layer_mutables_inner)
      iteration = loop_state_inner["loop_iteration"]
      advanced_mutables = _advance_rng_state(layer_mutables_inner, iteration)

      def _run(ls_and_mut, b):
        ls, mut = ls_and_mut
        new_ls, new_layer_state = pipeline_self.run_one_iteration(
            ls, b, layers_graph, layers_metrics,
            mut, positions, segment_ids, deterministic,
            model_mode, logical_partition_spec_stripped,
        )
        _, _, _new_metrics, new_mutables = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        # Return only loop_state and mutables -- metrics are NOT differentiated
        # and will be collected at the end from final module state.
        return (new_ls, new_mutables)

      # Rematerialize the inner step to save activation memory
      _run_remat = jax.remat(_run, policy=pipeline_self.get_pipeline_remat_policy())
      out, vjp_fun = jax.vjp(_run_remat, (loop_state_inner, advanced_mutables), bsw)
      return out, vjp_fun

    def run_single_microbatch_bwd(vjp_fun, g_out):
      d_lightweight, d_bsw = vjp_fun(g_out)
      return d_lightweight, d_bsw

    run_single_microbatch.defvjp(run_single_microbatch_fwd, run_single_microbatch_bwd)

    # ========================================================================
    # Level 2: run_pipeline_microbatches
    #
    # Wraps jax.lax.scan over Level 1 in custom_vjp.
    # Forward: jax.vjp(lambda: scan(Level1), loop_state_and_mutables, bsw)
    # Backward: d+g accumulation on BSW gradients.
    #
    # KEY: BSW is a function argument to jax.vjp, NOT in the scan carry.
    # This prevents BSW from being stacked across scan iterations.
    #
    # Mirrors pipeline_utils.py:220-248 (run_pipeline_microbatches_custom)
    # ========================================================================

    @jax.custom_vjp
    def run_pipeline_microbatches(loop_state_and_mutables, bsw):
      """Scan over microbatches with BSW as argument (not carry)."""
      return run_pipeline_microbatches_fwd(loop_state_and_mutables, bsw)[0]

    def run_pipeline_microbatches_fwd(loop_state_and_mutables, bsw):
      final_lightweight, scan_vjp_fun = jax.vjp(
          lambda l, b: jax.lax.scan(
              lambda carry, _: (run_single_microbatch(carry, b), None),
              l,
              None,
    ...
    def run_pipeline_microbatches_bwd(scan_vjp_fun, g_final_state):
      g_lightweight, g_bsw = g_final_state
      d_init_lightweight, d_init_bsw = scan_vjp_fun(g_lightweight)

      # d+g accumulation: add incoming BSW gradient to the computed one.
      # This prevents BSW gradient stacking across microbatch iterations.
    ...
    run_pipeline_microbatches.defvjp(run_pipeline_microbatches_fwd, run_pipeline_microbatches_bwd)

    # ========================================================================
    # Level 3: execute_pipeline_stage
    #
    # Wraps ONE repeat in custom_vjp.
    # Forward: weight_prefetching -> w_next, construct BSW = (w_curr, w_next),
    #          jax.vjp(Level2) -> scan_vjp, jax.linear_transpose(weight_prefetching)
    # Backward: zero-init g_w_curr, run scan_vjp -> g_bsw,
    #           weight_prefetching_t(g_w_next) -> g_pipeline_weights
    #
    # Mirrors pipeline_utils.py:306-355 (execute_pipeline_stage_pure)
    # ========================================================================

    @jax.custom_vjp
    def execute_pipeline_stage(loop_state_and_mutables, w_curr, pipeline_weights):
      """One repeat: prefetch, scan microbatches, advance w_next."""
      return execute_pipeline_stage_fwd(loop_state_and_mutables, w_curr, pipeline_weights)[0]

    def execute_pipeline_stage_fwd(loop_state_and_mutables, w_curr, pipeline_weights):
      loop_state_inner, _layer_mutables_inner = loop_state_and_mutables
      iteration = loop_state_inner["loop_iteration"]

      # Prefetch FSDP-sharded weights for the upcoming pipeline repeat
      w_next = pipeline_self.weight_prefetching(
          pipeline_weights,
          physical_partition_spec_full,
          iteration,
      )

      # Construct BSW: w_curr (active) + w_next (prefetched)
      bsw = (w_curr, w_next)

      # Bind arguments for linear_transpose (reduce-scatter of weight gradients)
      p_weight_prefetching = functools.partial(
          pipeline_self.weight_prefetching,
          physical_partition_spec=physical_partition_spec_full,
          loop_iteration=iteration,
      )

      # Since weight gathering (all-gather) is linear, we can derive its dual
      # (reduce-scatter) via jax.linear_transpose
      weight_prefetching_t = jax.linear_transpose(
          p_weight_prefetching,
          pipeline_weights,
      )

      # Execute forward pass of microbatches and generate VJP
      (final_lightweight, _bsw_out), scan_microbatches_vjp = jax.vjp(
          run_pipeline_microbatches,
          loop_state_and_mutables,
          bsw,
      )

      # Discard old weights (w_curr) and advance w_next as the new current
      return (final_lightweight, w_next), (scan_microbatches_vjp, weight_prefetching_t)

    def execute_pipeline_stage_bwd(residuals, g_outputs):
      g_lightweight, g_w_next = g_outputs
      scan_microbatches_vjp, weight_prefetching_t = residuals

      # Initialize zero cotangents for w_curr (consumed in forward pass)
      g_w_curr = jax.tree.map(jnp.zeros_like, g_w_next)
      g_bsw = (g_w_curr, g_w_next)

      # Backpropagate through the microbatch scan
      g_loop_state_and_mutables, g_bsw_out = scan_microbatches_vjp(
          (g_lightweight, g_bsw)
      )

      # Apply linear transpose of weight prefetch (reduce-scatter)
      g_w_curr_out, g_w_next_out = g_bsw_out
      (g_pipeline_weights,) = weight_prefetching_t(g_w_next_out)

      # Return gradients for: loop_state_and_mutables, w_curr, pipeline_weights
      return g_loop_state_and_mutables, g_w_curr_out, g_pipeline_weights

    execute_pipeline_stage.defvjp(execute_pipeline_stage_fwd, execute_pipeline_stage_bwd)

    # ========================================================================
    # Outer scan (repeats): scan over execute_pipeline_stage
    # ys=None like Linen -- metrics NOT returned as scan output
    # ========================================================================

    def outer_body(carry, _):
      """One repeat: execute Level 3, advance w_next to w_curr."""
      loop_state_outer, layer_mutables_outer, w_curr_outer = carry

      (new_ls_and_mut, w_next_out) = execute_pipeline_stage(
          (loop_state_outer, layer_mutables_outer),
          w_curr_outer,
          layers_params,
      )

      new_ls, new_mut = new_ls_and_mut

      # ys=None -- no scan outputs, matching Linen pattern
      return (new_ls, new_mut, w_next_out), None
    ...
    #
    # During bubble, BSW is static (w_curr, w_curr) -- no weight prefetching.
    # Use simple scan with jax.checkpoint (no custom_vjp needed).
    # ========================================================================
    if bubble_iterations > 0:
      ...
      def bubble_body(carry, _):
        ls, mut = carry
        mut = jax.lax.stop_gradient(mut)
        iteration = ls["loop_iteration"]
        advanced_mut = _advance_rng_state(mut, iteration)
        new_ls, new_layer_state = self.run_one_iteration(
            ls, bubble_bsw, layers_graph, layers_metrics,
            advanced_mut, positions, segment_ids, deterministic,
            model_mode, logical_partition_spec_stripped,
        )
        _, _, _bub_metrics, new_mut = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        # ys=None -- no scan outputs for bubble
        return (new_ls, new_mut), None
      ...
      if self.config.scan_pipeline_iterations:
        ...
        for _ in range(bubble_iterations):
          bubble_carry, _ = bubble_body(bubble_carry, None)
        loop_state, final_layer_mutables = bubble_carry

    # ========================================================================
    # Metrics collection: since ys=None for all scans, we do NOT have
    # per-step metrics stacked. Instead, we take the final metrics from
    # the module state after all iterations complete.
    #
    # For V23, we use the last available metrics (from the final run_one_iteration
    # call) as a representative sample. This matches the Linen pattern where
    # metrics are collected via variable_axes in nn.scan (which we cannot
    # replicate exactly in pure JAX scan).
    #
    # To produce stacked metrics matching the original shape expected by
    # nnx.update, we broadcast the final metrics across the total iteration count.
    # ========================================================================
    total_iterations = num_repeats * num_microbatches + bubble_iterations

    # Get the last metrics from final_layer_mutables state
    # The layers_metrics from the initial split serve as the structural template
    def _broadcast_metrics(m):
      """Broadcast a metric leaf to have a leading dimension of total_iterations."""
      if m is None:
        return None
      if hasattr(m, "shape"):
        return jnp.broadcast_to(m, (total_iterations,) + m.shape)
      return m

    stacked_metrics = jax.tree.map(_broadcast_metrics, layers_metrics)
