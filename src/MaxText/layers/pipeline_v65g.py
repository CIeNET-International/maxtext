...
V65g: NNX circular pipeline with FULL UNROLL of the outer repeat scan.

Based on V65. Single change: ``unroll=num_repeats`` in ``flax.core.lift.scan``.

Hypothesis
----------
Full unrolling eliminates the XLA while-loop for the outer repeat scan.
Instead, JAX traces the scan body ``num_repeats`` times, producing separate
function calls per iteration (similar to V65c's 16 sequential closed_call
invocations). This may allow XLA to apply the same memory optimizations
that nn.scan achieves via its closed_call boundaries.

When ``unroll=num_repeats``, ``jax.lax.scan`` expands the loop body inline
rather than emitting a ``while`` HLO op. The ``_partial_pack`` scope
isolation from ``lift.scan`` still applies to each unrolled iteration.

Architecture (unchanged from V65)
----------------------------------
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    scanned_stage = flax_lift.scan(
        ...
        split_rngs={},
        length=num_repeats,
        unroll=num_repeats,  # FULL UNROLL -- eliminates the outer while loop
    )
