...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- custom_vjp: stop_gradient on w_curr carry to save ~0.9 GB ----
    #
    # Linen avoids storing w_curr intermediates in the outer scan by using a
    # custom_vjp (execute_pipeline_stage_pure in pipeline_utils.py) that:
    #   1. stop_gradients the w_curr carry path
    #   2. Routes g_w_next through weight_prefetching_t (linear transpose of
    #      all-gather = reduce-scatter) to correctly accumulate into layers_params
    #
    # This is the NNX equivalent. The custom_vjp wraps the ENTIRE outer_body
    # content (weight prefetching + inner scan) so that inner scan gradients
    # flow through it correctly.

    def _run_inner_scan(loop_state_and_mutables, bsw):
      """Execute inner scan (microbatch iterations) with explicit BSW arg."""
      current_loop_state, current_layer_mutables = loop_state_and_mutables
      inner_body = lambda carry, xs: inner_body_fn(carry, xs, bsw)
      if remat_policy is not None:
        inner_body = jax.checkpoint(
            inner_body, policy=remat_policy,
    ...
    @jax.custom_vjp
    def weight_step(loop_state_and_mutables, w_curr, layers_params_arg):
      """One repeat: prefetch weights, run inner scan. Custom VJP saves 0.9 GB."""
      return weight_step_fwd(loop_state_and_mutables, w_curr, layers_params_arg)[0]

    def weight_step_fwd(loop_state_and_mutables, w_curr, layers_params_arg):
      current_loop_state, current_layer_mutables = loop_state_and_mutables
      iteration = current_loop_state["loop_iteration"]

      # stop_gradient on w_curr -- prevents scan from storing it for backward
      w_curr_sg = jax.lax.stop_gradient(w_curr)

      w_next = self.weight_prefetching(
          layers_params_arg, physical_partition_spec_full, iteration
      )
      bsw = (w_curr_sg, w_next)

      # Capture linear_transpose for gradient compensation in backward
      ...
      # Run inner scan — BSW passed directly, no bsw_ref side effect
      (new_loop_state, new_layer_mutables), inner_metrics = _run_inner_scan(
          (current_loop_state, current_layer_mutables), bsw
      )

      result = (new_loop_state, new_layer_mutables), w_next, inner_metrics
      residuals = (weight_prefetching_t,)
      return result, residuals

    def weight_step_bwd(residuals, g):
      (weight_prefetching_t,) = residuals
      g_loop_state_and_mutables, g_w_next, _g_metrics = g

      # g_w_next here is the gradient of the OUTPUT w_next.
      # This w_next becomes the NEXT repeat's w_curr.
      # The NEXT repeat's g_w_curr (from BSW usage) flows back as g_w_next to us.
      # Apply weight_prefetching_t (reduce-scatter) to convert g_w_next back to
      # g_layers_params in the FSDP-sharded parameter space.
      (g_layers_params,) = weight_prefetching_t(g_w_next)

      # g_w_curr = zero (we stopped it, gradient routed through g_layers_params)
      g_w_curr = jax.tree.map(jnp.zeros_like, g_w_next)

      return g_loop_state_and_mutables, g_w_curr, g_layers_params

    weight_step.defvjp(weight_step_fwd, weight_step_bwd)

    def outer_body(carry, _):
      """One repeat: custom_vjp-wrapped weight step + inner scan."""
      current_loop_state, current_layer_mutables, w_curr = carry

      (new_loop_state, new_layer_mutables), w_next, inner_metrics = weight_step(
    ...
    if bubble_iterations > 0:
      ...
      bubble_body = lambda carry, xs: inner_body_fn(carry, xs, bubble_bsw)
      if remat_policy is not None:
