...
V54: Manual _partial_pack — ALL state flattened to raw arrays, passed as
explicit positional arguments to jax.checkpoint inside scan body.
Mirrors Linen's _partial_pack scope_fn/repack_fn pattern without Linen Scopes.
Also tests axes_scan.scan with broadcast annotation (V54b path).
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V54: Manual _partial_pack — explicit positional args to checkpoint ----
    #
    # Mirrors Linen's _partial_pack: ALL state as explicit positional args at
    # the jax.checkpoint boundary (not closure-captured). _partial_pack creates
    # scope_fn(parent=None) making variables positional — we do the equivalent
    # by passing layers_params, layers_metrics, layers_graph, bsw as args.
    #
    # Two paths tested:
    #   V54a: jax.lax.scan + jax.checkpoint(explicit_args)
    #   V54b: axes_scan.scan(broadcast) + jax.checkpoint(explicit_args)
    # Toggle via config.scan_pipeline_use_axes_scan (default False = V54a)
    bsw_ref = [None]
    num_microbatches = self.config.num_pipeline_microbatches
    remat_policy = self.get_pipeline_remat_policy()
    use_axes_scan = getattr(self.config, 'scan_pipeline_use_axes_scan', False)

    # --- Checkpointed step: ALL state as explicit positional args ---
    # This is the key V54 change. Previous variants closure-captured
    # layers_params/metrics/graph/bsw. Here they're explicit args,
    # mirroring _partial_pack's scope_fn(parent=None) pattern.
    def _remat_step(params, metrics, graph, bsw, loop_state, mutables):
      iteration = loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          loop_state, bsw, graph, metrics,
      ...
      return new_loop_state, new_layer_metrics, new_layer_mutables

    if self.config.set_remat_policy_on_pipeline_iterations:
      _remat_step = jax.checkpoint(
          _remat_step, policy=remat_policy,
          prevent_cse=not self.config.scan_pipeline_iterations,
      )

    if use_axes_scan:
      # --- V54b: axes_scan.scan with broadcast annotation ---
      # broadcast_in = (params, metrics, graph)  [loop-invariant]
      # carry = (loop_state, mutables)            [loop-varying]
      # No scanned xs

      def inner_fn_axes(broadcast_in, carry, *_no_xs):
        params, metrics, graph = broadcast_in
        ls, muts = carry
        muts = jax.lax.stop_gradient(muts)
        bsw = bsw_ref[0]
        new_ls, new_mets, new_muts = _remat_step(
            params, metrics, graph, bsw, ls, muts,
        )
        return (params, metrics, graph), (new_ls, new_muts), new_mets

      inner_scanned = axes_scan_scan(
          inner_fn_axes,
          in_axes=(),
          out_axes=0,
      ...
      def run_inner_scan(loop_state_in, layer_mutables_in):
        _, (new_ls, new_muts), inner_mets = inner_scanned(
            (layers_params, layers_metrics, layers_graph),
            (loop_state_in, layer_mutables_in),
        )
        return (new_ls, new_muts), inner_mets

    else:
      # --- V54a: jax.lax.scan with explicit args to checkpoint ---
      def inner_body(carry, _):
        current_loop_state, current_layer_mutables = carry
        current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
        new_ls, new_mets, new_muts = _remat_step(
            layers_params, layers_metrics, layers_graph, bsw_ref[0],
            current_loop_state, current_layer_mutables,
        )
        return (new_ls, new_muts), new_mets

      def run_inner_scan(loop_state_in, layer_mutables_in):
        if self.config.scan_pipeline_iterations:
          return jax.lax.scan(
              inner_body, (loop_state_in, layer_mutables_in),
              None, length=num_microbatches,
          )
        else:
          carry = (loop_state_in, layer_mutables_in)
          metrics_list = []
          for _ in range(num_microbatches):
            carry, m = inner_body(carry, None)
    ...
    def outer_body(carry, _):
      ...
      (new_loop_state, new_layer_mutables), inner_metrics = run_inner_scan(
          current_loop_state, current_layer_mutables
      )
