...
V65d: Params in carry + raw jax.checkpoint (no lift.checkpoint).

Key changes from V65
--------------------
1. Params moved from ``variable_broadcast`` to USER CARRY (like V46/Linen).
2. ``lift.checkpoint`` replaced with raw ``jax.checkpoint`` inside the scope body.
   Raw jax.checkpoint has no ``_partial_pack`` scope machinery, so the body jaxpr
   is simple enough for PE to prove params identity pass-through.
3. Params returned OUTSIDE jax.checkpoint as identity → KNOWN carry → PE split
   → 2 recomp pairs → 8 pre-opt while loops.

Why V65b failed (OOM 34.2 GB)
------------------------------
V65b used lift.checkpoint which wraps in _partial_pack + jax.remat. The
_partial_pack adds scope_fn/repack_fn equations to the body jaxpr. PE could
NOT prove identity through the scope machinery → treated params as UNKNOWN
carry → saved per-iteration copies → 16 × param_size = OOM.

V65d avoids this by using RAW jax.checkpoint (no _partial_pack). The body
jaxpr is flat: params enter as arg, used inside, but carry output references
the SAME input variable. PE proves identity → no per-iteration copies.
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    self_config_set_remat = self.config.set_remat_policy_on_pipeline_iterations
    ...
    def _stage_fn_for_scope(scope, carry):
      """One repeat: read scope, call jax.checkpoint(inner), return carry.

      Params are in USER CARRY (not scope broadcast). Returned as identity
      OUTSIDE jax.checkpoint so PE can prove KNOWN → 2 recomp pairs.
      """
      loop_state_carry, w_curr, params_collection = carry

      # ---- Params from carry (NOT scope) ----
      local_param_arrays = _linen_collection_to_arrays(params_collection, param_keys)
      local_layers_params = _unflatten_nnx_state(
          local_param_arrays, param_treedef, param_is_var, param_var_types, param_meta
      )

      # ---- Read remaining arrays from scope ----
      ...
      # ---- Execute via RAW jax.checkpoint (no lift.checkpoint) ----
      # All values as EXPLICIT ARGS. No _partial_pack obscuring the body.
      # jax.checkpoint wraps the inner computation; params pass through OUTSIDE it.
      def _checkpointed_inner(ls_arg, w_arg, params_arg, muts_arg,
                               met_arg, pos_arg, seg_arg):
        """Inner function wrapped by raw jax.checkpoint.
        Captures only Python objects (ctx, layers_graph, remat_policy, etc.)."""
        return _execute_stage(ls_arg, w_arg, params_arg,
                              muts_arg, met_arg, pos_arg, seg_arg)

      if self_config_set_remat:
        checkpointed_inner = jax.checkpoint(
            _checkpointed_inner, policy=remat_policy,
            prevent_cse=not scan_pipeline_iters,
        )
      else:
        checkpointed_inner = _checkpointed_inner

      (new_loop_state, w_next, new_layer_mutables), inner_metrics = checkpointed_inner(
      ...
      # ---- Params returned as identity OUTSIDE jax.checkpoint ----
      # params_collection is the ORIGINAL carry input — not from _execute_stage output.
      # PE can prove this is identity → KNOWN carry → PE split → 2 recomp pairs.
      return (new_loop_state, w_next, params_collection), inner_metrics

    # ---- Build lift.scan(stage_fn) over REPEATS ----
    # NO lift.checkpoint wrapper — jax.checkpoint is called RAW inside the body.
    # This avoids _partial_pack at the checkpoint level, keeping the body jaxpr
    # simple enough for PE to prove params identity.
    ...
    # Prepare initial Linen variables (NO 'params' — params in user carry)
    ...
    # Run the repeat scan (carry includes params as 3rd element)
    ...
    (loop_state, final_w_curr, _final_params), repeat_metrics_raw = scan_result
