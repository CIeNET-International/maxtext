...
"""V66: Fresh NNX circular pipeline with two-level memory control.

Architecture: outer scan over repeats + inner scan over microbatches.
  - Outer body wrapped in jax.checkpoint(nothing_saveable) for memory
  - Inner microbatch step uses custom_vjp to save only (loop_state, rng)
  - BSW (buffer sliding window) computed per repeat, reused across microbatches
  - layers_params as closure (scan const, 1 copy)
  - No nn.scan, nn.remat, nn.Module, or flax.core.lift

Key design: w_curr in carry for 1 all-gather per repeat throughput,
nothing_saveable on outer checkpoint forces recomputation in backward.
"""
...
def is_spec_leaf(x):
  """Predicate matching leaves in the bsw_pps treedef."""
  return isinstance(x, P) or x is None


# Import the base class from the canonical pipeline module
from maxtext.layers.pipeline import NNXPipelineBase
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  Two-level scan architecture:
    outer_scan(repeats):
      carry = (loop_state, rng_mutables, w_curr)
      xs = repeat_indices
      body = jax.checkpoint(execute_one_repeat, policy=nothing_saveable)
        -> BSW = (w_curr, w_next) where w_next = weight_prefetching
        -> inner_scan(microbatches):
            body = custom_vjp microbatch step
              _fwd: run one iteration, save only (loop_state, rng) as residuals
              _bwd: recompute forward from saved residuals + BSW closure, vjp
      -> return (new_loop_state, new_rng, w_next), metrics
  """
  ...
  def from_repeat_weights_to_bsw(
    ...
    def _from_repeat_weights_to_bsw_shardmap(
      ...
      assert pps_treedef.num_leaves == len(weights_leaves), (
          f"repeat_weights/spec leaf count mismatch: specs={pps_treedef.num_leaves}, weights={len(weights_leaves)}"
      )
  ...
  def get_current_weights_from_bsw(self, bsw, loop_iteration, physical_partition_spec):
    ...
    if bsw_pps is not None:
      ...
      assert bsw_treedef == jax.tree.structure(bsw[1]), (
          "BSW half-tree structure mismatch: bsw[0] and bsw[1] must be structurally identical."
      )
  ...
  def run_one_iteration(
    ...
    """Executes forward/backward logic for a single microbatch inside the circular pipeline.

    Fetches params from BSW (params-only), gathers metrics/mutables for current
    repeat, merges into full state for forward pass, then scatter-updates non-params.
    """
    ...
    # 2. Gather non-params (metrics, mutables) for current repeat
    _, repeat_ids = self.get_microbatch_and_repeat_ids(loop_iteration)
    if self.config.num_pipeline_repeats > 1:
  ...
  def __call__(
    ...
    """V66 pipeline: two-level scan with custom_vjp inner + checkpoint outer.

    Outer scan over repeats with jax.checkpoint(nothing_saveable).
    Inner scan over microbatches with per-iteration remat.
    BSW computed once per repeat, reused across microbatches.
    """
    ...
    # --- Compute partition specs ---
    # Full spec (with circular_repeats) for BSW creation
    physical_partition_spec_full = logical_to_mesh(
        logical_partition_spec, mesh=self.mesh, rules=self.config.logical_axis_rules
    )
    # Stripped spec (repeats removed) for BSW consumption in run_one_iteration
    ...
    # Validate mutables contain only RngState
    assert all(
    ...
    # ---- Inner microbatch step ----
    def run_single_microbatch(carry, _unused_xs, buffer_sliding_window):
      """Execute one microbatch iteration using BSW from closure."""
      loop_state_inner, rng_mutables = carry
    ...
    def execute_one_repeat(carry, repeat_index):
      """Execute one full repeat: compute BSW, run all microbatches."""
      loop_state_outer, rng_mutables = carry

      total_iteration_at_repeat_start = repeat_index * num_microbatches

      # Compute w_curr: for repeat 0, use zeros (no prior prefetch);
      # for subsequent repeats, recompute from layers_params
      gathered_weights_current = jax.lax.cond(
          repeat_index == 0,
          lambda: jax.tree.map(lambda x: jnp.zeros(x.shape[1:], dtype=x.dtype), layers_params),
          lambda: self.weight_prefetching(
              layers_params, physical_partition_spec_full, total_iteration_at_repeat_start - 1
          ),
      )

      # Compute w_next via weight_prefetching (auto-diff through all_gather)
      ...
      # Run inner scan over microbatches
      def run_microbatch_scan(loop_state_in, rng_mutables_in, bsw):
        ...
        # Apply per-iteration remat if configured
      ...
      # Reset loop_iteration to the start of this repeat
      loop_state_outer = {**loop_state_outer, "loop_iteration": total_iteration_at_repeat_start}

      (new_loop_state, updated_rng_mutables), repeat_step_metrics = run_microbatch_scan(
          loop_state_outer, rng_mutables, buffer_sliding_window
      )

      return (new_loop_state, updated_rng_mutables), repeat_step_metrics

    # ---- Apply outer checkpoint (nothing_saveable — BSW recomputed in backward) ----
    if self.config.set_remat_policy_on_pipeline_iterations:
      checkpointed_repeat_body = jax.checkpoint(
          execute_one_repeat, policy=jax.checkpoint_policies.nothing_saveable
      )
    else:
    ...
    if bubble_iterations > 0:
      ...
      # Last repeat's weights for bubble iterations
    ...
    # ---- Reconstruct NNX state and return ----
...
# ---- Factory + Linen wrappers for test compatibility ----

def create_nnx_pipeline(
    config: Config, stage_factory: Any, mesh: Mesh, remat_policy: Any = None, *, rngs: nnx.Rngs
) -> NNXCircularPipeline:
  """Factory function to instantiate the V66 NNX Pipeline module."""
...
# Also expose as Pipeline for compatibility with test harness that uses create_pipeline
