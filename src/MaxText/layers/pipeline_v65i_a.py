...
"""V65i-A: V65i with w_curr carried in outer scan to halve all-gathers.

Based on V65i (21.3 GB, 85.6 tok/s, correct gradients).
Change: carry w_curr in outer scan instead of recomputing both BSW slots.
Each repeat: w_curr = previous w_next, only call weight_prefetching once for w_next.
Expected: ~25-26 GB (w_curr in carry adds memory), ~100-130 tok/s (1 all-gather vs 2).
"""
