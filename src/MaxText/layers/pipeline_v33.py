...
"""Pipeline V33: Params in INNER scan carry so remat detects them as inputs.

Per Flax author suggestion: pass params as carry to scan_body so jax.checkpoint
sees them as function inputs (carry), not closure-captured consts. This may
change how remat_partial_eval handles residual forwarding/hoisting.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V33: params in inner scan carry so jax.checkpoint sees them as inputs
    def inner_body(carry, _):
      current_loop_state, current_layer_mutables, carried_params = carry
      ...
      # params passed through unchanged — same value every iteration
      return (new_loop_state, new_layer_mutables, carried_params), new_layer_metrics
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        # V33: pass layers_params as inner carry element
        (new_loop_state, new_layer_mutables, _), inner_metrics = jax.lax.scan(
            inner_body, (current_loop_state, current_layer_mutables, layers_params),
            None, length=num_microbatches,
        )
      else:
        inner_carry = (current_loop_state, current_layer_mutables, layers_params)
        inner_metrics_list = []
        for _ in range(num_microbatches):
          inner_carry, step_metrics = inner_body(inner_carry, None)
          inner_metrics_list.append(step_metrics)
        new_loop_state, new_layer_mutables, _ = inner_carry
        inner_metrics = (
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        # V33: inner_body now expects 3-element carry
        (loop_state, final_layer_mutables, _), bubble_metrics = jax.lax.scan(
            inner_body, (loop_state, final_layer_mutables, layers_params), None, length=bubble_iterations
        )
      else:
        bubble_carry = (loop_state, final_layer_mutables, layers_params)
        bubble_metrics_list = []
        ...
        loop_state, final_layer_mutables, _ = bubble_carry
        bubble_metrics = (
