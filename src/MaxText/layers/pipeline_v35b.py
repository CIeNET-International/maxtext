...
"""Pipeline V35b: Remove w_curr from carry, NO outer checkpoint.

Same as V35 (recompute both w_curr and w_next each repeat) but WITHOUT
jax.checkpoint on outer_body. V35's outer checkpoint caused 76.2%
XLA fragmentation (36.37 GB OOM). V35b tests whether removing w_curr
from carry alone helps, letting XLA schedule buffers naturally.

Outer carry: (loop_state, layer_mutables) — no replicated weights.
w_curr and w_next are intermediates inside outer_body.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # V35b: NO outer checkpoint (V35's checkpoint caused 76.2% XLA fragmentation)
