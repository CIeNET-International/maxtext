...
"""Pipeline V11: Explicit layers_params in outer scan carry with stop_gradient fence.

Builds on V10 (all five fixes) with one additional change:
  6. (Finding 6) Move `layers_params` from closure capture to explicit scan carry
     in the OUTER scan, with a `jax.lax.stop_gradient` fence at the top of the
     scan body. This makes params visible to XLA's buffer planner as explicit
     function args rather than opaque closures.

     - `layers_params` is UNCHANGED between iterations (same stacked params every repeat).
     - The stop_gradient prevents gradient accumulation through the carry path.
     - Gradients for params flow through the custom_vjp BSW path (w_next computed
       from the explicit layers_params arg, not through the carried copy).
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def scan_body(carry, _):
      ...
      # V11: stop_gradient fence -- params are unchanged across iterations,
      # gradients should NOT accumulate through the carry path.
      carried_layers_params = jax.lax.stop_gradient(carried_layers_params)
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      # V11: Use carried_layers_params (explicit arg) instead of closure-captured layers_params
      ...
      # V11: Pass carried_layers_params back unchanged in the output carry
      return (new_loop_state, new_layer_mutables, carried_layers_params), new_layer_metrics
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  V11 addition: layers_params moved from closure capture to explicit outer scan
  carry with stop_gradient fence, making params visible to XLA buffer planner.
  """
  ...
  def __call__(
    ...
    #
    # V11: layers_params is now in outer scan carry (with stop_gradient fence)
    #   instead of closure capture, so XLA's buffer planner sees it as an
    #   explicit arg. Unlike BSW, layers_params is already stored for backward
    #   anyway (it's needed for gradient computation), so adding it to carry
    #   does NOT increase memory -- it just changes how XLA sees it.
    bsw_ref = [None]
    ...
    def outer_body(carry, _):
      ...
      V11: layers_params is now an explicit carry element (with stop_gradient
      fence) instead of being captured from closure. This makes params visible
      to XLA's buffer planner as explicit function args.
      """
      current_loop_state, current_layer_mutables, w_curr, carried_layers_params = carry
      iteration = current_loop_state["loop_iteration"]

      # V11 Finding 6: stop_gradient fence on carried layers_params.
      # Params are UNCHANGED between outer scan iterations (same stacked params
      # every repeat). The stop_gradient prevents gradient accumulation through
      # the carry path. Gradients for params flow through the custom BSW
      # all-gather path (w_next computed from carried_layers_params) instead.
      carried_layers_params = jax.lax.stop_gradient(carried_layers_params)
      ...
      # V11: Pass carried_layers_params back unchanged in the output carry
      return (new_loop_state, new_layer_mutables, w_next, carried_layers_params), inner_metrics
