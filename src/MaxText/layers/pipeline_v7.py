...
"""Pipeline V7: Fix vmap return + stop_gradient mutables (Findings 1+2).

Variant of pipeline.py combining two fixes:
  1. (Finding 1) vmap returns only non-param state, avoiding param stacking across stages.
  2. (Finding 2) stop_gradient on current_layer_mutables at the top of inner_body,
     preventing gradient tape from flowing through RNG state in the scan carry.
     Mutables (RNG keys/counts) should not be differentiated -- they are non-trainable
     state that just needs to flow forward. Differentiating through them wastes memory
     on gradient buffers for RNG arrays.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def get_main_vmap_func_for_iterations(self):
    """V7 override: vmap returns only non-param state to avoid stacking params across stages.

    Same as V6 -- params are static in forward, no need to stack them.
    """
    def func_to_vmap(graph, state, stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode):
      module = nnx.merge(graph, state)
      out = module(stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode)
      # V7: Only return non-param state
  ...
  def run_one_iteration(
    ...
    # V7: vmap now returns (output, (updated_metrics, updated_mutables)) instead of full state
    ...
    # Scatter-back: only update non-params
    if self.config.num_pipeline_repeats > 1:
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      current_loop_state, current_layer_mutables = carry
      # V7 Finding 2: stop_gradient on mutables to prevent gradient tape on RNG state.
      # Mutables (RNG keys/counts) are non-trainable -- differentiating through them
      # wastes memory on gradient buffers for arrays that have zero useful gradient.
