...
"""Pipeline V43: Author's pattern — params as explicit arg to jax.checkpoint.

Flax author suggested: params as closure const for scan (broadcast), but
EXPLICIT positional argument to remat_body (jax.checkpoint wraps remat_body).
This matches how nn.scan(nn.remat(body)) passes params — broadcast by scan,
then explicit inputs to remat via _partial_pack.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V43: Author's pattern — params as explicit arg to remat_body ----
    #
    # Flax author: "nn.scan(nn.remat(body)) passes params to scan as broadcasts,
    # then turns them into explicit inputs to remat."
    #
    # Pattern:
    #   scan_body: params as CLOSURE (broadcast const for scan, num_consts)
    #   remat_body: params as EXPLICIT POSITIONAL ARG (not closure of checkpoint)
    #
    # Baseline had: jax.checkpoint wraps inner_body which captures params via closure.
    # V43 splits: scan wrapper captures params → passes explicitly to checkpoint fn.
    num_microbatches = self.config.num_pipeline_microbatches

    def make_inner_body(bsw):
      """Factory: scan wrapper + remat_body with params as explicit arg."""

      def remat_body(params, metrics, carry_state):
        """Checkpoint-wrapped body. params/metrics are EXPLICIT positional args."""
        current_loop_state, current_layer_mutables = carry_state
        current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
        iteration = current_loop_state["loop_iteration"]
        advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)

        full_state = nnx.State.merge(params, metrics, advanced_mutables)
        m = nnx.merge(layers_graph, full_state)

        new_loop_state, new_layer_state = self.run_one_iteration(
            current_loop_state, bsw, layers_graph, metrics,
      ...
      def inner_body(carry, _):
        """Scan body. params/metrics as CLOSURE here, passed EXPLICITLY to remat_body."""
        current_loop_state, current_layer_mutables = carry
        new_loop_state, new_layer_mutables, new_layer_metrics = remat_body(
            layers_params, layers_metrics, (current_loop_state, current_layer_mutables)
        )
        return (new_loop_state, new_layer_mutables), new_layer_metrics
