...
"""Pipeline V47: NNX-native ``_partial_pack`` scope isolation (Solution 4).

The MaxText circular pipeline costs ~6.6 GB more compile-time memory under NNX
(29.9 GB) than under Linen (23.3 GB). Linen wins because ``nn.scan`` is built on
``flax.core.lift._partial_pack``: loop-invariant variable collections are frozen
and flow through an isolated ``Scope(parent=None)`` -- a *real traced function
boundary*. That boundary lowers to nested HLO computations and gives the Linen
pipeline 8 HLO ``while`` loops. The naive NNX port -- ``jax.lax.scan`` over a
body captured as a Python closure -- has no boundary and produces only 4.

V47 replicates that boundary with ``maxtext.utils.nnx_partial_pack``. The
circular pipeline's inner microbatch scan now runs through
``nnx_partial_pack_scan``: the per-step body executes inside a
``jax.jit(inline=False)`` call, and the broadcast weights (BSW) cross that jit
as an EXPLICIT positional argument rather than as a Python closure capture.

Why ``jax.jit(inline=False)`` and not the alternatives:
  * ``jax.remat`` / ``jax.checkpoint`` -- ``remat_p`` is a transparent jaxpr
    marker; it does NOT lower to a separate XLA computation (V45b: nested
    checkpoint was +6.2 GB worse, only 5 while loops).
  * ``jax.jit(inline=True)`` -- the body is inlined during tracing; the jaxpr is
    identical to the no-boundary baseline (V45: no effect).
  * ``jax.named_call`` -- inlined away in JAX 0.9.x; no primitive survives.
  * ``jax.jit(inline=False)`` (the default) -- emits the ``pjit`` primitive,
    which was verified to survive in the jaxpr even inside ``jax.lax.scan`` and
    through ``jax.grad``, and to lower to HLO with nested ``called_computations``
    -- a genuine separate XLA computation, the analogue of ``_partial_pack``'s
    isolated scope.

Status: HIGH RISK / EXPERIMENTAL. The ``pjit`` boundary is a real nested
computation, but whether XLA's memory scheduler reacts to it the same way it
reacts to ``_partial_pack``'s scan nesting is unproven -- it must be confirmed
by measuring HLO ``while`` count and peak memory via ``test_v47_compare.sh``.
"""
...
from maxtext.utils.nnx_partial_pack import nnx_partial_pack_call
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V47: NNX-native _partial_pack boundary for the inner microbatch body ----
    #
    # In the baseline, `inner_body` reads `bsw_ref[0]` -- the broadcast weights
    # (BSW) -- as a Python CLOSURE. XLA bakes those params straight into the
    # scan body; there is no traced boundary, hence only 4 HLO while loops.
    #
    # Linen's nn.scan is built on _partial_pack, which freezes loop-invariant
    # collections and routes them through an isolated Scope(parent=None) -- a
    # real traced function boundary that produces 8 HLO while loops.
    #
    # V47 reproduces that boundary: `_run_iteration_packed` receives `bsw` as
    # its FIRST EXPLICIT argument, and `nnx_partial_pack_call` runs it inside a
    # jax.jit(inline=False). The jit emits the `pjit` primitive -- the only
    # mechanism verified to survive (a) inside jax.lax.scan, (b) through
    # jax.grad, and (c) down to HLO as a real nested computation. `self`,
    # `layers_graph`, `layers_metrics` and the static flags are CLOSED OVER
    # (they are not JAX-traceable), so only array pytrees cross the boundary.
    def _run_iteration_packed(bsw, current_loop_state, advanced_mutables, positions_, segment_ids_):
      """Body run behind the jax.jit(inline=False) boundary.

      `bsw` is the explicit broadcast argument -- the NNX analogue of a frozen
      collection flowing through _partial_pack's Scope(parent=None).
      """
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state,
          bsw,
    ...
    def inner_body(carry, _):
      ...
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      # bsw_ref[0] is CLOSED OVER the scan (a loop constant) but is passed as an
      # EXPLICIT argument through the jit boundary inside nnx_partial_pack_call.
      new_loop_state, new_layer_mutables, new_layer_metrics = nnx_partial_pack_call(
          _run_iteration_packed,
          bsw_ref[0],
          current_loop_state,
          advanced_mutables,
          positions,
          segment_ids,
          name="nnx_partial_pack_iteration",
      )
      return (new_loop_state, new_layer_mutables), new_layer_metrics

    if self.config.set_remat_policy_on_pipeline_iterations:
      # remat wraps the jit-bounded body. jax.checkpoint is a transparent jaxpr
      # marker, so it does NOT erase the inner `pjit` boundary -- it only adds
      # the activation-rematerialization trade-off on top of it.
