...
V46 -- Solution 2: Full pipeline_utils.py port.
================================================

Background
----------
The NNX circular pipeline used a hand-written nested ``jax.lax.scan`` /
``jax.checkpoint`` structure that produced 4 HLO while loops and 29.9 GB
compile-time memory. The Linen ``CircularPipeline`` routes through the
3-level ``custom_vjp`` machinery in ``pipeline_utils.py`` and gets 8 HLO
while loops and 23.3 GB. The 6.6 GB gap comes from Flax ``_partial_pack``
scope isolation -- ``nn.scan(nn.remat(...))`` freezes broadcast variables
into a parent-less inner scope, giving XLA better buffer scheduling.

What V46 does
-------------
``NNXCircularPipeline.__call__`` is rewired to run the *exact* Linen path:

  * ``pipeline_utils.create_gradient_accumulation_scan`` -- Level 1+2 custom
    VJP: per-microbatch ``jax.remat`` + scan with d+g weight accumulation.
  * ``pipeline_utils.create_pipeline_stage`` -- Level 3 custom VJP:
    per-repeat ``weight_prefetching`` with ``jax.linear_transpose`` for the
    reduce-scatter dual.
  * ``pipeline_utils.create_flax_pipeline_scan`` -- wraps the stage in
    ``nn.scan(nn.remat(...))`` which triggers ``_partial_pack``.

The adapter (``_PipelineAdapter``)
----------------------------------
``pipeline_utils`` expects a Linen ``nn.Module`` exposing
``run_one_iteration(loop_state, bsw, positions, segment_ids, deterministic,
model_mode, logical_partition_spec=...)``, ``weight_prefetching`` and
``get_pipeline_remat_policy``. NNX's ``run_one_iteration`` needs three extra
arguments (``layers_graph``, ``layers_metrics``, ``advanced_mutables``).

``_PipelineAdapter`` is a REAL ``nn.Module`` (not a plain class), defined
inline inside ``_run_via_pipeline_utils`` so its methods can close over the
statically-captured NNX layer state. It is exactly the module that
``create_flax_pipeline_scan`` lifts with ``nn.scan(nn.remat(...))``, so Flax
``_partial_pack`` engages on it. The adapter:

  * Threads the NNX pipeline weights through the ``nn.scan`` carry as the
    ``pipeline_weights`` element -- identical to how Linen ``CircularPipeline``
    threads ``self.layers.variables``. ``create_pipeline_stage`` passes that
    carried pytree straight into ``weight_prefetching``; nothing is a dead
    closure shell.
  * ``run_one_iteration`` GENUINELY routes computation into the NNX
    pipeline's ``run_one_iteration`` (the NNX-specific args -- graphdef,
    metrics, advanced RNG mutables -- are reconstructed from immutable
    state captured before the scan; RNG mutables are derived purely from
    ``loop_state["loop_iteration"]`` via ``_advance_rng_state`` so nothing
    mutable crosses the traced boundary).

Lessons from failed variants encoded here
-----------------------------------------
  * V40 -- a plain-class adapter held mutable Python state inside the traced
    scope -> tracer leak. The adapter here has NO mutable Python state
    inside traced code.
  * V44 -- a thin ``RepeatStage`` shell registered a Linen variable but
    ignored it; the real compute stayed closure-captured NNX, so
    ``_partial_pack`` saw an empty shell and only added a redundant nested
    while loop -> OOM. Here ``run_one_iteration`` and ``weight_prefetching``
    CONSUME the ``pipeline_weights`` / ``bsw`` arguments that flow through
    the scan carry, so ``_partial_pack`` manages real, load-bearing
    variables.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    """V46 entry point -- routes the circular pipeline through the full
    ``pipeline_utils.py`` 3-level custom-VJP machinery via a Linen adapter.

    The whole body is delegated to ``_run_via_pipeline_utils`` so the
    method stays readable; the heavy lifting is in the adapter + the three
    ``pipeline_utils`` factory functions.
    """
    inputs = inputs.reshape(
        (
            self.config.num_pipeline_microbatches,
    ...
    final_output = self._run_via_pipeline_utils(
        loop_state, inputs, positions, segment_ids, deterministic, model_mode, logical_partition_spec
    )
    return jnp.reshape(
        final_output,
        (self.config.micro_batch_size_to_train_on, self.config.max_target_length, self.config.emb_dim),
  ...
  # ----------------------------------------------------------------------
  # V46: full pipeline_utils.py port
  # ----------------------------------------------------------------------

  def _run_via_pipeline_utils(
      self, loop_state, inputs, positions, segment_ids, deterministic, model_mode, logical_partition_spec
  ):
    """Drives the circular pipeline exactly like Linen ``CircularPipeline``.

    Mirrors ``pipeline_linen.py`` ``CircularPipeline.__call__`` lines
    1373-1401: builds the ``create_pipeline_stage`` stage functions for the
    real-repeat and bubble runs, wraps each with ``create_flax_pipeline_scan``
    (``nn.scan(nn.remat(...))`` -> ``_partial_pack`` engages), and executes
    them with a carry of ``(loop_state, w_curr, pipeline_weights)``.
    """
    # Two spec variants needed (identical reasoning to the baseline NNX path):
    # - Full spec (with circular_repeats axis) -> BSW creation in
    #   weight_prefetching: from_repeat_weights_to_bsw drops the first
    #   (repeat) dim, so the input must still carry it.
    # - Stripped logical spec (circular_repeats removed) -> BSW consumption
    #   in run_one_iteration: get_current_weights_from_bsw expects the
    #   repeat axis already gathered away.
    ...
    # ---- Split the NNX layer state once, before any tracing ----
    # graphdef + metrics + base RNG mutables are STATIC w.r.t. the scan
    # (closed over by the adapter, never mutated inside traced code -- the
    # V40 tracer-leak lesson). Only `layers_params` flows through the scan
    # carry, exactly as Linen threads `self.layers.variables`.
    layers_graph, layers_state = nnx.split(self.layers)
    ...
    # layers_mutables catch-all must be ONLY RngState (RngKey/RngCount).
    assert all(
        isinstance(v, nnx.RngState)
        for v in jax.tree.leaves(layers_mutables, is_leaf=lambda x: isinstance(x, nnx.Variable))
        if isinstance(v, nnx.Variable)
    ...
    # Strip nnx.Variable wrappers -> raw arrays. pipeline_utils threads the
    # weights as plain pytrees through jax.vjp / jax.lax.scan; outer-scope
    # nnx.Variable wrappers have _can_update=False and break aliasing checks.
    # run_one_iteration re-wraps via nnx.merge inside the vmap.
    def strip_vars(tree):
      return jax.tree.map(
          lambda x: x.value if isinstance(x, nnx.Variable) else x,
          tree,
    ...
    # ---- The Linen adapter ----
    #
    # ``pipeline_utils`` was written for the Linen ``CircularPipeline`` and
    # calls ``model.run_one_iteration`` / ``model.weight_prefetching`` /
    # ``model.get_pipeline_remat_policy`` on a Linen ``nn.Module`` whose
    # ``run_one_iteration`` has the 7-arg Linen signature::
    #
    #     run_one_iteration(loop_state, bsw, positions, segment_ids,
    #                       deterministic, model_mode, logical_partition_spec=...)
    #
    # NNX's ``NNXCircularPipeline.run_one_iteration`` needs three extra args
    # (``layers_graph``, ``layers_metrics``, per-iteration ``advanced_mutables``).
    #
    # ``_PipelineAdapter`` is a REAL ``nn.Module`` -- it is exactly what
    # ``create_flax_pipeline_scan`` lifts with ``nn.scan(nn.remat(...))``, so
    # Flax ``_partial_pack`` genuinely engages on it. The class is defined
    # inline so its methods can close over the statically-captured NNX state
    # (``layers_graph`` / ``layers_metrics`` / base ``layers_mutables``) --
    # the same closure pattern Linen ``CircularPipeline`` uses for positions
    # and specs. Crucially:
    #
    #   * NO mutable Python state is closed over inside traced code
    #     (the V40 ``mutables_ref[0]`` tracer-leak lesson). ``layers_graph``
    #     is a static ``GraphDef``; ``layers_metrics`` / ``layers_mutables``
    #     are read-only outer-trace constants; per-iteration RNG state is
    #     derived purely from ``loop_state["loop_iteration"]``.
    #   * ``run_one_iteration`` / ``weight_prefetching`` CONSUME the ``bsw`` /
    #     ``pipeline_weights`` that ``pipeline_utils`` threads through the
    #     ``nn.scan`` carry -- they are not a dead shell (the V44 lesson:
    #     the adapter must genuinely route computation so ``_partial_pack``
    #     manages load-bearing variables).
    nnx_pipeline = self

    class _PipelineAdapter(nn.Module):
      """Linen-signature facade over ``NNXCircularPipeline`` for pipeline_utils."""

      def get_pipeline_remat_policy(self_adapter):
        # Used by create_gradient_accumulation_scan as the per-microbatch
        # jax.remat policy.
        return nnx_pipeline.get_pipeline_remat_policy()

      def weight_prefetching(self_adapter, weights, physical_partition_spec, loop_iteration):
        # ``weights`` is the ``pipeline_weights`` pytree threaded through the
        # nn.scan carry by create_pipeline_stage. Routing it straight into
        # the NNX prefetch is what makes _partial_pack manage real data.
        return nnx_pipeline.weight_prefetching(weights, physical_partition_spec, loop_iteration)
      ...
      def run_one_iteration(
        ...
        """Linen 7-arg signature -> NNX 11-arg run_one_iteration.

        ``create_gradient_accumulation_scan`` calls this. The three NNX-only
        arguments are reconstructed:
          * ``layers_graph`` / ``layers_metrics`` -- captured statically
            before the scan (closed over).
          * ``advanced_mutables`` -- folded from ``loop_iteration`` via
            ``_advance_rng_state`` (jax.lax.scan has no ``split_rngs``;
            this reproduces ``nn.scan(split_rngs={"random": True})``).

        NNX ``run_one_iteration`` returns ``(new_loop_state, new_layer_state)``;
        the Linen contract that ``pipeline_utils`` expects returns ONLY the
        loop state. The layer-state half (metrics / mutables) is dropped here
        -- diagnostic only -- matching how Linen ``CircularPipeline`` keeps
        its own ``intermediates`` outside the threaded carry.
        """
        iteration = loop_state["loop_iteration"]
        # RNG state carries no gradient; fold the loop counter in to get a
        # unique dropout mask per microbatch -- mirrors the baseline NNX
        # inner_body.
        advanced_mutables = _advance_rng_state(jax.lax.stop_gradient(layers_mutables), iteration)
        new_loop_state, _new_layer_state = nnx_pipeline.run_one_iteration(
            loop_state,
            bsw,
            layers_graph,
            layers_metrics,
      ...
      def __call__(self_adapter, carry):
        # pipeline_utils never calls the adapter via __call__ (it calls the
        # named methods directly), but nn.Module requires a callable entry
        # point for binding. Kept as an identity so the module is valid.
        return carry

    adapter = _PipelineAdapter()

    # ---- Level 3 stage builders (pipeline_utils.create_pipeline_stage) ----
    # Mirrors pipeline_linen.py CircularPipeline.__call__ lines 1373-1384.
    base_scannable = functools.partial(
        pipeline_utils.create_pipeline_stage,
        deterministic=deterministic,
        model_mode=model_mode,
        logical_partition_spec=logical_partition_spec_stripped,
        physical_partition_spec=physical_partition_spec_full,
        positions=positions,
        segment_ids=segment_ids,
    )
    run_one_repeat_scannable = base_scannable(length=num_microbatches)
    run_bubbles_scannable = base_scannable(length=bubble_iterations)

    # ---- Wrap in nn.scan(nn.remat(...)) -- this triggers _partial_pack ----
    # Mirrors pipeline_linen.py CircularPipeline.__call__ lines 1386-1397.
    ...
    # ---- Execute: real repeats then bubble drain ----
    # Carry shape == Linen's: (loop_state, w_curr, pipeline_weights).
    # pipeline_weights is the params pytree threaded through the scan carry;
    # the per-microbatch d+g accumulation custom-VJP and the per-repeat
    # weight_prefetching linear_transpose dual run inside the lifted stage.
    initial_w_curr = jax.tree.map(lambda x: jnp.zeros(x.shape[1:], dtype=x.dtype), layers_params_raw)
    initial_carry_repeats = (loop_state, initial_w_curr, layers_params_raw)
    (loop_state, w_curr, pipeline_weights), _ = run_repeats_scanned(adapter, initial_carry_repeats)

    if bubble_iterations > 0:
      initial_carry_bubbles = (loop_state, w_curr, pipeline_weights)
      (loop_state, _, pipeline_weights), _ = run_bubbles_scanned(adapter, initial_carry_bubbles)

    # ``pipeline_weights`` is only consumed for its (passthrough) value; the
    # carry is closed below by discarding it.
    del pipeline_weights

    # ---- Write layer state back into the NNX module ----
    # pipeline_utils threads only the params pytree, and the forward pass
    # passes it through unchanged: ``create_pipeline_stage`` repacks the
    # carry as ``(*execute_pipeline_stage_pure(...), pipeline_weights)`` with
    # ``pipeline_weights`` untouched (gradients flow via the custom-VJP, not
    # via forward mutation). So the post-scan params equal the pre-scan
    # ``layers_params`` -- write those back directly, preserving the
    # nnx.Variable wrappers and the original tree structure.
    #
    # Metrics produced inside the NNX forward pass are diagnostic only and
    # -- matching how Linen ``CircularPipeline`` keeps its own
    # ``intermediates`` outside the returned activations -- are not threaded
    # back; base metrics / RNG mutables are written unchanged. Loss and
    # gradient correctness are unaffected: those flow through ``params``,
    # which the 3-level custom-VJP chain handles.
    final_layer_state = nnx.State.merge(layers_params, layers_metrics, layers_mutables)
    nnx.update(self.layers, final_layer_state)

    return self.realign_output_microbatches(loop_state["state_io"])
