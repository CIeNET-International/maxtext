...
class SequentialDecoderLayers(nnx.Module):
  ...
  def __init__(self, layer_class, num_layers, config, mesh, model_mode, rngs, **kwargs):
    ...
    for i in range(num_layers):
      layers_list.append(layer_class(config=config, mesh=mesh, model_mode=model_mode, rngs=rngs, **kwargs))
...
class ScannedDecoderLayers(nnx.Module):
  ...
  def __init__(self, layer_class, num_layers, config, mesh, model_mode, rngs, **kwargs):
    ...
    self.layers_stack = nnx.vmap(create_layer, in_axes=0, out_axes=0, axis_name="layers")(rngs)
...
class Decoder(nnx.Module):
  ...
  def _init_pipeline_layers(self, rngs):
    self.pipeline_module = self.get_pipeline_stage_module(self.get_decoder_layer_class())
    ...
    if num_full_blocks > 0:
      ...
      def create_block(p_key, d_key):
        ...
        return ScannableBlock(
            ...
            num_of_layers=pattern_len,
            rngs=block_rngs,
        )

      if self.config.scan_layers:
        # vmap over keys
        self.gemma_main_stack = nnx.vmap(create_block, transform_metadata={nnx.PARTITION_NAME: "layers"})(
            block_param_keys, block_dropout_keys
        )
      else:
        self.gemma_main_stack = [create_block(block_param_keys[i], block_dropout_keys[i]) for i in range(num_full_blocks)]
    else:
      self.gemma_main_stack = None

    if num_remaining > 0:
      # For a single block, we don't need vmap splitting, just new keys
      rem_rngs = nnx.Rngs(params=jax.random.split(rngs.params(), 1)[0], dropout=jax.random.split(rngs.dropout(), 1)[0])
  ...
  def _create_layer_stack(self, BlockClass, param_keys, dropout_keys, scan_axis_name="layers"):
    ...
    def create_layer(p_key, d_key):
      ...
      return BlockClass(config=cfg, mesh=self.mesh, quant=self.quant, model_mode=self.model_mode, rngs=layer_rngs)
    ...
    if self.is_scanned:
      ...
      stack = nnx.vmap(create_layer, transform_metadata={nnx.PARTITION_NAME: scan_axis_name})(param_keys, dropout_keys)
      return stack
    else:
  ...
  def get_pipeline_stage_module(self, decoder_blocks):
    ...
    # Fork RNGs before passing to stage module to avoid sharing with Pipeline
  ...
  def _scan_single_block(self, layer_stack, init_y, init_kv_stack, broadcast_args):
    ...
    def scan_fn(carry, x):
      ...
      if self.config.remat_policy != "none":
        ...
        layer_forward = jax.checkpoint(layer_forward, policy=policy, prevent_cse=prevent_cse)  # <--- KEY MISSING ARG
    ...
    with self.mesh:
      ...
      def collect_kvs(kvs):
        if kvs is not None:
          if isinstance(kvs, list):
            all_new_kvs.extend(kvs)
          else:
            all_new_kvs.append(kvs)
      ...
      if self.using_pipeline:
        ...
        for stack_idx, stack in enumerate(self.layer_stacks):
          ...
          if self.is_scanned:
            y, new_kv_stack, stack_mets = self._scan_single_block(stack, y, current_kvs_stack, broadcast_args)
            ...
            for i, layer in enumerate(stack):
              ...
              if cfg.remat_policy != "none":
                ...
                result = jax.checkpoint(layer_run_explicit, policy=policy, prevent_cse=prevent_cse)(y, kv_in)
              else:
