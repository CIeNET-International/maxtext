...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def inner_body_base(carry, _, bsw):
      c_loop_state, c_layer_mutables = carry
      c_layer_mutables = jax.lax.stop_gradient(c_layer_mutables)
      it = c_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(c_layer_mutables, it)
      new_loop_state, new_layer_state = self.run_one_iteration(
          c_loop_state,
          bsw,
          layers_graph,
    ...
    def outer_body(carry, iteration):
      ...
      w_curr = jax.lax.cond(
          ...
          lambda: self.weight_prefetching(layers_params, physical_partition_spec_full, iteration - 1)
      )
      w_next = self.weight_prefetching(layers_params, physical_partition_spec_full, iteration)
      bsw = (w_curr, w_next)

      def scan_fn(ls, lm, bsw_val):
        def inner_body_vjp(c, _):
           return inner_body_base(c, _, bsw_val)
        
        if self.config.set_remat_policy_on_pipeline_iterations:
          inner_body_vjp = jax.checkpoint(
              inner_body_vjp, policy=self.get_pipeline_remat_policy(), prevent_cse=not self.config.scan_pipeline_iterations
          )

        if self.config.scan_pipeline_iterations:
          (new_ls, new_lm), inner_metrics = jax.lax.scan(inner_body_vjp, (ls, lm), None, length=num_microbatches)
          return (new_ls, new_lm), inner_metrics
        else:
          inner_carry = (ls, lm)
          inner_metrics_list = []
          for _ in range(num_microbatches):
            inner_carry, step_metrics = inner_body_vjp(inner_carry, None)
          ...
          new_ls, new_lm = inner_carry
          return (new_ls, new_lm), inner_metrics

      current_loop_state["loop_iteration"] = iteration
      (new_loop_state, new_layer_mutables), inner_metrics = scan_fn(current_loop_state, current_layer_mutables, bsw)
      return (new_loop_state, new_layer_mutables), inner_metrics

    if self.config.set_remat_policy_on_pipeline_iterations:
      outer_body_scannable = jax.checkpoint(outer_body, policy=self.get_pipeline_remat_policy())
    else:
    ...
    # Force using jax.lax.scan so XLA compiles outer_body ONCE, preventing OOM
    ...
    if bubble_iterations > 0:
      final_w_curr = self.weight_prefetching(layers_params, physical_partition_spec_full, num_repeats - 1)
      bubble_bsw = (final_w_curr, final_w_curr)

      def inner_body_bubble(c, _):
        return inner_body_base(c, _, bubble_bsw)
      ...
      if self.config.set_remat_policy_on_pipeline_iterations:
        ...
        bubble_body_scannable = inner_body_bubble

      loop_state["loop_iteration"] = num_repeats
      if self.config.scan_pipeline_iterations:
        (loop_state, final_layer_mutables), bubble_metrics = jax.lax.scan(
            bubble_body_scannable, (loop_state, final_layer_mutables), None, length=bubble_iterations
        )
      else:
