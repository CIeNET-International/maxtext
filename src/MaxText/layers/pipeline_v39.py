...
"""Pipeline V39: Multiple explicit while_loops for more HLO scheduling boundaries.

Linen produces 8 while loops in HLO (vs NNX's 4). More loops give XLA more
boundaries to optimize buffer lifetimes. V39 creates additional while loops by:
  1. Using jax.lax.while_loop for inner microbatch scan (explicit loop counter)
  2. Using jax.lax.while_loop for outer repeat scan
  3. Adding a separate recomputation while_loop for backward

Goal: More while loops in HLO → more XLA scheduling boundaries → lower temp.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V39: Multiple while_loops for more HLO scheduling boundaries ----
    ...
    def inner_step(carry):
      ...
      current_loop_state, current_layer_mutables, step_idx = carry
      ...
      _, _, _, new_layer_mutables = nnx.split(
          new_layer_state, _is_static_param, nnx.Intermediate, ...
      )
      return (new_loop_state, new_layer_mutables, step_idx + 1)

    if self.config.set_remat_policy_on_pipeline_iterations:
      inner_step = jax.checkpoint(
          inner_step, policy=self.get_pipeline_remat_policy(),
      )

    def run_inner_microbatches(loop_state, layer_mutables):
      """Inner microbatch loop as explicit while_loop."""
      def cond_fn(carry):
        return carry[2] < num_microbatches

      init = (loop_state, layer_mutables, jnp.int32(0))
      final_loop_state, final_mutables, _ = jax.lax.while_loop(cond_fn, inner_step, init)
      return final_loop_state, final_mutables

    def run_one_repeat_step(carry):
      """One repeat iteration for outer while_loop."""
      current_loop_state, current_layer_mutables, w_curr, repeat_idx = carry
      iteration = current_loop_state["loop_iteration"]
      ...
      new_loop_state, new_layer_mutables = run_inner_microbatches(
          current_loop_state, current_layer_mutables
      )

      return (new_loop_state, new_layer_mutables, w_next, repeat_idx + 1)

    # ---- Execute: outer while_loop (repeats) ----
    initial_w_curr = jax.tree.map(lambda x: jnp.zeros(x.shape[1:], dtype=x.dtype), layers_params)

    def outer_cond(carry):
      return carry[3] < num_repeats

    outer_init = (loop_state, layers_mutables, initial_w_curr, jnp.int32(0))
    loop_state, final_layer_mutables, final_w_curr, _ = jax.lax.while_loop(
        outer_cond, run_one_repeat_step, outer_init
    )
    ...
    if bubble_iterations > 0:
      ...
      def bubble_cond(carry):
        return carry[2] < bubble_iterations

      bubble_init = (loop_state, final_layer_mutables, jnp.int32(0))
      loop_state, final_layer_mutables, _ = jax.lax.while_loop(
          bubble_cond, inner_step, bubble_init
      )
