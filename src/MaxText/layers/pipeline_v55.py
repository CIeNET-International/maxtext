...
V55: Uses nnx_scan_remat.scan_with_remat_simple for coordinated scan+remat,
mirroring Linen's nn.scan(nn.remat(body)) structure with axes_scan.scan +
jax.remat(explicit_broadcast_args). The key structural difference from
base pipeline: broadcast args (params, metrics, graph, bsw) are the FIRST
positional arg to jax.remat inside axes_scan.scan, matching the exact
call pattern of lift.scan -> lift.checkpoint -> jax.remat(rematted).
"""
...
from maxtext.utils.nnx_scan_remat import scan_with_remat_simple
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V55: Coordinated scan+remat via nnx_scan_remat ----
    #
    # Mirrors Linen's nn.scan(nn.remat(body)) EXACTLY:
    #   - axes_scan.scan handles broadcast routing (params as 1st positional arg)
    #   - jax.remat wraps body with broadcast as explicit 1st arg
    #   - The combination produces the same call pattern as
    #     lift.scan -> axes_scan.scan -> lift.checkpoint -> jax.remat(rematted)
    #
    # Structure: scan_with_remat_simple(
    #   body_fn(broadcast, carry) -> (new_carry, output),
    #   broadcast_args = (params, metrics, graph),  # loop-invariant
    #   init_carry = (loop_state, mutables),         # loop-varying
    # )
    bsw_ref = [None]
    ...
    def inner_body_fn(broadcast, carry):
      """Microbatch body matching Linen's rematted(variable_groups, rngs, *args).

      broadcast = (params, metrics, graph) — loop-invariant, explicit 1st arg
      carry = (loop_state, mutables) — loop-varying
      """
      params, metrics, graph = broadcast
      loop_state, layer_mutables = carry
      layer_mutables = jax.lax.stop_gradient(layer_mutables)
      iteration = loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          loop_state, bsw_ref[0], graph, metrics,
    ...
    def run_inner_scan(current_loop_state, current_layer_mutables):
      if self.config.scan_pipeline_iterations:
        (_, final_carry), stacked_mets = scan_with_remat_simple(
            body_fn=inner_body_fn,
            broadcast_args=(layers_params, layers_metrics, layers_graph),
            init_carry=(current_loop_state, current_layer_mutables),
            length=num_microbatches,
            remat_policy=remat_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )
        return final_carry, stacked_mets
      else:
        carry = (current_loop_state, current_layer_mutables)
        metrics_list = []
        broadcast = (layers_params, layers_metrics, layers_graph)
        for _ in range(num_microbatches):
          carry, m = inner_body_fn(broadcast, carry)
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        (_, bubble_carry), bubble_metrics = scan_with_remat_simple(
            body_fn=inner_body_fn,
            broadcast_args=(layers_params, layers_metrics, layers_graph),
            init_carry=(loop_state, final_layer_mutables),
        ...
        for _ in range(bubble_iterations):
          bubble_carry, bub_metrics = inner_body_fn(broadcast, bubble_carry)
