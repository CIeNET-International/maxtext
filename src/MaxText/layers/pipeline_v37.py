...
"""Pipeline V37: Move outer scan outside pipeline module.

Mirrors Linen's structure where nn.scan wraps the pipeline stage from
outside (decoder level), not inside __call__. The pipeline module only
handles one repeat at a time. This changes the trace boundary — scan
body calls a module method, not an internal closure.

Goal: Different jaxpr structure → different HLO → better XLA scheduling.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V37: Scan body calls module method (not internal closure) ----
    ...
    # Exposed method for one repeat — scan calls this from outside
    def run_one_repeat(carry, _):
      """One repeat: called by outer scan. Module method, not closure."""
    ...
    # Wrap run_one_repeat in jax.checkpoint (matching nn.remat at outer level)
    if self.config.set_remat_policy_on_pipeline_iterations:
      run_one_repeat_checkpointed = jax.checkpoint(run_one_repeat)
    else:
      run_one_repeat_checkpointed = run_one_repeat
    ...
    if self.config.scan_pipeline_iterations:
      ...
      for _ in range(num_repeats):
        outer_carry, rep_metrics = run_one_repeat_checkpointed(outer_carry, None)
