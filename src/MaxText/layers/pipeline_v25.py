...
"""Pipeline V25: Add extra jax.checkpoint boundaries inside run_one_iteration.

Variant targeting scan body equation count. Adds checkpoint boundaries around
the vmap call and the scatter-update inside NNXCircularPipeline.run_one_iteration.
This adds a checkpoint boundary inside the scan body, matching Linen's pattern
of having 4 remat checkpoints vs NNX's 2, potentially reducing XLA temp memory
by creating more granular recomputation segments.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def run_one_iteration(
    ...
    # V25: Wrap the vmap forward pass in jax.checkpoint to create an additional
    # remat boundary inside the scan body. This matches Linen's pattern of having
    # 4 remat checkpoints vs NNX's 2, potentially reducing XLA temp memory by
    # creating more granular recomputation segments.
    @jax.checkpoint
    def _checkpointed_vmap_forward(graph, state, inputs, seg_ids, pos, det, mode):
      return vmap_func(graph, state, inputs, seg_ids, pos, det, mode)

    stages_output, (updated_stage_metrics, updated_stage_mutables) = _checkpointed_vmap_forward(
        pipeline_weights_graph,
        stage_weights_state,
        stages_inputs,
        stages_segment_ids,
    ...
    # V25: Wrap scatter-update in jax.checkpoint to add another remat boundary.
    # This isolates the weight scatter from the forward pass, potentially allowing
    # XLA to schedule buffer lifetimes more efficiently.
    # Scatter-back: only update mutables (params handled by AD/gradient, metrics returned directly)
    if self.config.num_pipeline_repeats > 1:

      @jax.checkpoint
      def _checkpointed_scatter_update(current_mutables, updated_mutables, rep_ids):
        def _scatter_update_mutables(fw, uw):
          ...
          r_ids = self.shard_dim_by_stages(rep_ids, 0, physical_partition_spec=None)
          updated_fw = jax.vmap(_update_one_stage, in_axes=(1, 0, 0), out_axes=1)(fw, uw, r_ids)
          return self.shard_dim_by_stages(updated_fw, 1, physical_partition_spec=None, is_stage_weight=False)

        return jax.tree.map(_scatter_update_mutables, current_mutables, updated_mutables)

      new_mutables_state = _checkpointed_scatter_update(current_layer_mutables, updated_stage_mutables, repeat_ids)
