...
"""Pipeline V30: Uses axes_scan.scan for broadcast constancy verification.

Replaces jax.lax.scan with flax.core.axes_scan.scan in the inner microbatch
loop. axes_scan.scan pre-traces the body to identify broadcast (loop-invariant)
values, extracts them as constants outside the loop, and only carries
loop-dependent values through jax.lax.scan. This matches Linen's nn.scan
memory behavior where closed-over weights are treated as true constants rather
than being materialized N times in the scan carry.

Key difference from pipeline.py:
  - NNXPipeline: scan_body uses axes_scan.scan with layers_params + layers_metrics
    as broadcast_in (loop-invariant) and (loop_state, layers_mutables) as carry.
  - NNXCircularPipeline: inner_body uses axes_scan.scan with layers_metrics as
    broadcast_in and (loop_state, layers_mutables) as carry. The outer scan
    (repeats) still uses jax.lax.scan since BSW changes per repeat.
"""
...
from flax.core.axes_scan import scan as axes_scan_scan
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V30: axes_scan.scan for broadcast constancy verification ----
    #
    # axes_scan.scan signature:
    #   fn(broadcast_in, carry, *xs) -> (broadcast_out, carry, scan_out)
    #
    # Mapping NNX state to axes_scan roles:
    #   broadcast_in = (layers_params, layers_metrics)  -- loop-invariant weights/metrics
    #   carry        = (loop_state, layers_mutables)    -- changes each iteration
    #   xs           = None (no scanned inputs)         -- dummy scan dimension
    #   scan_out     = new_layer_metrics                -- stacked across iterations
    #
    # axes_scan pre-traces the body to verify broadcast values are true constants,
    # extracts them outside the loop, and only carries loop-dependent values
    # through jax.lax.scan. This matches Linen's nn.scan memory behavior.

    def axes_scan_body(broadcast_in, carry, _xs):
      """Body function for axes_scan.scan.

      Args:
        broadcast_in: (layers_params, layers_metrics) -- loop-invariant state
        carry: (loop_state, layers_mutables) -- per-iteration state
        _xs: dummy scanned input (None broadcast)

      Returns:
        (broadcast_out, carry, scan_out) where:
          broadcast_out = broadcast_in (unchanged)
          carry = (new_loop_state, new_layer_mutables)
          scan_out = new_layer_metrics
      """
      bc_params, bc_metrics = broadcast_in
      ...
      current_layer_state = nnx.State.merge(bc_params, bc_metrics, advanced_mutables)
      ...
      return broadcast_in, (new_loop_state, new_layer_mutables), new_layer_metrics

    if self.config.set_remat_policy_on_pipeline_iterations:
      axes_scan_body = jax.checkpoint(
          axes_scan_body, policy=self.get_pipeline_remat_policy(), prevent_cse=not self.config.scan_pipeline_iterations
      )

    if self.config.scan_pipeline_iterations:
      # axes_scan.scan returns a function: (broadcast_in, carry, *xs) -> (broadcast_out, carry, ys)
      # in_axes=axes_broadcast means the dummy xs input is broadcast (not scanned)
      # out_axes=0 means scan_out is stacked along axis 0
      scan_fn = axes_scan_scan(
          axes_scan_body,
          in_axes=axes_broadcast,
          out_axes=0,
          length=total_iterations,
          unroll=1,
          _split_transpose=False,
          check_constancy_invariants=True,
      )
      broadcast_out, (loop_state, final_layer_mutables), stacked_metrics = scan_fn(
          (layers_params, layers_metrics),  # broadcast_in
          (loop_state, layers_mutables),    # carry (init)
          None,                             # xs (broadcast dummy)
      )
    else:
      current_carry = (loop_state, layers_mutables)
      broadcast_in = (layers_params, layers_metrics)
      metrics_history = []
      for _ in range(total_iterations):
        broadcast_in, current_carry, step_metrics = axes_scan_body(broadcast_in, current_carry, None)
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V30: Nested scan with axes_scan.scan for inner microbatch loop ----
    #
    # outer scan (repeats) -- still jax.lax.scan:
    #   BSW changes per repeat, so it cannot be broadcast. The outer loop uses
    #   jax.lax.scan with bsw_ref closure (same as pipeline.py).
    #
    # inner scan (microbatches) -- axes_scan.scan:
    #   layers_metrics is broadcast (loop-invariant within each repeat).
    #   axes_scan pre-traces the body to verify broadcast constancy and extracts
    #   loop-invariant values as constants outside jax.lax.scan.
    #
    ...
    def inner_axes_scan_body(broadcast_in, carry, _xs):
      """Inner body for axes_scan.scan over microbatches.

      Args:
        broadcast_in: layers_metrics -- loop-invariant within each repeat
        carry: (loop_state, layers_mutables) -- per-iteration state
        _xs: dummy scanned input (broadcast)

      Returns:
        (broadcast_out, carry, scan_out) where:
          broadcast_out = broadcast_in (unchanged metrics)
          carry = (new_loop_state, new_layer_mutables)
          scan_out = new_layer_metrics
      """
      bc_metrics = broadcast_in
      current_loop_state, current_layer_mutables = carry
      ...
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state, bsw_ref[0], layers_graph, bc_metrics,
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      inner_axes_scan_body = jax.checkpoint(
          inner_axes_scan_body, policy=self.get_pipeline_remat_policy(),
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        # axes_scan.scan for inner microbatch loop
        inner_scan_fn = axes_scan_scan(
            inner_axes_scan_body,
            in_axes=axes_broadcast,
            out_axes=0,
        ...
        _bc_out, (new_loop_state, new_layer_mutables), inner_metrics = inner_scan_fn(
            layers_metrics,                                 # broadcast_in
            (current_loop_state, current_layer_mutables),   # carry (init)
            None,                                           # xs (broadcast dummy)
        )
      else:
        inner_carry = (current_loop_state, current_layer_mutables)
        bc_in = layers_metrics
        inner_metrics_list = []
        for _ in range(num_microbatches):
          bc_in, inner_carry, step_metrics = inner_axes_scan_body(bc_in, inner_carry, None)
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        # Bubble also uses axes_scan.scan for consistency
        bubble_scan_fn = axes_scan_scan(
            inner_axes_scan_body,
            in_axes=axes_broadcast,
            out_axes=0,
        ...
        _bc_out, (loop_state, final_layer_mutables), bubble_metrics = bubble_scan_fn(
            layers_metrics,                              # broadcast_in
            (loop_state, final_layer_mutables),          # carry (init)
            None,                                        # xs (broadcast dummy)
        )
      else:
        ...
        for _ in range(bubble_iterations):
          bc_in, bubble_carry, bub_metrics = inner_axes_scan_body(bc_in, bubble_carry, None)
