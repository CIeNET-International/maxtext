...
"""Pipeline V8: Fix vmap return + eliminate metric scatter (Findings 1+5).
...
Variant of pipeline.py combining two fixes:
  ...
  2. (Finding 5) Remove scatter-update of metrics back into [repeats, stages, ...] tensor.
     Instead, return per-stage metrics directly and let the scan collect them via ys output
     (which it already does). Eliminates heavy dynamic_slice_in_dim ops on metrics each iteration.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def get_main_vmap_func_for_iterations(self):
    """V8 override: vmap returns only non-param state to avoid stacking params across stages."""
    def func_to_vmap(graph, state, stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode):
      module = nnx.merge(graph, state)
      out = module(stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode)
  ...
  def run_one_iteration(
    ...
    # V8: vmap now returns (output, (updated_metrics, updated_mutables)) instead of full state
    ...
    # V8 Finding 5: No scatter-update for metrics. Instead of using dynamic_update_slice_in_dim
    # to scatter per-stage metrics back into the full [repeats, stages, ...] tensor, we return
    # them directly. The scan collects metrics via its ys output, so scatter is redundant overhead.
    # Only scatter-update mutables (RNG state), which need to persist across iterations.
    if self.config.num_pipeline_repeats > 1:
      ...
      # V8: Only scatter-update mutables, return metrics directly (no scatter)
