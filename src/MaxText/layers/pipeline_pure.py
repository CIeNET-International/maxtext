...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def scan_body(carry, _):
      ...
      # Mutables are RNG state — carry for correctness (fold_in changes them)
      # but never need gradients. stop_gradient matches Linen's variable_broadcast
      # which excludes non_trainable from backward gradient computation.
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- Adapted pipeline_utils pattern for NNX ----
    #
    # Mirrors pipeline_utils.py's 3-level custom_vjp structure but adapted
    # for NNX module (no Linen Scope). Uses jax.lax.scan + jax.checkpoint
    # instead of nn.scan + nn.remat for the outer loop.
    #
    # Level 1 (L1): run_single_microbatch — jax.remat + jax.vjp on one iteration
    # Level 2 (L2): run_pipeline_microbatches — jax.lax.scan over L1, d+g accumulation
    # Level 3 (L3): execute_pipeline_repeat — weight prefetch + jax.linear_transpose
    #
    # The inner L1/L2 structure matches pipeline_utils.create_gradient_accumulation_scan
    # exactly. L3 matches pipeline_utils.create_pipeline_stage. The outer loop uses
    # jax.lax.scan instead of nn.scan.
    import functools
    ...
    # ---- Adapt run_one_iteration to Linen-like signature ----
    # pipeline_utils expects: model.run_one_iteration(loop_state, bsw, pos, seg, det, mode, lps)
    # NNX version needs: (loop_state, bsw, graph, metrics, mutables, pos, seg, det, mode, lps)
    # Close over the extra args so the inner functions see a simple interface.

    def _nnx_run_one_iteration(loop_state, bsw, positions, segment_ids,
                               deterministic, model_mode, logical_partition_spec,
                               current_layer_mutables):
      """Wrapper matching pipeline_utils interface but handling NNX state."""
      # Mutables are RNG state — need carry for fold_in but never need gradients.
      # stop_gradient matches Linen's variable_broadcast which excludes
      # non_trainable/RNG from backward gradient computation through the scan.
      current_layer_mutables = jax.lax.stop_gradient(current_layer_mutables)
      iteration = loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      new_loop_state, new_layer_state = self.run_one_iteration(
          loop_state, bsw, layers_graph, layers_metrics,
          advanced_mutables, positions, segment_ids, deterministic,
          model_mode, logical_partition_spec,
      )
    ...
    # ---- L1: Single microbatch with remat + vjp ----
    @jax.custom_vjp
    ...
    def run_single_microbatch_fwd(lightweight_state, bsw):
      loop_state, layer_mutables = lightweight_state
      def _run(ls_and_mut, b):
        ls, mut = ls_and_mut
        new_ls, new_mut = _nnx_run_one_iteration(
            ls, b, positions, segment_ids, deterministic,
            model_mode, logical_partition_spec_stripped, mut)
        return (new_ls, new_mut)
      _run_remat = jax.remat(_run, policy=remat_policy)
      out, vjp_fun = jax.vjp(_run_remat, lightweight_state, bsw)
      return out, vjp_fun

    def run_single_microbatch_bwd(res, g_out):
      vjp_fun = res
      d_state, d_bsw = vjp_fun(g_out)
      return d_state, d_bsw

    run_single_microbatch.defvjp(run_single_microbatch_fwd, run_single_microbatch_bwd)

    # ---- L2: Microbatch scan with d+g gradient accumulation ----
    @jax.custom_vjp
    def run_pipeline_microbatches(lightweight_state, bsw):
      return run_pipeline_microbatches_fwd(lightweight_state, bsw)[0]

    def run_pipeline_microbatches_fwd(lightweight_state, bsw):
      final_state, scan_vjp_fun = jax.vjp(
          lambda ls, b: jax.lax.scan(
              lambda carry, _: (run_single_microbatch(carry, b), None),
              ls, None, length=num_microbatches,
          )[0],
          lightweight_state, bsw,
      )
      return (final_state, bsw), scan_vjp_fun

    def run_pipeline_microbatches_bwd(residuals, g_final):
      scan_vjp_fun = residuals
      g_state, g_bsw = g_final
      d_state, d_bsw = scan_vjp_fun(g_state)
      d_bsw = jax.tree.map(
    ...
    # ---- L3: Single repeat with weight prefetch + linear_transpose ----
    @jax.custom_vjp
    def execute_pipeline_repeat(loop_state_and_mut, w_curr, pipeline_params):
      return execute_pipeline_repeat_fwd(loop_state_and_mut, w_curr, pipeline_params)[0]

    def execute_pipeline_repeat_fwd(loop_state_and_mut, w_curr, pipeline_params):
      loop_state, layer_mutables = loop_state_and_mut
      w_next = self.weight_prefetching(
          pipeline_params, physical_partition_spec_full,
      ...
      weight_prefetching_t = jax.linear_transpose(
          p_weight_prefetching, pipeline_params,
      )
      (final_state_and_mut, _), scan_vjp = jax.vjp(
          run_pipeline_microbatches,
          (loop_state, layer_mutables), bsw,
      )
      return (final_state_and_mut, w_next), (scan_vjp, weight_prefetching_t)

    def execute_pipeline_repeat_bwd(residuals, g_outputs):
      g_state_and_mut, g_w_next = g_outputs
      scan_vjp, weight_prefetching_t = residuals
      g_w_curr = jax.tree.map(jnp.zeros_like, g_w_next)
      g_bsw = (g_w_curr, g_w_next)
      g_state_and_mut, g_bsw = scan_vjp((g_state_and_mut, g_bsw))
      g_w_curr, g_w_next = g_bsw
      (g_pipeline_params,) = weight_prefetching_t(g_w_next)
      return g_state_and_mut, g_w_curr, g_pipeline_params

    execute_pipeline_repeat.defvjp(execute_pipeline_repeat_fwd, execute_pipeline_repeat_bwd)

    # ---- Outer loop: scan over repeats ----
    num_repeats = self.config.num_pipeline_repeats
    initial_w_curr = jax.tree.map(lambda x: jnp.zeros(x.shape[1:], dtype=x.dtype), layers_params)

    def outer_body(carry, _):
      loop_state, layer_mutables, w_curr, pipeline_params = carry
      # stop_gradient on mutables: RNG state carried for correctness but
      # excluded from gradient computation, matching variable_broadcast behavior.
      layer_mutables = jax.lax.stop_gradient(layer_mutables)
      (new_loop_state_and_mut, w_next), _ = execute_pipeline_repeat(
          (loop_state, layer_mutables), w_curr, pipeline_params,
      ), None
      (new_loop_state, new_layer_mutables) = new_loop_state_and_mut
      return (new_loop_state, new_layer_mutables, w_next, pipeline_params), None

    if self.config.set_remat_policy_on_pipeline_iterations:
      outer_body = jax.checkpoint(
          outer_body, policy=remat_policy,
    ...
    if bubble_iterations > 0:
      def bubble_body(carry, _):
        loop_state, layer_mutables = carry
        bsw = (final_w_curr, final_w_curr)
        (new_state_and_mut, _) = run_pipeline_microbatches(
            (loop_state, layer_mutables), bsw,
        )
        new_loop_state, new_layer_mutables = new_state_and_mut
        return (new_loop_state, new_layer_mutables), None

      if self.config.scan_pipeline_iterations:
        (loop_state, final_layer_mutables), _ = jax.lax.scan(
            bubble_body, (loop_state, final_layer_mutables), None, length=1
        )
      else:
        (loop_state, final_layer_mutables), _ = bubble_body(
            (loop_state, final_layer_mutables), None
        )
