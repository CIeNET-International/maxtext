...
"""Pipeline V35: Remove replicated weights from outer scan carry.

Instead of carrying w_curr (replicated, ~3.4 GB) through the outer scan,
recompute both w_curr and w_next each repeat via all-gather. This trades
1 extra all-gather per repeat for removing replicated weights from scan carry,
which scan stores per iteration for backward.

Additionally wraps outer_body in jax.checkpoint so the all-gather results
(intermediates) are recomputed in backward, not stored per repeat.

Outer carry: (loop_state, layer_mutables) — no replicated weights.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V35: Recompute weights each repeat, no w_curr in carry ----
    #
    # outer scan (repeats):
    #   1. Recompute BOTH w_curr and w_next via all-gather (2 all-gathers per repeat)
    #   2. Pass BSW to make_inner_body() — captured as closure (not carry)
    #   3. Run inner scan over microbatches
    #   4. jax.checkpoint wraps outer_body — recomputes in backward
    #
    # inner scan (microbatches):
    #   1. BSW captured via closure from make_inner_body
    #   2. Run one pipeline iteration (forward through decoder)
    #   3. jax.checkpoint wraps inner body — recomputes forward during backward
    #
    # Why recompute w_curr instead of carrying it:
    #   Baseline carries w_curr (~3.4 GB replicated) in outer scan carry.
    #   Scan stores ALL intermediate carry values for backward.
    #   Removing w_curr from carry saves ~3.4 GB × num_repeats.
    #   Cost: 1 extra all-gather per repeat (recomputing w_curr).
    num_microbatches = self.config.num_pipeline_microbatches

    def make_inner_body(bsw):
      """Factory: inner scan body with BSW captured as closure."""
    ...
    def outer_body(carry, _):
      """One repeat: recompute both w_curr and w_next (no w_curr in carry)."""
      current_loop_state, current_layer_mutables = carry
      iteration = current_loop_state["loop_iteration"]

      # Recompute BOTH current and next weights each repeat.
      # w_curr: all-gather for the current repeat's weights
      # w_next: all-gather for the next repeat's weights (standard prefetch)
    ...
    # Wrap outer body in jax.checkpoint: w_curr/w_next/inner_scan recomputed in backward
    if self.config.set_remat_policy_on_pipeline_iterations:
    ...
    if bubble_iterations > 0:
      ...
      final_w = self.from_repeat_weights_to_bsw(
          bubble_repeat_weights, physical_partition_spec_full
      )
      bubble_body = make_inner_body((final_w, final_w))
