...
"""V65k: V65i base (raw jax.lax.scan, no custom_vjp at scan level) + carry SHARDED repeat weights.

Based on pipeline_v65i.py (NNXCircularPipeline) with the following change:
  - Instead of carrying fully-gathered BSW in the scan, carry only the SHARDED
    repeat weights (output of from_all_variables_to_repeat_weights). This
    dramatically reduces carry size since sharded weights are 1/N the size of
    fully-gathered weights (where N = FSDP parallelism factor).

Carry structure: (loop_state, rng_mutables, sharded_prev_repeat_weights)

Per outer iteration:
  1. All-gather carried sharded weights -> w_curr (via from_repeat_weights_to_bsw)
     - For iteration 0: use zeros (via jax.lax.cond to avoid computing both branches)
  2. Prefetch next: sharded_next = from_all_variables_to_repeat_weights(layers_params, total_iter + 1)
     (dynamic_slice only, no communication)
  3. All-gather next: w_next = from_repeat_weights_to_bsw(sharded_next, spec)
  4. bsw = (w_curr, w_next)
  5. Return (new_loop_state, updated_rng, sharded_next) as carry

Key: from_all_variables_to_repeat_weights does dynamic_slice (no communication).
     from_repeat_weights_to_bsw does all-gather (communication).
     By carrying the sharded output of the first, we avoid carrying the full unsharded weights.
"""
...
class NNXCircularPipelineV65k(NNXPipelineBase):
  """V65k: Circular pipeline carrying SHARDED repeat weights in scan carry.

  Key optimization over V65i NNXCircularPipeline:
    - Carry only sharded repeat weights (output of from_all_variables_to_repeat_weights,
      which uses dynamic_slice with no communication) instead of fully-gathered BSW.
    - Per outer iteration: all-gather the carried sharded weights to get w_curr,
      prefetch next sharded weights via dynamic_slice, all-gather to get w_next.
    - BSW = (w_curr, w_next) is constructed fresh each iteration from smaller
      sharded carry, reducing scan carry size by FSDP parallelism factor.

  Preserves all V65i features:
    - No custom_vjp at scan level
    - jax.checkpoint on outer + inner bodies
    - Raw jax.lax.scan
  """
  ...
  def from_all_variables_to_repeat_weights(self, weights_state, loop_iteration):
    ...
    This is a LOCAL operation (dynamic_slice only, no communication / all-gather).
    Returns sharded repeat weights with the repeat dimension removed.
    """
    if self.config.num_pipeline_repeats == 1:
  ...
  def from_repeat_weights_to_bsw(
    ...
    This is a COMMUNICATION operation (all-gather over FSDP axes).
    """
  ...
  def __call__(
    ...
    def execute_one_repeat(carry, repeat_index):
      """Outer scan body: one repeat iteration.

      V65k change: carry includes sharded_prev_repeat_weights.
      Per iteration:
        1. All-gather carried sharded weights -> w_curr (skip for iteration 0 via jax.lax.cond)
        2. Prefetch next sharded weights (dynamic_slice, no communication)
        3. All-gather next -> w_next
        4. BSW = (w_curr, w_next)
        5. Return sharded_next as new carry element
      """
      loop_state, rng_mutables, sharded_prev_repeat_weights = carry

      total_iter = repeat_index * num_microbatches

      # Step 1: All-gather carried sharded weights -> w_curr
      # For iteration 0, there are no valid previous weights, so use zeros.
      # Use jax.lax.cond to avoid computing the all-gather branch when not needed.
      def _allgather_prev():
        return self.from_repeat_weights_to_bsw(sharded_prev_repeat_weights, physical_partition_spec_full)

      def _zeros_like_bsw():
        return jax.tree.map(lambda x: jnp.zeros(x.shape, dtype=x.dtype), sharded_prev_repeat_weights)
      ...
      # Step 2: Prefetch next sharded weights (dynamic_slice only, no communication)
      sharded_next = self.from_all_variables_to_repeat_weights(layers_params, total_iter)

      # Step 3: All-gather next sharded weights -> w_next
      w_next = self.from_repeat_weights_to_bsw(sharded_next, physical_partition_spec_full)

      # Step 4: Construct BSW from w_curr and w_next
      buffer_sliding_window = (w_curr, w_next)
      ...
      # Fix loop_iteration: set to repeat_index * num_microbatches (not total_iterations)
      loop_state = {**loop_state, "loop_iteration": total_iter}
      (new_loop_state, updated_rng_mutables), repeat_step_metrics = run_microbatch_scan(
          loop_state, rng_mutables, buffer_sliding_window
      )

      # Step 5: Return sharded_next as new carry element
      return (new_loop_state, updated_rng_mutables, sharded_next), repeat_step_metrics
    ...
    # ---- Initial carry: extract repeat 0's sharded weights (dynamic_slice, no communication) ----
    sharded_prev_initial = self.from_all_variables_to_repeat_weights(layers_params, 0)

    # ---- Execute: outer scan over repeats ----
    num_repeats = self.config.num_pipeline_repeats

    (loop_state, final_rng_mutables, _final_sharded_weights), repeat_metrics = jax.lax.scan(
        checkpointed_repeat_body,
        (loop_state, layers_mutables, sharded_prev_initial),
    ...
    if bubble_iterations > 0:
      ...
      # For bubble iterations, use the last repeat's weights (already sharded from carry).
      # All-gather them once for the bubble BSW.
      last_repeat_sharded = self.from_all_variables_to_repeat_weights(
          layers_params, total_iteration_after_repeats - 1
      )
      last_repeat_weights = self.from_repeat_weights_to_bsw(last_repeat_sharded, physical_partition_spec_full)
...
) -> NNXCircularPipelineV65k:
  """Factory function to instantiate the V65k NNX Pipeline module."""
  return NNXCircularPipelineV65k(
...
def create_pipeline(
  ...
  """Returns the ToLinen-wrapped V65k NNX pipeline.
