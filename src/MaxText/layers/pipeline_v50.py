...
================================================================================
Goal: close the 6.6 GB compile-time memory gap between the NNX circular pipeline
(29.9 GB) and the Linen circular pipeline (23.3 GB). Linen's ``_partial_pack``
emits real, separate XLA computations (8 HLO while loops). The NNX port using
plain ``jax.lax.scan`` + ``jax.checkpoint`` only emits 4. The hypothesis for V50
is that wrapping the per-microbatch ``inner_body`` in a *named call* forces JAX
to stage it out as a distinct jaxpr/HLO sub-computation (a real function
boundary), instead of inlining it into the surrounding scan body.

PRIMITIVE SELECTION (verified empirically on this JAX 0.9.2 install):
  - ``jax.named_call`` STILL EXISTS, but in JAX 0.9.2 it has been reduced to a
    pure name-stack annotation:
        ``return source_info_util.extend_name_stack(name)(fun)``
    It NO LONGER emits a ``core.closed_call_p`` primitive. ``make_jaxpr`` of a
    ``named_call``-wrapped function shows the body fully INLINED (raw ``mul`` /
    ``add`` ops, no call boundary) -- identical to ``jax.jit(inline=True)``.
    Using it would repeat the V45 mistake (inline => zero effect on memory).
  - ``jax.jit(inline=False)`` DOES emit a real sub-computation: ``make_jaxpr``
    shows a nested ``jit[ name=... jaxpr={...} ]`` equation -- a genuine
    pjit/jit call primitive that is NOT inlined. This is the actual function
    boundary V50 needs.
  => Per the task's documented fallback ("if it truly does not [emit a call],
     fall back to functools.partial(jax.jit, inline=False)"), V50 uses
     ``jax.jit(inline=False)``.

APPLICATION POINT:
  - ``inner_body`` (the per-microbatch pipeline iteration) is wrapped in
    ``jax.jit(inline=False)``. This is the body executed num_microbatches times
    per repeat -- making it a named sub-computation is what produces a distinct
    HLO function, mirroring Linen's ``_partial_pack`` boundary.
  - Compose order: ``jax.checkpoint`` is applied FIRST (innermost), then the
    ``jax.jit(inline=False)`` boundary OUTSIDE it, i.e.
        inner_body = jax.jit(jax.checkpoint(inner_body), inline=False)
    so the named-call boundary contains the rematerialized region.

CORRECTNESS:
  ``jax.jit(inline=False)`` is a pure structural transform -- it changes neither
  the math nor the gradients. Loss at step 2 must still match the baseline
  (0.049). See ``test_v50_compare.sh``.
================================================================================
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  V50: ``inner_body`` is wrapped in ``jax.jit(inline=False)`` so each
  per-microbatch pipeline iteration is staged as a distinct named jit/pjit
  sub-computation (a real HLO function boundary), instead of being inlined
  into the surrounding ``jax.lax.scan`` body. ``jax.named_call`` in JAX 0.9.2
  is only a name-stack annotation and gets inlined, so it cannot be used.
  """
  ...
  def __call__(
    ...
    # ---- V50: jax.named_call -> jax.jit(inline=False) function boundary ----
    # Wrap inner_body (per-microbatch body) so it is staged out as a distinct
    # named jit/pjit sub-computation -- a real jaxpr/HLO function boundary --
    # rather than being inlined into the surrounding jax.lax.scan body.
    #
    # jax.named_call exists in JAX 0.9.2 but is now only a name-stack
    # annotation (source_info_util.extend_name_stack) and gets fully inlined
    # in the jaxpr -- identical to jax.jit(inline=True), zero structural
    # effect (the V45 mistake). jax.jit(inline=False) instead emits a real,
    # non-inlined jit/pjit call equation, so V50 uses that.
    #
    # Compose order: jax.checkpoint (above) is innermost; the
    # jax.jit(inline=False) boundary is applied OUTSIDE it here, so the
    # named-call boundary wraps the rematerialized region.
    inner_body = jax.jit(inner_body, inline=False)
