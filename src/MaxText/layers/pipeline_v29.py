...
"""Pipeline V29: Circular pipeline with NNX-native variable group separation.

Variant of pipeline.py that uses scan_with_variable_groups for the inner
microbatch scan in NNXCircularPipeline. This replicates Linen's _partial_pack
+ axes_scan mechanism: params are explicit inputs to jax.remat (not closures),
matching the gradient accumulation pattern that Linen nn.scan(nn.remat(body))
produces.

Only the inner scan (microbatches) is modified. The outer scan (repeats) and
all other logic remain identical to pipeline.py.
"""
...
from maxtext.utils.nnx_scan_utils import scan_with_variable_groups
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- Nested scan structure (V29: variable group separation) ----
    #
    # V29 key change: the inner microbatch scan uses scan_with_variable_groups
    # to replicate Linen's _partial_pack + axes_scan mechanism:
    #
    #   1. layers_params are the "broadcast" group -- constant across inner
    #      scan iterations, passed as EXPLICIT first arg to jax.remat
    #   2. layers_mutables (RNG state) are the "carry" group -- threaded
    #      through scan carry
    #   3. jax.remat sees remat_body(broadcast_params, carry_mutables, ...)
    #      as a pure function with explicit param input -- NOT closure captures
    #
    # This gives XLA the same visibility into gradient accumulation paths that
    # Linen's nn.scan(nn.remat(body)) provides. The BSW mechanism (dual buffer
    # weight caching) is unchanged -- BSW is still set by outer_body and read
    # from bsw_ref closure. The difference is HOW the params participate in
    # the remat/scan nesting.
    #
    # outer scan (repeats) -- UNCHANGED from pipeline.py:
    #   1. All-gather weights once -> BSW
    #   2. Store BSW in bsw_ref[0] (closure, NOT carry -- carry would OOM)
    #   3. Run inner scan over microbatches via scan_with_variable_groups
    #
    # inner scan (microbatches) -- V29 change:
    #   1. scan_with_variable_groups splits layers into broadcast (params)
    #      and carry (mutables) groups
    #   2. Broadcast params become explicit first arg to remat wrapper
    #   3. BSW still read from bsw_ref[0] for the actual forward computation
    #   4. Gradient paths through params are explicit to XLA (not closures)
    bsw_ref = [None]
    num_microbatches = self.config.num_pipeline_microbatches

    # ---- V29 inner scan: explicit variable group separation ----
    #
    # We construct the inner scan using the pattern from scan_with_variable_groups
    # MANUALLY here (rather than calling the utility directly) because the
    # circular pipeline's inner body has complex interactions:
    #   - BSW closure from outer_body
    #   - _advance_rng_state for dropout masks
    #   - _is_static_param filter (not just nnx.Param)
    #   - layers_graph / layers_metrics from outer scope
    #
    # The CRITICAL difference from pipeline.py's inner_body:
    #   OLD: jax.checkpoint(inner_body, ...) where inner_body captures
    #        layers_params as a CLOSURE -- remat doesn't see them as inputs
    #   NEW: jax.remat(remat_inner, ...) where remat_inner takes
    #        broadcast_params as EXPLICIT FIRST ARG -- remat sees the
    #        gradient path through params clearly

    def remat_inner(broadcast_params, carry_mutables, carry_loop_state, _xs_i):
      """Pure function with explicit broadcast params for jax.remat.

      This is the V29 equivalent of pipeline.py's inner_body, but structured
      so that params are an EXPLICIT argument rather than a closure capture.
      This matches Linen's _partial_pack: remat sees the param input directly,
      giving XLA cleaner gradient accumulation paths.

      Args:
        broadcast_params: nnx.State of params (broadcast group). Constant
            across scan iterations. EXPLICIT arg to remat (not closure).
        carry_mutables: nnx.State of mutables/RNG state (carry group).
            Changes each iteration via _advance_rng_state.
        carry_loop_state: dict with shift, state_io, circ_storage, etc.
            Threaded through scan carry.
        _xs_i: unused scan input slice (always None for this scan).

      Returns:
        (new_loop_state, new_carry_mutables, inner_metrics)
      """
      carry_mutables = jax.lax.stop_gradient(carry_mutables)
      iteration = carry_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(carry_mutables, iteration)

      # BSW still read from closure (set by outer_body). This is unchanged
      # from pipeline.py -- BSW cannot be in carry (OOM) or remat args
      # (too large). The V29 change is about layers_params, not BSW.
      new_loop_state, new_layer_state = self.run_one_iteration(
          carry_loop_state, bsw_ref[0], layers_graph, layers_metrics,
    ...
    # Step 3 (from scan_with_variable_groups pattern): wrap in jax.remat.
    # broadcast_params is the EXPLICIT first arg -- not a closure.
    remat_policy = (
        self.get_pipeline_remat_policy()
        if self.config.set_remat_policy_on_pipeline_iterations
        else None
    )
    if remat_policy is not None:
      remat_inner_wrapped = jax.remat(remat_inner, policy=remat_policy)
    else:
      remat_inner_wrapped = remat_inner

    # Step 4 (from scan_with_variable_groups pattern): scan body.
    # layers_params (broadcast) is CLOSED OVER the scan -- not in carry.
    # layers_mutables (carry) threads through scan carry.
    def inner_scan_body(carry, xs_i):
      carry_mutables, carry_loop_state = carry
      new_loop_state, new_carry_mutables, inner_metrics = remat_inner_wrapped(
          layers_params,  # CLOSED OVER -- explicit arg to remat, not in carry
          carry_mutables,
          carry_loop_state,
          xs_i,
      )
      return (new_carry_mutables, new_loop_state), inner_metrics
    ...
    def outer_body(carry, _):
      ...
      if self.config.scan_pipeline_iterations:
        # V29: inner scan with explicit variable group separation
        (new_layer_mutables, new_loop_state), inner_metrics = jax.lax.scan(
            inner_scan_body,
            (current_layer_mutables, current_loop_state),
            None, length=num_microbatches,
        )
      else:
        inner_carry = (current_layer_mutables, current_loop_state)
        inner_metrics_list = []
        for _ in range(num_microbatches):
          inner_carry, step_metrics = inner_scan_body(inner_carry, None)
          inner_metrics_list.append(step_metrics)
        new_layer_mutables, new_loop_state = inner_carry
        inner_metrics = (
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        (final_layer_mutables, loop_state), bubble_metrics = jax.lax.scan(
            inner_scan_body, (final_layer_mutables, loop_state), None, length=bubble_iterations
        )
      else:
        bubble_carry = (final_layer_mutables, loop_state)
        bubble_metrics_list = []
        for _ in range(bubble_iterations):
          bubble_carry, bub_metrics = inner_scan_body(bubble_carry, None)
          bubble_metrics_list.append(bub_metrics)
        final_layer_mutables, loop_state = bubble_carry
        bubble_metrics = (
