...
V65: NNX circular pipeline using inline lifted scan (nnx_lifted_scan pattern).
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def _stage_fn_for_scope(scope, carry):
      ...
      # Read params from scope (carried across repeats manually via custom_vjp)
      ...
      def _run_microbatches_fwd(loop_state_arg, bsw_arg, muts_arg,
        ...
        def scan_fn(ls_arg, b_arg):
          # inner_body defined INSIDE scan_fn so it uses b_arg (positional),
          # NOT bsw_arg (closure). jax.vjp tracks gradients through positional
          # args only — using bsw_arg would produce zero BSW gradient.
      ...
      def _execute_stage_bwd(residuals, g_out):
        ...
        return d_ls, d_bsw[0], g_params, None, None, None, None
      ...
      # IDENTITY PASS-THROUGH for params: write the exact same arrays back.
      # This allows JAX to accumulate gradients in-place on the carry buffer.
      for k, a in zip(param_keys, local_param_arrays):
        scope.put_variable('params', k, a)
    ...
    scanned_stage = flax_lift.scan(
        ...
        variable_carry=['carry_state', 'params'],
        variable_axes={},
        split_rngs={},
        length=num_repeats,
    )
