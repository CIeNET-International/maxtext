...
V52: Two-level @jax.custom_vjp matching Linen's pipeline_utils.py pattern.
Level 1 (per-microbatch): jax.remat + jax.vjp around run_one_iteration
Level 2 (per-repeat): jax.linear_transpose for reduce_scatter + jax.vjp for inner scan
BSW passed as explicit argument (no mutable list closure).
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  """NNX circular pipeline with two-level custom_vjp matching Linen's pattern.

  V52: Two-level @jax.custom_vjp matching Linen's pipeline_utils.py pattern.

  Level 1 (per-microbatch): run_iteration_custom
    - Wraps run_one_iteration with jax.remat + jax.vjp
    - Matches Linen's run_single_microbatch_custom from pipeline_utils.py
    - Stops gradients for positions/segment_ids (None cotangents)

  Level 2 (per-repeat): execute_repeat_pure
    - Wraps weight_prefetching + inner scan over microbatches
    - Uses jax.linear_transpose for reduce_scatter (weight grads)
    - Uses jax.vjp over inner scan to capture backward closure
    - Matches Linen's execute_pipeline_stage_pure from pipeline_utils.py

  Key design changes from previous versions:
    - BSW passed as explicit argument to scan_microbatches (not closure)
    - layers_params passed as explicit arg to execute_repeat_pure (not closure)
    - No bsw_ref mutable list -- eliminates UnexpectedTracerError risk
    - jax.checkpoint NOT used separately -- Level 1 custom_vjp has jax.remat
  """
  ...
  def __call__(
    ...
    # ---- Nested scan structure (V52: Level 2 custom_vjp only) ----
    #
    # Architecture:
    #   - jax.checkpoint on inner body (same as base pipeline)
    #   - Level 2 @custom_vjp per repeat: jax.vjp captures full scan backward,
    #     jax.linear_transpose routes weight gradients via reduce_scatter
    #   - NO Level 1 custom_vjp (nnx.State.merge gradient reconstruction
    #     produces pytree mismatch with vjp_fn expected structure)
    #
    # BSW passed as explicit argument to scan_microbatches.
    # layers_params passed as explicit arg to execute_repeat_pure.
    num_microbatches = self.config.num_pipeline_microbatches
    remat_policy = self.get_pipeline_remat_policy()

    # --- Inner scan function (BSW as explicit arg, checkpoint on body) ---
    def scan_microbatches(loop_state, bsw, layer_mutables):
      def _inner_body(carry, _):
        ...
        advanced_mutables = _advance_rng_state(muts, iteration)
        new_ls, new_layer_state = self.run_one_iteration(
            ls, bsw, layers_graph, layers_metrics,
        ...
        return (new_ls, new_layer_mutables), new_layer_metrics

      if self.config.set_remat_policy_on_pipeline_iterations:
        _inner_body = jax.checkpoint(
            _inner_body, policy=remat_policy,
            prevent_cse=not self.config.scan_pipeline_iterations,
        )

      if self.config.scan_pipeline_iterations:
        return jax.lax.scan(
            _inner_body, (loop_state, layer_mutables),
            None, length=num_microbatches,
        )
      else:
        carry = (loop_state, layer_mutables)
        metrics_list = []
        for _ in range(num_microbatches):
          carry, m = _inner_body(carry, None)
    ...
    # --- Level 2: per-repeat custom_vjp ---
    # jax.vjp captures the full inner scan backward. jax.linear_transpose
    # routes weight gradients via reduce_scatter (all-gather dual).
    @jax.custom_vjp
    ...
    def _execute_repeat_fwd(loop_state, layer_mutables, w_curr, pipeline_weights):
      ...
      weight_prefetching_t = jax.linear_transpose(
          p_weight_prefetching, pipeline_weights
      )
      ...
      return (new_loop_state, new_layer_mutables, w_next, scan_metrics), \
             (scan_vjp_fn, weight_prefetching_t)

    def _execute_repeat_bwd(residuals, g_outputs):
      scan_vjp_fn, weight_prefetching_t = residuals
      g_loop_state, g_layer_mutables, g_w_next, g_scan_metrics = g_outputs

      g_init_ls, g_bsw, g_init_muts = scan_vjp_fn(
          ((g_loop_state, g_layer_mutables), g_scan_metrics)
      )

      g_w_curr_from_scan, g_w_next_from_scan = g_bsw
      g_w_next_total = jax.tree.map(
          lambda a, b: a + b, g_w_next, g_w_next_from_scan
      )
    ...
    # Matches Linen: bubble uses Level 2 custom_vjp with weight_prefetching_t
    # so that bsw[1] gradient routes through the bubble's own reduce-scatter
    # (not through the outer carry's weight_prefetching_t from the last repeat).
    if bubble_iterations > 0:

      @jax.custom_vjp
      def execute_bubble_pure(loop_state, layer_mutables, w_curr, pipeline_weights):
        return _execute_bubble_fwd(loop_state, layer_mutables, w_curr, pipeline_weights)[0]

      def _execute_bubble_fwd(loop_state, layer_mutables, w_curr, pipeline_weights):
        iteration = loop_state["loop_iteration"]
        # Prefetch w_next for bubble (same as Linen's bubble stage)
        w_next = self.weight_prefetching(
            pipeline_weights, physical_partition_spec_full, iteration
        )
        bsw_bubble = (w_curr, w_next)
        ...
        def bubble_scan_fn(ls, bsw, muts):
          def _bubble_inner(carry, _):
            ls_i, muts_i = carry
            muts_i = jax.lax.stop_gradient(muts_i)
            iter_i = ls_i["loop_iteration"]
            advanced_muts = _advance_rng_state(muts_i, iter_i)
            new_ls_i, new_layer_state_i = self.run_one_iteration(
                ls_i, bsw, layers_graph, layers_metrics,
                advanced_muts, positions, segment_ids,
                deterministic, model_mode, logical_partition_spec_stripped,
            )
            _, _, new_metrics_i, new_muts_i = nnx.split(
                new_layer_state_i, _is_static_param, nnx.Intermediate, ...
            )
            return (new_ls_i, new_muts_i), new_metrics_i

          if self.config.set_remat_policy_on_pipeline_iterations:
            _bubble_inner_fn = jax.checkpoint(
                _bubble_inner, policy=remat_policy,
                prevent_cse=not self.config.scan_pipeline_iterations,
            )
          else:
            _bubble_inner_fn = _bubble_inner

          if self.config.scan_pipeline_iterations:
            return jax.lax.scan(
                _bubble_inner_fn, (ls, muts), None, length=bubble_iterations,
            )
          else:
            carry = (ls, muts)
            metrics_list = []
            for _ in range(bubble_iterations):
              carry, m = _bubble_inner_fn(carry, None)
              metrics_list.append(m)
            metrics = (
                jax.tree.map(lambda *xs: jnp.stack(xs), *metrics_list)
                if metrics_list else layers_metrics
        ...
        (scan_carry, scan_metrics), scan_vjp_fn = jax.vjp(
            bubble_scan_fn, loop_state, bsw_bubble, layer_mutables
        )
        new_loop_state, new_layer_mutables = scan_carry
      ...
      def _execute_bubble_bwd(residuals, g_outputs):
        scan_vjp_fn, weight_prefetching_t = residuals
        g_loop_state, g_layer_mutables, g_w_next, g_scan_metrics = g_outputs
      ...
      execute_bubble_pure.defvjp(_execute_bubble_fwd, _execute_bubble_bwd)

      # Execute bubble via Level 2 custom_vjp (same structure as repeats)
      new_ls, new_muts, _, bubble_metrics = execute_bubble_pure(
          loop_state, final_layer_mutables, final_w_curr, layers_params
      )
      loop_state = new_ls
      final_layer_mutables = new_muts
