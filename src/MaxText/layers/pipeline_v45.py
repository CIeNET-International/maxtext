...
"""Pipeline V45: NNX-native _partial_pack via nested jit trace boundary.

Attempts to replicate the scope isolation that _partial_pack achieves in
nn.scan(nn.remat(body)) using pure JAX + NNX. The key mechanism is
@functools.partial(jax.jit, inline=True) on the repeat body, which creates
a separate trace (jaxpr function call) without a separate compilation unit.

Hypothesis: the 8 HLO while loops in Linen come from _partial_pack creating
isolated scopes (parent=None) that produce nested function boundaries in the
jaxpr. jit(inline=True) on the outer_body achieves a similar trace-level
separation, making broadcast params (layers_params_raw) an explicit arg
rather than a closure constant.

layers_params_raw (stripped of nnx.Variable wrappers) is passed as an
explicit argument to the jit'd function rather than captured as a closure.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V45: NNX-native _partial_pack via nested jit trace boundary ----
    #
    # Key idea: wrap the outer_body in @jax.jit(inline=True) so that
    # layers_params_raw becomes an EXPLICIT argument to a separate jaxpr
    # function, rather than a closure constant. This creates a trace-level
    # boundary analogous to _partial_pack's scope_fn(parent=None).
    #
    # The inner function sees broadcast params as a traced input (like
    # variable_broadcast freezes params and passes them into an isolated scope).
    # jit(inline=True) does NOT create a separate compilation unit — it only
    # creates a separate trace (jaxpr function call boundary).
    #
    # Why this might help:
    #   - _partial_pack creates nested jaxpr boundaries (scope_fn calls)
    #   - These boundaries cause XLA to see nested while loops (8 total)
    #   - jit(inline=True) creates a similar jaxpr function boundary
    #   - XLA may then schedule memory differently for the nested structure

    num_microbatches = self.config.num_pipeline_microbatches
    num_repeats = self.config.num_pipeline_repeats
    pipeline_self = self

    # Strip nnx.Variable wrappers for clean JAX arrays — jit'd functions
    # work with raw arrays, not nnx.Variable containers.
    layers_params_raw = jax.tree.map(
        lambda x: x.value if isinstance(x, nnx.Variable) else x,
        layers_params,
        is_leaf=lambda x: isinstance(x, nnx.Variable),
    )

    # BSW ref for bubble iterations (outside jit boundary)
    bsw_ref = [None]

    # ---- Isolated repeat body via jit(inline=True) ----
    # This creates a separate trace (jaxpr function call) for the repeat body.
    # broadcast_params is an explicit arg, NOT a closure — mimicking
    # _partial_pack's scope isolation where params are frozen and passed in.
    @functools.partial(jax.jit, inline=True)
    def _isolated_repeat(broadcast_params, carry_state):
      """Isolated trace: broadcast_params is explicit, creating a jaxpr boundary.

      This mirrors _partial_pack which:
        1. Extracts broadcast vars from scope
        2. Creates inner scope with parent=None
        3. Passes frozen vars as explicit inputs to scope_fn

      Here, broadcast_params plays the role of the frozen broadcast vars,
      and the jit boundary plays the role of the parent=None scope.
      """
      current_loop_state, current_layer_mutables, w_curr = carry_state
      iteration = current_loop_state["loop_iteration"]

      # Weight prefetching — uses broadcast_params (the explicit arg)
      # Reconstruct layers_params with Variable wrappers for API compatibility
      layers_params_wrapped = jax.tree.map(
          lambda raw, template: template.replace(value=raw) if isinstance(template, nnx.Variable) else raw,
          broadcast_params,
          layers_params,
          is_leaf=lambda x: isinstance(x, nnx.Variable),
      )
      w_next = pipeline_self.weight_prefetching(
          layers_params_wrapped, physical_partition_spec_full, iteration
      )
      bsw = (w_curr, w_next)

      # Inner microbatch body
      def inner_body(carry, _):
        cur_ls, cur_mut = carry
        cur_mut = jax.lax.stop_gradient(cur_mut)
        iter_val = cur_ls["loop_iteration"]
        adv_mut = _advance_rng_state(cur_mut, iter_val)
      ...
      if pipeline_self.config.set_remat_policy_on_pipeline_iterations:
        inner_body = jax.checkpoint(
            inner_body,
            policy=pipeline_self.get_pipeline_remat_policy(),
            prevent_cse=not pipeline_self.config.scan_pipeline_iterations,
        )

      if pipeline_self.config.scan_pipeline_iterations:
        ...
        for _ in range(num_microbatches):
          inner_carry, step_m = inner_body(inner_carry, None)
          inner_metrics_list.append(step_m)
        new_ls, new_mut = inner_carry
        inner_metrics = (
      ...
      return (new_ls, new_mut, w_next), inner_metrics

    # Outer scan: layers_params_raw as explicit broadcast arg (not closure)
    def outer_body(carry, _):
      return _isolated_repeat(layers_params_raw, carry)
    ...
    if bubble_iterations > 0:
      ...
      if self.config.scan_pipeline_iterations:
        ...
        for _ in range(bubble_iterations):
          bubble_carry, bub_m = bubble_body(bubble_carry, None)
          bubble_metrics_list.append(bub_m)
