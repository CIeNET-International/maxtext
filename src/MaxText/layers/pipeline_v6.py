...
"""Pipeline V6: Fix vmap return (Finding 1 - no param stacking).

Variant of pipeline.py targeting root cause #1: jax.vmap state stacking bloat.
The vmapped function no longer returns full nnx.state(module). Instead it returns
ONLY non-param state (metrics + mutables). Params don't change in the forward pass
(handled by AD), so returning them in vmap output wastes memory by stacking across
stages. run_one_iteration reconstructs the full state from the partial return.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def get_main_vmap_func_for_iterations(self):
    """V6 override: vmap returns only non-param state to avoid stacking params across stages.

    Params don't change during forward (handled by AD/gradient), so including them
    in vmap output causes XLA to stack param-sized tensors across all stages,
    wasting memory proportional to num_stages * param_size.
    """
    def func_to_vmap(graph, state, stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode):
      module = nnx.merge(graph, state)
      out = module(stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode)
      # V6: Only return non-param state -- params are static in forward, no need to stack
  ...
  def run_one_iteration(
    ...
    # V6: vmap now returns (output, (updated_metrics, updated_mutables)) instead of full state
    ...
    # V6: No need to nnx.split updated_stage_weights_state -- we already have the non-param parts
    if self.config.num_pipeline_repeats > 1:
