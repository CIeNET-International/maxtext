...
V64: lift.scan over REPEATS + lift.checkpoint wrapping stage_fn that CONTAINS
jax.lax.scan over microbatches + custom_vjp per microbatch.

Matches Linen's EXACT nesting:
  lift.scan(REPEATS) → lift.checkpoint(stage_fn containing jax.lax.scan(MICROBATCHES))

The remat body CONTAINS the microbatch scan → separate recomputation while loop.
The custom_vjp is opaque to partial eval → zero internal residuals.
_partial_pack scope isolation prevents tracer leaks.

Based on V62/V63 which use flax.core.apply + lift.scan + lift.checkpoint for _partial_pack
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- V64: lift.scan(REPEATS) + lift.checkpoint(stage_fn containing scan) ----
    #
    # Matches Linen's exact architecture:
    #   lift.scan over REPEATS (not microbatches)
    #   lift.checkpoint wraps stage_fn which CONTAINS jax.lax.scan over microbatches
    #   custom_vjp per microbatch inside the scan (opaque barrier)
    #
    # This produces: jax.remat(fn_containing_jax.lax.scan) → separate recomp while loop
    # Combined with custom_vjp opacity → minimal recomp carry (like Linen's 5 elements)
    ...
    # ---- Flatten NNX state for Linen scope integration ----
    ...
    # Flatten ALL JAX arrays for scope — _partial_pack only isolates scope variables.
    # Anything left as a closure capture becomes a DynamicJaxprTracer leak.
    param_arrays, param_treedef, param_is_var, param_var_types, param_lwp = _flatten_nnx_state(layers_params)
    param_keys = [f'p{i}' for i in range(len(param_arrays))]

    met_t_arrays, met_t_treedef, met_t_is_var, met_t_var_types, met_t_lwp = _flatten_nnx_state(layers_metrics)
    met_t_keys = [f't{i}' for i in range(len(met_t_arrays))]
    ...
    # ---- Per-microbatch custom_vjp (opaque barrier to partial eval) ----
    @jax.custom_vjp
    ...
    # ---- Stage function: contains microbatch scan (called per repeat) ----
    # This is what lift.checkpoint wraps. It CONTAINS jax.lax.scan.
    # -> jax.remat(stage_fn_containing_scan) -> separate recomp while loop
    #
    # CRITICAL: ALL JAX arrays read from scope, not closure. _partial_pack at
    # both lift.scan and lift.checkpoint boundaries converts scope vars to
    # positional args — prevents DynamicJaxprTracer leaks across trace levels.
    def _stage_fn_for_scope(scope, carry):
      loop_state, w_curr = carry

      # Read params from scope (broadcast — loop invariant, isolated by _partial_pack)
      local_param_arrays = [scope.get_variable('params', k) for k in param_keys]
      local_layers_params = _unflatten_nnx_state(
          local_param_arrays, param_treedef, param_is_var, param_var_types, param_lwp
      )
      ...
      local_met_arrays = [scope.get_variable('metrics_template', k) for k in met_t_keys]
      local_layers_metrics = _unflatten_nnx_state(
          local_met_arrays, met_t_treedef, met_t_is_var, met_t_var_types, met_t_lwp
      )

      # Weight prefetching (scope-provided params)
      iteration = loop_state["loop_iteration"]
      w_next = self.weight_prefetching(
          local_layers_params, physical_partition_spec_full, iteration
      )
      ...
      cur_mut_arrays = _linen_collection_to_arrays(carry_state_dict, mut_keys)
      current_layer_mutables = _unflatten_nnx_state(
          cur_mut_arrays, mut_treedef, mut_is_var, mut_var_types, mut_lwp
      )

      # Per-microbatch custom_vjp defined LOCALLY — all JAX captures come from
      # scope reads above (at current trace level), not outer-scope closures.
      @jax.custom_vjp
      def _run_iter_local(loop_state_arg, bsw_arg, mutables_arg):
        return _run_iter_local_fwd(loop_state_arg, bsw_arg, mutables_arg)[0]

      def _run_iter_local_fwd(loop_state_arg, bsw_arg, mutables_arg):
        def _run(ls, b):
          return self.run_one_iteration(
              ls, b, layers_graph, local_layers_metrics,
              mutables_arg, local_positions, local_segment_ids,
              deterministic, model_mode, logical_partition_spec_stripped,
          )
        _run_remat = jax.remat(_run, policy=remat_policy)
      ...
      # ---- Inner microbatch scan (regular jax.lax.scan, INSIDE remat body) ----
      def inner_body(inner_carry, _):
        ...
        new_ls, new_layer_state = _run_iter_local(ls, bsw_ref[0], advanced_muts)
        _, _, new_metrics, new_muts = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        new_met_arrays, _, _, _, _ = _flatten_nnx_state(new_metrics)
        return (new_ls, new_muts), _arrays_to_linen_collection(new_met_arrays, met_keys)

      if self.config.scan_pipeline_iterations:
        (new_loop_state, new_layer_mutables), inner_metrics = jax.lax.scan(
            inner_body, (loop_state, current_layer_mutables),
            None, length=num_microbatches,
        )
      else:
        scan_carry = (loop_state, current_layer_mutables)
        inner_metrics_list = []
        for _ in range(num_microbatches):
          scan_carry, step_met = inner_body(scan_carry, None)
          inner_metrics_list.append(step_met)
        new_loop_state, new_layer_mutables = scan_carry
        inner_metrics = (
            jax.tree.map(lambda *xs: jnp.stack(xs), *inner_metrics_list)
            if inner_metrics_list else _arrays_to_linen_collection(met_arrays, met_keys)
        )
    ...
    # lift.scan over REPEATS -- stage_fn (with scan inside) is the body
    ...
    linen_variables = {
        ...
        'metrics_template': _arrays_to_linen_collection(met_t_arrays, met_t_keys),
    ...
    final_layer_mutables = _unflatten_nnx_state(
        final_mut_arrays, mut_treedef, mut_is_var, mut_var_types, mut_lwp
    )
    ...
    repeat_metrics = _unflatten_nnx_state(
        repeat_met_arrays, met_treedef, met_is_var, met_var_types, met_lwp
    )
    # Reshape from (num_repeats, num_microbatches, ...) -> (total_iterations, ...)
    ...
    if bubble_iterations > 0:
      ...
      def bubble_body(inner_carry, _):
        ...
        new_ls, new_layer_state = _run_iter_custom(ls, bsw_ref[0], advanced_muts)
        _, _, new_metrics, new_muts = nnx.split(
            new_layer_state, _is_static_param, nnx.Intermediate, ...
        )
        new_met_arrays, _, _, _, _ = _flatten_nnx_state(new_metrics)
      ...
      bubble_metrics = _unflatten_nnx_state(
          bubble_met_arrays, met_treedef, met_is_var, met_var_types, met_lwp
      )
