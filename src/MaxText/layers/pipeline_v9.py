...
"""Pipeline V9: Fix vmap + stop_gradient + no scatter (Findings 1+2+5).

Variant of pipeline.py combining three fixes:
  ...
  3. (Finding 5) Remove metric scatter-update path, return per-stage metrics directly.
This addresses three of the five root causes simultaneously.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def get_main_vmap_func_for_iterations(self):
    """V9 override: vmap returns only non-param state to avoid stacking params across stages."""
    def func_to_vmap(graph, state, stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode):
      module = nnx.merge(graph, state)
      out = module(stages_inputs, stages_segment_ids, stages_positions, deterministic, model_mode)
  ...
  def run_one_iteration(
    ...
    # V9: vmap returns (output, (updated_metrics, updated_mutables))
    ...
    # V9 Finding 5: No scatter-update for metrics, only scatter mutables
    if self.config.num_pipeline_repeats > 1:
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      current_loop_state, current_layer_mutables = carry
      # V9 Finding 2: stop_gradient on mutables to prevent gradient tape on RNG state
