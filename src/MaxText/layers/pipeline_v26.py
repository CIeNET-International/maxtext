...
"""Pipeline V26: Separate weight selection from compute in run_one_iteration.

Variant targeting scan body equation count. Splits run_one_iteration into two
phases: (1) weight selection from BSW (includes shard_map), done OUTSIDE
jax.checkpoint, and (2) the forward compute pass, wrapped in its own
jax.checkpoint. This separates the shard_map equations from the compute
equations, reducing the number of ops that need to be recomputed during
backward and potentially improving XLA buffer scheduling.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def fetch_weights_for_iteration(
      self,
      loop_state,
      bsw,
      layers_metrics,
    ...
    """V26 Phase 1: Weight selection from BSW (includes shard_map).

    This phase is separated from the compute phase so it can live OUTSIDE
    jax.checkpoint. The shard_map equations for weight selection do not need
    to be recomputed during backward -- only the forward compute does.
    """
    loop_iteration = loop_state["loop_iteration"]
    microbatch_ids, _ = self.get_microbatch_and_repeat_ids(loop_iteration)
    physical_partition_spec = logical_to_mesh(logical_partition_spec, self.mesh, rules=self.config.logical_axis_rules)
    ...
    return stage_params, stage_metrics, stage_mutables, repeat_ids
  ...
  def run_one_iteration(
    ...
    """V26: Split into weight-fetch (phase 1) and compute (phase 2).

    Phase 1 (fetch_weights_for_iteration): weight selection from BSW with
    shard_map. Runs OUTSIDE any extra checkpoint boundary.
    Phase 2 (_run_compute): forward pass through decoder + scatter-update.
    Wrapped in jax.checkpoint so only this phase is recomputed during backward.
    """
    state_io = loop_state["state_io"]
    shift = loop_state["shift"]
    circ_storage = loop_state["circ_storage"]
    ...
    # Phase 1: Weight selection (outside checkpoint -- shard_map not recomputed)
    stage_params, stage_metrics, stage_mutables, repeat_ids = self.fetch_weights_for_iteration(
        loop_state, bsw, layers_metrics, current_layer_mutables, logical_partition_spec,
    )

    # Phase 2: Forward compute (inside checkpoint -- recomputed during backward)
    vmap_func = self.get_main_vmap_func_for_iterations()

    @jax.checkpoint
    def _run_compute(params, metrics, mutables, graph, inputs, seg_ids, pos, det, mode):
      weights_state = nnx.State.merge(params, metrics, mutables)
      output, (upd_metrics, upd_mutables) = vmap_func(
          graph, weights_state, inputs, seg_ids, pos, det, mode,
      )
      return output, upd_metrics, upd_mutables

    stages_output, updated_stage_metrics, updated_stage_mutables = _run_compute(
        stage_params, stage_metrics, stage_mutables,
        pipeline_weights_graph, stages_inputs, stages_segment_ids, stages_positions,
