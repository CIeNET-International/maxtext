...
def train_step(model, config, state_mesh_shardings, params_shardings, state, data, dropout_rng):
  ...
  if config.gradient_accumulation_steps > 1:
    ...
    (loss, aux), raw_grads = grad_func(model, config, data, dropout_rng, differentiable_params, *extra_dpo_args, is_train=True)
  ...
  if config.enable_nnx:
    ...
    flat_grads = {path: leaf for path, leaf in jax.tree_util.tree_leaves_with_path(grads)}
  ...
  if not config.optimizer_memory_host_offload:
    ...
    if config.enable_nnx:
      ...
      def sanitize_metric(leaf):
        if hasattr(leaf, 'dtype') and jnp.issubdtype(leaf.dtype, jnp.inexact): return leaf
        return jnp.zeros((1,), dtype=jnp.float32)
