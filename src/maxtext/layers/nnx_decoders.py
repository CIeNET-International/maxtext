...
class NNXDecoder(nnx.Module):
  ...
  def _apply_layers_sequentially(self, layers, x_in, *args, length: int, kv_caches_stacked=None, **kwargs):
    """Runs the layer stack using nnx.scan, or a static unrolled loop when kv_caches are provided.
    ...
    Args:
      ...
      *args: Positional args broadcast to every layer call.
      ...
      kv_caches_stacked: Optional Python list of per-layer KV-cache pytrees. When provided
        (e.g. by vLLM), `jax.lax.scan` is bypassed in favor of a static Python `for` loop
        so that PagedAttention's in-place buffer updates are preserved. The list is
        mutated in-place with the updated caches.
      **kwargs: Keyword args forwarded to the layer (filtered by the layer signature).

    Returns:
      (final_carry, updated_layers). When kv_caches_stacked is provided the caller's
      list is mutated in-place; the return tuple itself stays a 2-tuple to match all
      branch-side call sites.
    """
    ...
    if use_kv:
      ...
      for i in range(length):
        ...
        # Update the list in-place (mutates the caller's list reference)
        if updated_kv is not None:
          kv_caches_list[i] = updated_kv

      # During inference with vLLM, parameters do not change and we don't need
      # intermediates, so skip the state-merge/nnx.update round-trip and return
      # the original `layers` module.
      return current_carry, layers
      ...
      if cfg.scan_layers:
        if self.is_deepseek:
          ...
          if kv_caches is not None:
            ...
            # which breaks vLLM PagedAttention in-place memory updates. The static
            # unroll path inside _apply_layers_sequentially handles this case.
