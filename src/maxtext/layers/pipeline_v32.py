...
"""Pipeline V32: nnx.clone on non-differentiable state inside scan body.

Based on Flax Issue #5116 finding: nnx.merge without clone on non-diff state
(metrics, mutables) causes extra tensor tracking through jax.checkpoint.
nnx.clone creates a deep copy that breaks the trace-level link, preventing
the checkpoint boundary from saving unnecessary state copies as residuals.
"""
...
class NNXPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def scan_body(carry, _):
      ...
      # V32: clone non-diff state before merging (Flax Issue #5116)
      current_layer_state = nnx.State.merge(layers_params, nnx.clone(layers_metrics), nnx.clone(advanced_mutables))
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    def inner_body(carry, _):
      ...
      iteration = current_loop_state["loop_iteration"]
      advanced_mutables = _advance_rng_state(current_layer_mutables, iteration)
      # V32: nnx.clone on non-differentiable state before merging into module.
      # Without clone, nnx.merge inside jax.checkpoint preserves trace-level
      # links to the outer scope's metrics/mutables, causing the checkpoint
      # to save unnecessary state copies as residuals (Flax Issue #5116).
      cloned_metrics = nnx.clone(layers_metrics)
      cloned_mutables = nnx.clone(advanced_mutables)
      new_loop_state, new_layer_state = self.run_one_iteration(
          current_loop_state,
          bsw_ref[0],
          layers_graph,
