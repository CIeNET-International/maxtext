...
pipeline_n3_no_carry.py -- ABLATION variant of pipeline_n3.py.

ABLATION: w_curr removed from scan carry (2 all-gathers/repeat like v65i), but
KEEPS 2-level custom_vjp + linear_transpose. Tests whether no-carry low memory
(21.3 GB) holds with custom_vjp. Expected memory near v65i, throughput lower
than n3 (2 all-gathers).

Single targeted change vs pipeline_n3.py:
  - Outer scan carry was (loop_state, layers_mutables, w_curr); w_curr is removed
    so the carry is now (loop_state, layers_mutables). The outer scan iterates
    over repeat_index (xs = jnp.arange(num_repeats)).
  - Inside each repeat, BOTH BSW slots are recomputed fresh from layers_params
    (mirroring pipeline_v65i.execute_one_repeat): w_curr via jax.lax.cond (zeros
    for repeat 0, else weight_prefetching at iter-1) and w_next via
    weight_prefetching at iter. This does 2 all-gathers per repeat instead of 1.
  - execute_stage custom_vjp no longer takes w_curr from carry; both w_curr and
    w_next derive from pipeline_weights (layers_params) inside the forward. The
    backward maps BOTH gathered-weight gradients (d_w_curr and d_w_next) back to
    FSDP-sharded parameter space via linear_transpose and sums them into
    d_pipeline_weights. There is no w_curr carry-gradient slot anymore.

Everything else (inner per-microbatch remat custom_vjp, d+g accumulation,
class name NNXCircularPipeline) is unchanged from pipeline_n3.py.
"""
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    ABLATION: w_curr removed from scan carry. Both BSW slots are recomputed
    fresh from layers_params each repeat (2 all-gathers/repeat, like v65i).

    Architecture (matches Linen golden pattern from pipeline_utils.py):

    outer_scan(repeats, xs=arange(num_repeats)):
      carry = (loop_state, layers_mutables)   # w_curr NO LONGER carried
      layers_params as CLOSURE (scan constant, 1 copy)

      scan_body -> execute_stage(loop_state, layers_params, mutables, repeat_index)
        [Outer custom_vjp: BSW + linear_transpose]
        fwd: w_curr = cond(repeat==0, zeros, weight_prefetching(iter-1))  # 1 all-gather
             w_next = weight_prefetching(iter)                           # 1 all-gather
             BSW = (w_curr, w_next) -> inner_scan
             save (scan_vjp, w_curr_prefetch_t, w_next_prefetch_t)
        bwd: scan_vjp(g) -> d+g accumulation -> d_w_curr, d_w_next
             d_params = w_next_prefetch_t(d_w_next) + w_curr_prefetch_t(d_w_curr)
    ...
    # ---- 2-level custom_vjp nested scan structure (ABLATION: no w_curr carry) ----
    #
    # outer scan (repeats, xs=arange(num_repeats)):
    #   layers_params as CLOSURE (scan constant -- 1 copy, not per-iteration)
    #   carry = (loop_state, layers_mutables)   # w_curr NO LONGER carried
    #
    #   outer_body -> execute_stage custom_vjp:
    #     fwd: w_curr = cond(repeat==0, zeros, weight_prefetching(iter-1))
    #          w_next = weight_prefetching(iter)   -> BSW -> inner_scan_with_vjp
    #          saves (scan_vjp_fn, w_curr_prefetch_t, w_next_prefetch_t) as residuals
    #     bwd: scan_vjp_fn backprop -> d+g on BSW -> d_w_curr, d_w_next
    #          d_params = w_next_prefetch_t(d_w_next) + w_curr_prefetch_t(d_w_curr)
    #
    ...
    #
    # ABLATION: w_curr is NO LONGER a positional arg / carry input. Both BSW
    # slots (w_curr and w_next) are recomputed fresh from pipeline_weights_arg
    # inside the forward (mirroring pipeline_v65i.execute_one_repeat). This does
    # 2 all-gathers per repeat. The w_curr gradient therefore flows back into
    # pipeline_weights (layers_params), not into a carry slot, via a SECOND
    # linear_transpose. layers_params is CLOSURE of the outer jax.lax.scan.

    def _compute_w_curr(weights, total_iter, repeat_index_arg):
      """Recompute the BSW current slot from raw weights (mirrors v65i).

      Repeat 0 has no previous repeat, so the current slot is all-zeros.
      Otherwise it is the all-gather (weight_prefetching) at iter-1.
      This is linear in `weights` (both cond branches are linear), so
      jax.linear_transpose over it is well-defined.
      """
      return jax.lax.cond(
          repeat_index_arg == 0,
          lambda w: jax.tree.map(lambda x: jnp.zeros(x.shape[1:], dtype=x.dtype), w),
          lambda w: self.weight_prefetching(w, physical_partition_spec_full, total_iter - 1),
          weights,
      )

    @jax.custom_vjp
    def execute_stage(loop_state_arg, pipeline_weights_arg, mutables_arg, repeat_index_arg):
      """One pipeline repeat: prefetch weights (x2), run microbatch scan."""
      return execute_stage_fwd(loop_state_arg, pipeline_weights_arg, mutables_arg, repeat_index_arg)[0]

    def execute_stage_fwd(loop_state_arg, pipeline_weights_arg, mutables_arg, repeat_index_arg):
      """Forward: 2x weight prefetching + microbatch scan, save linear_transposes."""
      iteration = loop_state_arg["loop_iteration"]

      # Recompute BOTH BSW slots fresh from pipeline_weights (2 all-gathers).
      # w_curr: zeros for repeat 0, else gather at iter-1 (mirrors v65i).
      # w_next: gather at iter.
      w_curr = _compute_w_curr(pipeline_weights_arg, iteration, repeat_index_arg)
      ...
      # Build linear_transposes of BOTH all-gathers for backward reduce-scatter.
      # Both map a gathered-weight cotangent back to FSDP-sharded param space.
      p_weight_prefetching = functools.partial(
          self.weight_prefetching,
          physical_partition_spec=physical_partition_spec_full,
          loop_iteration=iteration,
      )
      w_next_prefetching_t = jax.linear_transpose(
          p_weight_prefetching,
          pipeline_weights_arg,
      )
      # w_curr is produced by _compute_w_curr (cond over weights); its transpose
      # captures the iter-1 gather (and the zero branch for repeat 0).
      p_compute_w_curr = functools.partial(
          _compute_w_curr,
          total_iter=iteration,
          repeat_index_arg=repeat_index_arg,
      )
      w_curr_prefetching_t = jax.linear_transpose(
          p_compute_w_curr,
          pipeline_weights_arg,
      )
      ...
      # Output: (loop_state, mutables), metrics  (w_next NO LONGER returned to carry)
      # Residuals: (scan_vjp_fn, w_curr_prefetching_t, w_next_prefetching_t, bsw)
      #   bsw kept only for its tree structure (to seed a zero passthrough grad).
      return (
          ((final_ls, final_muts), metrics),
          (scan_vjp_fn, w_curr_prefetching_t, w_next_prefetching_t, bsw),
      )

    def execute_stage_bwd(residuals, g_out):
      """Backward: scan VJP backprop + 2x linear_transpose for weight gradients."""
      scan_vjp_fn, w_curr_prefetching_t, w_next_prefetching_t, bsw = residuals
      (g_result, g_metrics) = g_out
      g_ls, g_muts = g_result

      # w_curr/w_next are recomputed inside fwd and are NOT carried/returned, so
      # the scan's bsw passthrough output has no downstream consumer: its output
      # cotangent is zero. We still must pass a structurally-correct g_bsw to
      # scan_vjp_fn. The d+g accumulation (incoming g_bsw added to scan-accumulated
      # grads) happens inside run_microbatch_scan_bwd, so a zero seed here is
      # correct -- the real BSW gradients come out as d_bsw.
      g_bsw = jax.tree.map(jnp.zeros_like, bsw)

      # Backprop through microbatch scan.
      # scan_fn returns ((final_ls, final_muts, bsw), metrics), so the cotangent
      # tuple is ((g_ls, g_muts, g_bsw), g_metrics).
      ...
      # Apply linear_transpose of BOTH all-gathers (reduce-scatter dual) to map
      # gathered-weight gradients back to FSDP-sharded parameter space, then SUM.
      # w_curr now derives from pipeline_weights (via iter-1 gather inside the
      # cond), so its gradient flows into pipeline_weights too -- there is no
      # w_curr carry slot anymore.
      (d_pipeline_weights_next,) = w_next_prefetching_t(d_w_next)
      (d_pipeline_weights_curr,) = w_curr_prefetching_t(d_w_curr)
      d_pipeline_weights = jax.tree.map(
          lambda a, b: a + b, d_pipeline_weights_next, d_pipeline_weights_curr
      )

      # Return gradients for: loop_state, pipeline_weights, mutables, repeat_index
      # repeat_index is an integer scan input -> no gradient (None).
      return d_ls, d_pipeline_weights, None, None

    execute_stage.defvjp(execute_stage_fwd, execute_stage_bwd)

    # ---- Outer scan body (repeats) ----
    # ABLATION: carry is (loop_state, layers_mutables) -- w_curr removed. The
    # scan iterates over repeat_index (xs=jnp.arange(num_repeats)) so each repeat
    # can recompute both BSW slots from layers_params inside execute_stage.

    def outer_body(carry, repeat_index):
      """One repeat: execute_stage custom_vjp with layers_params as positional arg."""
      current_loop_state, current_layer_mutables = carry

      # Align loop_iteration to this repeat's start so the in-fwd weight_prefetching
      # (and the iter-1 cond for w_curr) gathers the correct repeat (mirrors v65i).
      current_loop_state = {
          **current_loop_state,
          "loop_iteration": repeat_index * num_microbatches,
      }
      ...
      # Both BSW slots are recomputed from layers_params inside execute_stage_fwd.
      (new_loop_state, new_layer_mutables), inner_metrics = execute_stage(
          current_loop_state, layers_params, current_layer_mutables, repeat_index
      )
    ...
    # ABLATION: carry is (loop_state, layers_mutables) -- no w_curr. xs is
    # jnp.arange(num_repeats) so each repeat knows its index and recomputes BSW.
    num_repeats = self.config.num_pipeline_repeats
    repeat_indices = jnp.arange(num_repeats)
    ...
    if scan_pipeline_iters:
      ...
      for repeat_index in range(num_repeats):
        outer_carry, rep_metrics = outer_body(outer_carry, jnp.int32(repeat_index))
    ...
    if bubble_iterations > 0:
      # ABLATION: no final_w_curr from carry. Recompute the last repeat's weights
      # fresh (mirrors v65i bubble), then use a fixed BSW for the drain.
      ...
      bsw_ref[0] = (last_repeat_weights, last_repeat_weights)

      # Align loop_iteration to the start of the drain (mirrors v65i).
