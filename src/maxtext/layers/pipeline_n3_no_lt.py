...
pipeline_n3_no_lt.py -- ABLATION of pipeline_n3.py: linear_transpose removed,
weight grads via auto-diff through all_gather. Expected: higher memory than
pipeline_n3 (stores gathered weights as residuals). Roughly equivalent to
v65_fix2 (~29.4 GB).

The ONLY change vs pipeline_n3.py is in the outer custom_vjp (execute_stage):
``weight_prefetching`` is moved INSIDE the vjp'd region so ``jax.vjp``
auto-differentiates through the all_gather to produce the weight gradient,
instead of using ``jax.linear_transpose`` for a reduce-scatter dual. This
forces JAX to store the gathered weights as VJP residuals.

Differences from pipeline.py:
  - NNXCircularPipeline.__call__ rewritten with:
    * Outer custom_vjp (execute_stage): manages BSW; weight grad via auto-diff
      through weight_prefetching all-gather (NO linear_transpose).
...
class NNXCircularPipeline(NNXPipelineBase):
  """NNX circular pipeline -- ABLATION: linear_transpose removed.

  Identical to pipeline_n3.py except the outer custom_vjp (execute_stage)
  includes ``weight_prefetching`` INSIDE the vjp'd region, so the weight
  gradient is produced by auto-diff through the all_gather rather than by
  ``jax.linear_transpose``. This stores the gathered weights as VJP residuals
  and is expected to use MORE memory than pipeline_n3 (~29.4 GB, similar to
  v65_fix2).
  ...
  Key design:
    ...
    - Outer _bwd handles BSW gradients via d+g pattern; weight grad via auto-diff
  """
  ...
  def __call__(
    ...
    """NNXCircularPipeline forward pass -- ABLATION: linear_transpose removed.

    Architecture (matches Linen golden pattern from pipeline_utils.py, except
    the outer custom_vjp auto-differentiates through weight_prefetching):
    ...
    outer_scan(repeats):
      ...
      scan_body -> execute_stage(loop_state, w_curr, layers_params, mutables)
        [Outer custom_vjp: BSW; weight grad via auto-diff, NO linear_transpose]
        fwd: vjp(weight_prefetching -> BSW -> inner_scan) -> save stage_vjp
        bwd: stage_vjp(g) -> (d_loop_state, d_w_curr, d_params) [auto-diff]
    ...
    # ---- Outermost custom_vjp: execute_stage (ABLATION: no linear_transpose) ----
    #
    # ABLATION of Linen's execute_pipeline_stage_pure: weight_prefetching is
    # included INSIDE the vjp'd region, so jax.vjp auto-differentiates through
    # the all_gather to produce the weight gradient. This stores the gathered
    # weights as VJP residuals (no reduce-scatter dual via linear_transpose).
    #
    ...
    def execute_stage_fwd(loop_state_arg, w_curr_arg, pipeline_weights_arg, mutables_arg):
      """Forward: vjp over (weight_prefetching + microbatch scan) -- no linear_transpose."""
      iteration = loop_state_arg["loop_iteration"]

      # Full differentiable region: prefetch (all_gather) is INSIDE the vjp so
      # auto-diff produces d_pipeline_weights by storing the gathered weights
      # as residuals (the ablation -- no linear_transpose reduce-scatter dual).
      def _full(ls, w_curr_inner, pw_inner):
        # Prefetch next repeat's weights (all-gather)
        w_next = self.weight_prefetching(pw_inner, physical_partition_spec_full, iteration)
        # Construct Buffer Sliding Window
        bsw = (w_curr_inner, w_next)
        (final_ls, final_muts, _bsw_out), metrics = run_microbatch_scan(ls, bsw, mutables_arg)
        return (final_ls, w_next, final_muts), metrics

      # Output: (loop_state, w_next, mutables), metrics
      # Residual: stage_vjp_fn (auto-diff gives d_ls, d_w_curr, d_pipeline_weights)
      stage_output, stage_vjp_fn = jax.vjp(
          _full, loop_state_arg, w_curr_arg, pipeline_weights_arg
      )
      return stage_output, stage_vjp_fn

    def execute_stage_bwd(stage_vjp_fn, g_out):
      """Backward: auto-diff through prefetch + scan gives all gradients."""
      # g_out matches _full output: ((g_ls, g_w_next, g_muts), g_metrics)
      # stage_vjp_fn returns gradients for (loop_state, w_curr, pipeline_weights).
      d_ls, d_w_curr, d_pipeline_weights = stage_vjp_fn(g_out)
