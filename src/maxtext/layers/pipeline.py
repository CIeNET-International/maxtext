...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  See docs/CIRCULAR_PIPELINE_TECHNICAL_GUIDE.md for full explanation.
  """
  ...
  def __call__(
    ...
    def outer_body(carry, _):
      ...
      # 1. All-gather weights for this repeat.
      #    from_all_variables_to_repeat_weights selects per-stage repeat via
      #    get_microbatch_and_repeat_ids(iteration). Each stage gets correct weights.
      ...
      # 2. Tag BSW so inner jax.checkpoint saves it (prevents backward re-gather).
      cur_bsw = jax.ad_checkpoint.checkpoint_name(cur_bsw, "bsw_weights")

      # 3. Store in closure. Both slots identical → BSW select is a no-op.
