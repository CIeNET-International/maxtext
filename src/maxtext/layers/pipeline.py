...
class NNXCircularPipeline(PipelineBase):
  ...
  *current* repeat is executing.
  ...
  def __call__(
    ...
    # layers_mutables is the catch-all carried through the repeat scan via the
    # "carry_state" Linen collection. It holds RngState (RngKey/RngCount) and any
    # non_trainable variables (e.g. moe.Tid2EidVar, the DeepSeek-V4 hash-routing
    # table). Carrying non_trainable here is correct: advance_rng_state touches
    # only RngState leaves (utils/pipeline_utils.py) and passes everything else
    # through unchanged, the carry writes it back each repeat, and the custom_vjp
    # stop_gradients it (no spurious gradients). This restores Linen parity
    # (old_linen_pipeline.py:892-901 threaded the non_trainable collection through
    # the scan); the prior RngState-only assert crashed any pipelined model with a
