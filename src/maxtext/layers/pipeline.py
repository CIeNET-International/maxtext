...
def create_pipeline(
    config: Config,
    stage_factory: Callable[[nnx.Rngs], nnx.Module],
    mesh: Mesh = None,
    remat_policy: Any = None,
) -> nn.Module:
  ...
  Args:
    ...
    stage_factory: Callable[[nnx.Rngs], nnx.Module] constructing one pipeline stage.
  ...
  return cls(config=config, stage_factory=stage_factory, mesh=mesh, remat_policy=remat_policy)
