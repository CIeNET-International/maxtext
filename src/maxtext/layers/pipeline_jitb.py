...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    # ---- jit(inline=False) boundary experiment (pipeline_jitb) ----
    # Hypothesis: forcing the per-iteration recompute body behind a func.CallOp
    # (jax.jit inline=False) makes XLA schedule it OUT-OF-LINE and RELEASE the
    # recomputed f32 MLP/QKV residuals (mimicking nn.remat's separate-loop
    # release), closing the +8.5 GB vs Linen WITHOUT nn.remat. remat_policy is
    # UNCHANGED (full -> pipeline saves iteration_input+decoder_layer_input,
    # recomputes the rest). ONLY structural change vs pipeline.py.
    #
    # Leak-safety: all OUTER-TRACE TRACERS (bsw, positions, segment_ids,
    # layers_metrics) are passed as EXPLICIT ARGS; only static values (self,
    # layers_graph GraphDef, flags, specs) are closed over. self.layers params
    # are unused as arrays here (weights arrive via bsw), so the self closure
    # contributes no live tracer.
    def _inner_core(carry, bsw, positions_a, segment_ids_a, layers_metrics_a):
      current_loop_state, current_layer_mutables = carry
    ...
    if self.config.set_remat_policy_on_pipeline_iterations:
      _inner_core = jax.checkpoint(
          _inner_core,
          policy=self.get_pipeline_remat_policy(),
          prevent_cse=not self.config.scan_pipeline_iterations,
      )
    # THE EXPERIMENT: out-of-line func.CallOp boundary around the (checkpointed)
    # recompute body. inline=False forces XLA to emit it as a called computation.
    _inner_core = jax.jit(_inner_core, inline=False)

    def inner_body(carry, _):
      return _inner_core(carry, bsw_ref[0], positions, segment_ids, layers_metrics)
