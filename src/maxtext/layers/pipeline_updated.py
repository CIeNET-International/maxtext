...
_CIRCULAR_CORE_NNX_SCAN = "nnx_scan"
_CIRCULAR_CORE_JAX_STATE = "jax_state"
_CIRCULAR_CORE_ENV = "MAXTEXT_CIRCULAR_PIPELINE_CORE"
...
def _select_circular_pipeline_core() -> str:
  """Select the circular pipeline execution core for experiments."""
  selected = os.environ.get(_CIRCULAR_CORE_ENV, _CIRCULAR_CORE_NNX_SCAN)
  allowed = {_CIRCULAR_CORE_NNX_SCAN, _CIRCULAR_CORE_JAX_STATE}
  if selected not in allowed:
    raise ValueError(
        f"Invalid {_CIRCULAR_CORE_ENV}={selected!r}. "
        f"Expected one of {sorted(allowed)}."
    )
  return selected


def _assert_only_rng_mutables_in_scan_carry(mutables):
  """Assert that mutable scan carry contains only NNX RNG state variables."""
  if not __debug__:
    return
  bad_types = sorted(
      {
          type(v).__name__
          for v in jax.tree.leaves(mutables, is_leaf=lambda x: isinstance(x, nnx.Variable))
          if isinstance(v, nnx.Variable) and not isinstance(v, nnx.RngState)
      }
  )
  assert not bad_types, (
      "Non-RngState variable found in layers_mutables scan carry: "
      f"{bad_types}. Params and FP8 state must be split out of carry."
  )
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def __call__(
    ...
    circular_core = _select_circular_pipeline_core()
    if circular_core == _CIRCULAR_CORE_NNX_SCAN:
      return self._call_nnx_scan_core(
          inputs,
          segment_ids,
          positions,
          deterministic,
    ...
    if circular_core == _CIRCULAR_CORE_JAX_STATE:
      return self._call_jax_state_core(
          inputs,
          segment_ids,
          positions,
          deterministic,
    ...
    raise AssertionError(f"Unhandled circular pipeline core: {circular_core}")

  def _call_nnx_scan_core(
      self,
      inputs,
      segment_ids,
      positions,
    ...
    """Run circular pipeline with the NNX scan core."""
    inputs = inputs.reshape(
        (
            self.config.num_pipeline_microbatches,
            self.pipeline_microbatch_size,
    ...
    _assert_only_rng_mutables_in_scan_carry(layers_mutables)
    ...
    # `w_curr` is the remaining large outer-scan carry. It must stay sharded or
    # be removed before performance parity can be accepted.
  ...
  def _call_jax_state_core(
    ...
    outputs, final_state = self._run_jax_state_core(
        inputs,
        segment_ids,
        positions,
        deterministic,
    ...
    nnx.update(self.layers, final_state)
    return outputs

  def _run_jax_state_core(
      self,
      inputs,
      segment_ids,
      positions,
    ...
    """Run circular pipeline with explicit NNX graph/state carried through JAX."""
    inputs = inputs.reshape(
        (
            self.config.num_pipeline_microbatches,
            self.pipeline_microbatch_size,
    ...
    if is_linen_initializing():
      ...
      return outputs, nnx.State.merge(layers_params, layers_metrics, layers_mutables)

    # Two spec variants needed:
    # - Full spec (with circular_repeats axis) -> BSW creation inside repeat body via
    #   from_all_variables_to_repeat_weights + from_repeat_weights_to_bsw.
    # - Stripped logical spec (circular_repeats removed) -> BSW consumption via
    #   run_one_iteration, where the repeat axis has already been gathered away.
    physical_partition_spec_full = logical_to_mesh(
        logical_partition_spec, mesh=self.mesh, rules=self.config.logical_axis_rules
    )
    logical_partition_spec_stripped = pipeline_utils.strip_pipeline_repeat_logical_axis(logical_partition_spec)
    ...
    def run_single_microbatch(carry, _unused_xs, buffer_sliding_window):
      ...
      _, _, per_step_metrics, updated_mutables = nnx.split(new_layer_state, _is_static_param, nnx.Intermediate, ...)
      return (new_loop_state, updated_mutables), per_step_metrics

    def run_microbatch_scan(loop_state_in, layer_mutables_in, buffer_sliding_window, length):
      def microbatch_step(microbatch_carry, _unused):
        return run_single_microbatch(microbatch_carry, None, buffer_sliding_window)
      ...
      if self.config.scan_pipeline_iterations:
        return jax.lax.scan(
            microbatch_step,
            (loop_state_in, layer_mutables_in),
            None,
            length=length,
        )

      microbatch_carry = (loop_state_in, layer_mutables_in)
      microbatch_metrics_list = []
      for _ in range(length):
        microbatch_carry, step_metrics = microbatch_step(microbatch_carry, None)
      ...
      return microbatch_carry, microbatch_metrics
    ...
    def execute_one_repeat(carry, repeat_index):
      ...
      gathered_weights_next = self.weight_prefetching(
          ...
          total_iteration_at_repeat_start,
      )
      gathered_weights_current = jax.lax.cond(
          repeat_index == 0,
          lambda: jax.tree.map(jnp.zeros_like, gathered_weights_next),
          lambda: self.weight_prefetching(
              layers_params,
              physical_partition_spec_full,
              total_iteration_at_repeat_start - 1,
          ),
      )
      buffer_sliding_window = (gathered_weights_current, gathered_weights_next)

      current_loop_state = {**current_loop_state, "loop_iteration": total_iteration_at_repeat_start}
      return run_microbatch_scan(
          current_loop_state,
          current_layer_mutables,
          buffer_sliding_window,
          num_microbatches,
    ...
    if self.config.scan_pipeline_iterations:
      ...
      repeat_carry = (loop_state, layers_mutables)
      repeat_metrics_list = []
      for repeat_index in range(num_repeats):
        repeat_carry, repeat_step_metrics = execute_one_repeat(repeat_carry, repeat_index)
        repeat_metrics_list.append(repeat_step_metrics)
      loop_state, final_layer_mutables = repeat_carry
      repeat_metrics = (
          jax.tree.map(lambda *xs: jnp.concatenate(xs, axis=0), *repeat_metrics_list)
          if repeat_metrics_list
          else layers_metrics
    ...
    if bubble_iterations > 0:
      ...
      last_repeat_weights = self.weight_prefetching(
          layers_params,
          physical_partition_spec_full,
          total_iteration_after_repeats - 1,
      )
      ...
      (loop_state, final_layer_mutables), bubble_metrics = run_microbatch_scan(
          loop_state,
          final_layer_mutables,
          bubble_bsw,
          bubble_iterations,
      )
    ...
    final_state = nnx.State.merge(layers_params, stacked_metrics, final_layer_mutables)
    final_output = self.realign_output_microbatches(loop_state["state_io"])
    outputs = jnp.reshape(
        final_output,
        (self.config.micro_batch_size_to_train_on, self.config.max_target_length, self.config.emb_dim),
