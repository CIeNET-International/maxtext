...
pipeline_n3_sharding.py -- THROUGHPUT VARIANT: adds jax.lax.with_sharding_constraint
on prefetched weights to force async all-gather (golden Linen optimization). Tests
throughput improvement toward 116 tok/s target. Memory should match pipeline_n3.

Identical to pipeline_n3.py except for ONE targeted addition: after
``weight_prefetching`` produces the gathered weights (``w_next``), and on the
initial ``w_curr`` gather, each leaf is wrapped with
``jax.lax.with_sharding_constraint`` constrained to the post-all-gather layout
(FSDP/context/fsdp_transpose axes removed -- the same ``bsw_pps`` already derived
inside ``from_repeat_weights_to_bsw`` via
``pipeline_utils.derive_stage_weight_partition_specs``). This explicit constraint
mirrors the golden Linen code so XLA schedules the all-gather asynchronously,
overlapping it with the previous repeat's compute instead of relying on opaque
wrapper functions.

Base NNXCircularPipeline behavior (unchanged):
  - NNXCircularPipeline.__call__ with:
...
class NNXCircularPipeline(NNXPipelineBase):
  ...
  def _gathered_bsw_pps(self, physical_partition_spec):
    """Returns the post-all-gather physical partition specs for prefetched weights.

    Reuses the exact derivation already performed inside
    ``from_repeat_weights_to_bsw``: removes the FSDP/context axes that are
    all-gathered during the forward pass and slices off the leading
    repeat/stage dimension. The resulting spec describes the layout the
    gathered weights actually have (replicated across the FSDP/data axis), and
    matches the params-only tree structure of ``w_next`` / the initial
    ``w_curr``.
    """
    if physical_partition_spec is None:
      return None
    # Mirror from_repeat_weights_to_bsw's axes_to_remove exactly so the
    # constraint layout is consistent with the gather.
    axes_to_remove = ["fsdp", "fsdp_transpose", "context"]
    return pipeline_utils.derive_stage_weight_partition_specs(physical_partition_spec, axes_to_remove)

  def _constrain_to_gathered_layout(self, gathered_weights, physical_partition_spec):
    """Forces async all-gather by constraining prefetched weights to their gathered layout.

    THROUGHPUT VARIANT addition (golden Linen optimization): applies
    ``jax.lax.with_sharding_constraint`` to every leaf of the freshly gathered
    weights, pinning them to ``bsw_pps`` (the FSDP/context-removed layout). This
    explicit constraint lets XLA schedule the all-gather asynchronously,
    overlapping it with the previous repeat's compute -- instead of leaving the
    layout implicit via wrapper functions.
    """
    bsw_pps = self._gathered_bsw_pps(physical_partition_spec)
    if bsw_pps is None or self.config.shard_mode == ShardMode.AUTO:
      # No physical spec (e.g. no FSDP) or AUTO sharding: nothing to constrain.
      return gathered_weights

    def _constrain_leaf(weight, pspec):
      if pspec is None or weight is None:
        return weight
      sharding = NamedSharding(self.mesh, pspec)
      return jax.lax.with_sharding_constraint(weight, sharding)

    spec_leaves = jax.tree_util.tree_leaves(bsw_pps, is_leaf=is_spec_leaf)
    spec_iter = iter(spec_leaves)
    return jax.tree.map(lambda w: _constrain_leaf(w, next(spec_iter)), gathered_weights)
  ...
  def __call__(
    ...
    def execute_stage_fwd(loop_state_arg, w_curr_arg, pipeline_weights_arg, mutables_arg):
      ...
      # THROUGHPUT VARIANT: force async all-gather (golden Linen optimization).
      # Constrain the gathered weights to their post-all-gather layout so XLA
      # overlaps the gather with the previous repeat's compute.
      w_next = self._constrain_to_gathered_layout(w_next, physical_partition_spec_full)
    ...
    # THROUGHPUT VARIANT: constrain the initial w_curr BSW carry buffer to the
    # same post-all-gather layout as the prefetched w_next, so every BSW slot
    # (and thus every gather feeding the carry) shares the async-friendly layout.
    initial_w_curr = self._constrain_to_gathered_layout(initial_w_curr, physical_partition_spec_full)
