#!/bin/bash
# V45: NNX-native _partial_pack via nested jit(inline=True) trace boundary
#
# Attempts to replicate _partial_pack scope isolation WITHOUT Linen modules.
# Uses @functools.partial(jax.jit, inline=True) on the repeat body so that
# layers_params_raw becomes an EXPLICIT argument to a separate jaxpr function,
...
#
# If V45 -> ~23.3 GB: jit(inline=True) replicates _partial_pack's effect
# If V45 -> ~29.9 GB: the trace boundary alone is insufficient
#
# Usage: bash test_v45_compare.sh
...
cp "$DECODERS_FILE" "${DECODERS_FILE}.bak_v45"
cp "$NNX_DECODERS_FILE" "${NNX_DECODERS_FILE}.bak_v45"

restore() {
    cp "${DECODERS_FILE}.bak_v45" "$DECODERS_FILE"
    cp "${NNX_DECODERS_FILE}.bak_v45" "$NNX_DECODERS_FILE"
    rm -f "${DECODERS_FILE}.bak_v45" "${NNX_DECODERS_FILE}.bak_v45"
}
trap restore EXIT
...
v45_total="N/A"; v45_temp="N/A"; v45_tps="N/A"; v45_loss="N/A"
...
run_variant() {
    ...
    local HLO_DIR="${OUTPUT_DIR}/hlo_v45_${name}"
    rm -rf "${HLO_DIR}"
    mkdir -p "${HLO_DIR}"
    unset XLA_FLAGS
    ...
    python -m maxtext.trainers.pre_train.train \
        "${COMMON_ARGS[@]}" \
        run_name="${name}" \
        base_output_directory=/dev/shm/v45/${name} \
        dump_hlo=true \
        dump_hlo_local_dir="${HLO_DIR}" \
        dump_hlo_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/hlo_v45_${name}" \
        dump_hlo_delete_local_after=false \
        dump_jaxpr=true \
        dump_jaxpr_local_dir="${OUTPUT_DIR}/jaxpr_v45_${name}" \
        dump_jaxpr_delete_local_after=false \
        dump_jaxpr_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/jaxpr_v45_${name}" \
        2>&1 | tee "$log_file"
...
echo "================================================================"
echo "V45: NNX-native _partial_pack via jit(inline=True) trace boundary"
echo "Date: $(date)"
echo "================================================================"
echo ""
...
echo ""
echo "If V45 -> ~23.3 GB: jit(inline=True) replicates _partial_pack"
echo "If V45 -> ~29.9 GB: trace boundary alone is insufficient"
echo ""

run_variant "baseline" "pipeline"
run_variant "v45" "pipeline_v45"
...
printf "%-12s %-12s %-12s %-12s %s\n" "v45" "$v45_total" "$v45_temp" "$v45_tps" "$v45_loss"
echo ""

# Post-run: check HLO while loop count
echo "=== HLO While Loop Count ==="
for name in baseline v45; do
    hlo=$(find "${OUTPUT_DIR}/hlo_v45_${name}" -name "*jit_train_step*before_optimizations*" 2>/dev/null | head -1)
    if [ -n "$hlo" ]; then
...
for name in baseline v45; do
    jaxpr_dir="${OUTPUT_DIR}/jaxpr_v45_${name}"
