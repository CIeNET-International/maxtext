...
class MTPBlockTestModel(nnx.Module):
  ...
  def __init__(
    ...
    self.shared_embedding = embeddings.Embed(
        num_embeddings=self.config.vocab_size, num_features=self.config.base_emb_dim, config=self.config, rngs=rngs
    )
    ...
    self.multi_token_prediction_block = multi_token_prediction.MultiTokenPredictionBlock(
        config=self.config, mesh=self.mesh, name="mtp_block", transformer_layer_module=DecoderLayer, decoder=decoder_for_mtp
    )
