#!/bin/bash
# V43b: Author's pattern — params as explicit arg to jax.checkpoint
#
...
#
# Usage: bash test_v43b_compare.sh
...
cp "$DECODERS_FILE" "${DECODERS_FILE}.bak_v43b"
cp "$NNX_DECODERS_FILE" "${NNX_DECODERS_FILE}.bak_v43b"

restore() {
    cp "${DECODERS_FILE}.bak_v43b" "$DECODERS_FILE"
    cp "${NNX_DECODERS_FILE}.bak_v43b" "$NNX_DECODERS_FILE"
    rm -f "${DECODERS_FILE}.bak_v43b" "${NNX_DECODERS_FILE}.bak_v43b"
}
trap restore EXIT
...
v43b_total="N/A"; v43b_temp="N/A"; v43b_tps="N/A"; v43b_loss="N/A"
...
run_variant() {
    ...
    local HLO_DIR="${OUTPUT_DIR}/hlo_v43b_${name}"
    rm -rf "${HLO_DIR}"
    mkdir -p "${HLO_DIR}"
    unset XLA_FLAGS
    ...
    python -m maxtext.trainers.pre_train.train \
        "${COMMON_ARGS[@]}" \
        run_name="${name}" \
        base_output_directory=/dev/shm/v43b/${name} \
        dump_hlo=true \
        dump_hlo_local_dir="${HLO_DIR}" \
        dump_hlo_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/hlo_v43b_${name}" \
        dump_hlo_delete_local_after=false \
        dump_jaxpr=true \
        dump_jaxpr_local_dir="${OUTPUT_DIR}/jaxpr_v43b_${name}" \
        dump_jaxpr_delete_local_after=false \
        dump_jaxpr_gcs_dir="gs://mesa-maxtext/pipeline_loop_0521/jaxpr_v43b_${name}" \
        2>&1 | tee "$log_file"
...
echo "================================================================"
echo "V43b: Author pattern extended — ALL large state explicit to remat"
echo "Date: $(date)"
echo "Target: Linen = 23.3 GB Total, 13.6 GB Temp"
echo "================================================================"

run_variant "baseline" "pipeline"
run_variant "v43b" "pipeline_v43b"
...
printf "%-12s %-12s %-12s %-12s %s\n" "v43b" "$v43b_total" "$v43b_temp" "$v43b_tps" "$v43b_loss"
echo "================================================================"
